...
module.exports = async function(fastify) {
    ...
    fastify.addHook('preHandler', async (request, reply) => {
        ...
        if (url.includes('/api/pressing/staff')) return;
        let dhtOrderId = null;
        if (request.body && request.body.dht_order_id) {
            ...
            if (url.includes('/api/pressing/records/') || url.includes('/api/pressing/toggle/')) {
                const rec = await db.get('SELECT dht_order_id FROM pressing_records WHERE id = $1', [id]);
    ...
    fastify.post('/api/pressing/toggle/:id', { preHandler: [authenticate] }, async (req, reply) => {
        ...
        const rec = await db.get('SELECT pr.*, COALESCE(oi.production_cancelled, false) AS production_cancelled FROM pressing_records pr LEFT JOIN dht_order_items oi ON oi.id = pr.order_item_id WHERE pr.id=$1', [id]);
