...
module.exports = async function(fastify) {
    ...
    fastify.addHook('preHandler', async (request, reply) => {
        ...
        } else if (request.body && request.body.record_id && url.includes('/api/printing/gc-extend')) {
            const rec = await db.get('SELECT dht_order_id FROM printing_records WHERE id = $1', [Number(request.body.record_id)]);
        ...
        if (dhtOrderId && !isNaN(dhtOrderId)) {
            const order = await db.get('SELECT is_draft, is_locked, locked_at, (SELECT full_name FROM users WHERE users.id = dht_orders.locked_by) AS locked_by_name FROM dht_orders WHERE id = $1', [dhtOrderId]);
            if (order) {
                if (order.is_draft) {
                    return reply.code(400).send({ error: 'Đơn hàng đang trong trạng thái sửa đổi (nháp), không thể thực hiện thao tác sản xuất!' });
                }
                if (order.is_locked && order.locked_at) {
                    const lockedAt = new Date(order.locked_at);
                    const now = new Date();
                    const diffMinutes = (now - lockedAt) / (1000 * 60);
                    if (diffMinutes < 5) {
                        return reply.code(423).send({ error: `⚠️ Đơn đang được sửa bởi ${order.locked_by_name || 'sale'}, bao giờ sửa xong mới được làm` });
                    }
                }
            }
        }
    ...
    try {
        await db.run(`ALTER TABLE printing_records ADD COLUMN IF NOT EXISTS is_discarded BOOLEAN DEFAULT FALSE`);
    } catch(e) { console.error('[BPI] is_discarded column migration error:', e.message); }
    ...
    fastify.get('/api/printing/records', { preHandler: [authenticate] }, async (req) => {
        ...
        if (sort_by === 'roll' || isPetOrTemField) {
            orderBy = `ORDER BY 
                ...
                CASE WHEN COALESCE(up.pettem_roll_remaining, 0) > 0 THEN 0 ELSE 1 END ASC,
                up.pettem_roll_import_date DESC NULLS LAST,
                COALESCE(up.pettem_roll_seq, up.pettem_roll_id) DESC NULLS LAST,
                up.pettem_roll_created_at DESC NULLS LAST,
        ...
        const records = await db.all(`
            WITH unified_printing AS (
                SELECT 
                    ...
                    ptr.import_date AS pettem_roll_import_date,
                    ptr.created_at AS pettem_roll_created_at,
                    ptr.notes AS pettem_roll_notes,
                    seq_list.seq AS pettem_roll_seq,
                    o.order_code, o.shipping_priority, COALESCE(o.is_draft, false) AS is_draft, o.is_locked, o.locked_at, u_locked.full_name AS locked_by_name,
                ...
                SELECT 
                    ...
                    NULL::date AS pettem_roll_import_date,
                    NULL::timestamptz AS pettem_roll_created_at,
