...
module.exports = async function(fastify) {
    ...
    fastify.get('/api/trasoat/orders/:id/detail', { preHandler: [authenticate] }, async (request, reply) => {
        ...
        const itemsTimeline = items.map((item, idx) => {
            ...
            const itemFinishing = finishing.filter(f => f.order_item_id === item.id || itemSewingIds.has(f.sewing_record_id));
...
function _buildItemTimeline(item, isShipped, order, itemCutting, itemPrinting, itemPressing, itemSewing, itemFinishing, prodSteps, itemSchedule = null, itemReports = []) {
    ...
    if (isPetTem) {
        ...
        const checkFDone = f => {
            if (isNoSew) return f.is_completed;
        ...
        if (!isNoSew) timeline.push({ name: 'Kiểm Tra CL', short: 'QC', done: qcDone, time: qcDisplayTime, worker: qcDisplayWorker, progress: qcProgress });
