    ...
    fastify.get('/api/search/customers', { preHandler: [authenticate] }, async (request, reply) => {
        ...
        let crmQuery = `
            SELECT c.id, c.crm_type, c.customer_name, c.phone, c.phone2, c.facebook_link,
                   ...
                   (SELECT COUNT(*) FROM consultation_logs cl WHERE cl.customer_id = c.id AND cl.log_type NOT IN ('chuyen_doi_crm', 'tao_tk_affiliate', 'gui_lai_so', 'khong_xu_ly', 'dat_coc', 'chot_don', 'dang_san_xuat', 'hoan_thanh', 'sau_ban_hang', 'huy_coc', 'hoan_thanh_cap_cuu', 'huy', 'cho_duyet_huy', 'duyet_huy', 'huy_don_tra_coc', 'da_huy_don_tra_coc', 'da_huy_don', 'cho_duyet_huy_don', 'gui_hang', 'tu_van_don_tiep', 'cancel_auto_revert') AND (cl.content IS NULL OR (cl.content NOT LIKE '%Pancake%' AND cl.content NOT LIKE '%Đồng bộ%'))) as consult_count,
