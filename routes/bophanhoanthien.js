...
module.exports = async function(fastify) {
    ...
    fastify.addHook('preHandler', async (request, reply) => {
        ...
        if (url.includes('/api/finishing/staff') || url.includes('/api/finishing/display-settings')) return;
        let dhtOrderId = null;
        if (request.body && request.body.dht_order_id) {
            ...
            if (url.includes('/api/finishing/records/') || url.includes('/api/finishing/toggle/')) {
                const rec = await db.get('SELECT dht_order_id FROM finishing_records WHERE id = $1', [id]);
                if (rec) dhtOrderId = rec.dht_order_id;
            }
        } else if (request.params && request.params.finishingRecordId) {
            const finishingRecordId = Number(request.params.finishingRecordId);
            const rec = await db.get('SELECT dht_order_id FROM finishing_records WHERE id = $1', [finishingRecordId]);
            if (rec) dhtOrderId = rec.dht_order_id;
        }
        if (dhtOrderId) {
