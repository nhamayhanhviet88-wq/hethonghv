    ...
    fastify.get('/api/sewing/records', { preHandler: [authenticate] }, async (req) => {
        ...
        let queryStr = `
            SELECT sr.*, COALESCE(dt.name, u.full_name) AS sewer_name, c.name AS contractor_name,
                   ...
                   COALESCE((SELECT product_name FROM cutting_records WHERE order_item_id = sr.order_item_id ORDER BY CASE WHEN product_name LIKE '%P1%' THEN 0 ELSE 1 END, id ASC LIMIT 1), (SELECT doi.design_pdf_name FROM dht_order_items doi WHERE doi.id = sr.order_item_id)) AS cut_product_name,
