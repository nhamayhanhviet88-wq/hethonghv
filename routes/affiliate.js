...
const NET_REV_SQL = `COALESCE(
  ...
  - COALESCE((SELECT d3.discount_amount FROM dht_orders d3 WHERE d3.order_code = oc.order_code), 0)`;
...
async function _calcFirstOrderRevenue(db, affiliates, from, to) {
    ...
    if (allRefCustIds.length > 0) {
        ...
        if (firstOrderIds.length > 0) {
            ...
            const revRows = await db.all(`
                SELECT sub.customer_id, COALESCE(SUM(sub.rev), 0) as revenue FROM (
                    SELECT oc.customer_id, ${NET_REV_SQL} as rev
                    FROM order_codes oc
                    WHERE oc.id IN (${foph})${dateFilter}
                    GROUP BY oc.id, oc.customer_id, oc.order_code
                ) sub GROUP BY sub.customer_id
            `, params);
            const custRevMap = {};
    ...
    if (selfEntries.length > 0) {
        ...
        const selfOrders = await db.all(`
            SELECT oc.id as order_id, oc.customer_id, ${NET_REV_SQL} as revenue
            FROM order_codes oc
            JOIN customers c ON c.id = oc.customer_id
...
async function affiliateRoutes(fastify) {
    ...
    fastify.get('/api/affiliate/detail-stats', { preHandler: [authenticate, requireRole('giam_doc', 'quan_ly', 'quan_ly_cap_cao', 'truong_phong', 'nhan_vien', 'part_time')] }, async (request, reply) => {
        ...
        if (type === 'customers') {
            ...
            const dataQ = `
                SELECT c.id, c.customer_name, c.phone, c.crm_type, c.order_status, c.created_at,
                       ...
                       COALESCE((SELECT SUM(sub_rev.rev) FROM (SELECT COALESCE((SELECT SUM(di.item_total) FROM dht_orders d JOIN dht_order_items di ON di.dht_order_id = d.id WHERE d.order_code = oc3.order_code), (SELECT SUM(oi3.total) FROM order_items oi3 WHERE oi3.order_code_id = oc3.id), 0) - COALESCE((SELECT d3.vat_amount FROM dht_orders d3 WHERE d3.order_code = oc3.order_code), 0) - COALESCE(oc3.discount_amount, 0) as rev FROM order_codes oc3 WHERE oc3.customer_id = c.id AND COALESCE(oc3.status, 'active') != 'cancelled' AND EXISTS (SELECT 1 FROM consultation_logs cl WHERE cl.customer_id = c.id AND cl.log_type = 'chot_don')) sub_rev), 0) as customer_revenue
                FROM customers c
        ...
        if (type === 'orders' || type === 'revenue') {
            // ★ FIRST-ORDER RULE: only show attributable orders
            const affIdsQ = `SELECT u.id, u.source_customer_id, u.assigned_to_user_id FROM users u WHERE ${affIdFilter}`;
            const affRows = await db.all(affIdsQ, affFilterParams);
            const affIds = affRows.map(r => r.id);
            if (affIds.length === 0) return { total: 0, page: 1, limit: Number(limit), data: [] };

            // ═══ Compute attributable order IDs (same logic as _calcFirstOrderRevenue) ═══
            const attributableOrderIds = new Set();
            const ph = affIds.map(() => '?').join(',');
            ...
            const allRefCustIds = referredCusts.map(c => c.id);
            if (allRefCustIds.length > 0) {
                const foMap = await _getFirstOrderMap(db, allRefCustIds);
                Object.values(foMap).filter(Boolean).forEach(oid => attributableOrderIds.add(oid));
            }

            // PART B: Self-purchase orders
            const selfEntries = affRows.filter(a => a.source_customer_id).map(a => ({
                affId: a.id, custId: a.source_customer_id, hasParent: !!a.assigned_to_user_id
            }));
            if (selfEntries.length > 0) {
                const selfCustIds = selfEntries.map(e => e.custId);
                const selfFoMap = await _getFirstOrderMap(db, selfCustIds);
                const firstOrderIdsToExclude = new Set();
                selfEntries.forEach(e => {
                    if (e.hasParent && selfFoMap[e.custId]) firstOrderIdsToExclude.add(selfFoMap[e.custId]);
                });
                const sph = selfCustIds.map(() => '?').join(',');
                const selfOrders = await db.all(`
                ...
                `, selfCustIds);
                selfOrders.forEach(o => {
                    if (!firstOrderIdsToExclude.has(o.id)) attributableOrderIds.add(o.id);
                });
            }

            // PART C: First orders from child affiliates (exclude already in PART A)
            const refCustIdSet = new Set(allRefCustIds);
            const childAffs = await db.all(`
            ...
            if (attributableOrderIds.size === 0) return { total: 0, page: 1, limit: Number(limit), data: [] };

            // ═══ Query with first-order filter ═══
            const oidArr = [...attributableOrderIds];
            const oidPh = oidArr.map(() => '?').join(',');
            let dateFilter = '';
            const dateParams = [];
            if (from) { dateFilter += ' AND oc.created_at >= ?'; dateParams.push(from); }
            if (to) { dateFilter += ' AND oc.created_at <= ?'; dateParams.push(to + ' 23:59:59'); }

            const countR = await db.get(`SELECT COUNT(*) as total FROM order_codes oc WHERE oc.id IN (${oidPh}) ${dateFilter}`, [...oidArr, ...dateParams]);
    ...
    fastify.get('/api/affiliate/commission', { preHandler: [authenticate] }, async (request, reply) => {
        ...
        if (customerIds.length > 0) {
            ...
            let innerWhere = `oc.customer_id IN (${cph2}) AND oc.status = 'completed'`;
            const compRevParams = [...customerIds];
            if (selfCreatedAt) {
                innerWhere += ` AND oc.created_at >= ?`;
                compRevParams.push(selfCreatedAtRaw);
            }
            const compRevQuery = `
                SELECT sub.customer_id, COALESCE(SUM(sub.rev), 0) as completed_revenue FROM (
                    SELECT oc.customer_id, ${NET_REV_SQL} as rev
                    FROM order_codes oc
                    WHERE ${innerWhere}
                    GROUP BY oc.id, oc.customer_id, oc.order_code
        ...
        if (customerIds.length > 0) {
            ...
            let totalRevQuery = `
                ...
                FROM (
                    SELECT oc.customer_id, ${NET_REV_SQL} as net_rev
                    FROM order_codes oc
            ...
            totalRevQuery += ` GROUP BY oc.id, oc.customer_id, oc.order_code) sub GROUP BY sub.customer_id`;
        ...
        if (filteredIds.length > 0) {
            ...
            commQuery += ` GROUP BY oc.id, oc.customer_id, oc.order_code, oc.created_at`;
            ...
            if (selfCustId) {
                ...
                if (selfCreatedAt) {
                    const selfTotalRow = await db.get(`
                        SELECT COALESCE(SUM(sub.rev), 0) as total FROM (
                            SELECT ${NET_REV_SQL} as rev
                            FROM order_codes oc
                            WHERE oc.customer_id = ? AND (oc.status IS NULL OR oc.status != 'cancelled')
                            AND oc.created_at >= ?
    ...
    fastify.get('/api/affiliate/all-orders', { preHandler: [authenticate] }, async (request, reply) => {
        ...
        ordQuery += ` GROUP BY oc.id, oc.order_code, oc.status, oc.created_at, oc.customer_id
            ORDER BY oc.created_at DESC`;
    ...
    fastify.get('/api/affiliate/balance', { preHandler: [authenticate] }, async (request, reply) => {
        ...
        if (filteredIdsB.length > 0) {
            ...
            balQuery += ` GROUP BY oc.id, oc.customer_id, oc.order_code, oc.created_at`;
            ...
            for (const aff of affiliates) {
                ...
                if (filteredIdsS.length > 0) {
                    ...
                    const orderRowsS = await db.all(`
                        ...
                        FROM order_codes oc
                        WHERE oc.customer_id IN (${cph}) AND oc.status = 'completed'
                        GROUP BY oc.id, oc.customer_id, oc.order_code, oc.created_at
                    `, filteredIdsS);
    ...
    fastify.get('/api/affiliate/leaderboard', { preHandler: [authenticate] }, async (request, reply) => {
        ...
        if (allCustomerIds.length > 0) {
            ...
            let q = `SELECT sub.customer_id, COUNT(*) as total_orders, COALESCE(SUM(sub.rev), 0) as total_revenue
                FROM (SELECT oc.id, oc.customer_id, ${NET_REV_SQL} as rev
                    FROM order_codes oc
                    WHERE oc.customer_id IN (${cph}) AND oc.status = 'completed'`;
            ...
            q += ` GROUP BY oc.id, oc.customer_id, oc.order_code) sub GROUP BY sub.customer_id`;
        ...
        for await (const part of parts) {
            if (part.type === 'file' && part.file) {
                ...
                if (buffer.length > 0) {
                    ...
                    const filename = `${part.fieldname}_${Date.now()}_${Math.random().toString(36).slice(2)}.jpeg`;
    ...
    fastify.get('/api/affiliate/stat-detail', { preHandler: [authenticate, requireRole('giam_doc', 'quan_ly', 'quan_ly_cap_cao', 'truong_phong', 'nhan_vien')] }, async (request, reply) => {
        ...
        if (type === 'customers') {
            ...
            if (rows.length > 0) {
                ...
                const revRows = await db.all(`SELECT sub.customer_id, COALESCE(SUM(sub.rev), 0) as revenue FROM (SELECT oc.customer_id, ${NET_REV_SQL} as rev FROM order_codes oc WHERE oc.customer_id IN (${cph}) AND oc.status='completed' GROUP BY oc.id, oc.customer_id, oc.order_code) sub GROUP BY sub.customer_id`, cids);
                const revMap = {};
        ...
        if (type === 'orders' || type === 'revenue') {
            ...
            if (attributableOrderIds) {
                ...
                // ═══ Global view: compute attributable order IDs for ALL affiliates ═══
                attributableOrderIds = new Set();
                const ph = affIds.map(() => '?').join(',');

                // PART A: First orders from ALL referred customers
                const allRefCusts = await db.all(`SELECT id, referrer_id FROM customers WHERE referrer_id IN (${ph})`, affIds);
                const allRefCustIds = allRefCusts.map(c => c.id);
                if (allRefCustIds.length > 0) {
                    const foMap = await _getFirstOrderMap(db, allRefCustIds);
                    Object.values(foMap).filter(Boolean).forEach(oid => attributableOrderIds.add(oid));
                }

                // PART B: Self-purchase orders for each affiliate
                const selfEntries = allAffs.filter(a => a.source_customer_id).map(a => ({
                    affId: a.id, custId: a.source_customer_id, hasParent: !!a.assigned_to_user_id
                }));
                if (selfEntries.length > 0) {
                    const selfCustIds = selfEntries.map(e => e.custId);
                    ...
                    const sph = selfCustIds.map(() => '?').join(',');
                    const selfOrders = await db.all(`
                        SELECT oc.id, oc.customer_id FROM order_codes oc
                        JOIN customers c ON c.id = oc.customer_id
                        WHERE oc.customer_id IN (${sph})
                          AND COALESCE(oc.status, 'active') != 'cancelled'
                          AND COALESCE(c.cancel_approved, 0) != 1
                ...
                // PART C: First orders from child affiliates (excluding already counted in PART A)
                const refCustIdSet = new Set(allRefCustIds);
                const childAffsAll = await db.all(`
                    SELECT u.source_customer_id, u.assigned_to_user_id FROM users u
                    WHERE u.assigned_to_user_id IN (${ph})
                    AND u.role IN ('hoa_hong','ctv','nuoi_duong','sinh_vien','tkaffiliate')
                    AND u.source_customer_id IS NOT NULL
                `, affIds);
                const uniqueChildAffsAll = childAffsAll.filter(c => !refCustIdSet.has(c.source_customer_id));
                if (uniqueChildAffsAll.length > 0) {
                    const childCustIds = uniqueChildAffsAll.map(c => c.source_customer_id);
                    const childFoMap = await _getFirstOrderMap(db, childCustIds);
                    Object.values(childFoMap).filter(Boolean).forEach(oid => attributableOrderIds.add(oid));
                }

                if (attributableOrderIds.size === 0) return { success: true, rows: [], total: 0 };
                const oidArr2 = [...attributableOrderIds];
                orderWhere = [`oc.id IN (${oidArr2.map(() => '?').join(',')})`];
                orderParams = [...oidArr2];
            }
