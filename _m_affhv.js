...
(async () => {
    ...
    for (const [k,l,i,co,tc,so,seo] of btns) {
        await db.run(`INSERT INTO consult_type_configs (key,label,icon,color,text_color,is_active,sort_order,section_order,crm_menu,rule_phase)
            VALUES ('${k}','${l}','${i}','${co}','${tc}',true,${so},${seo},'affiliate_hv','huy_capuu')
    ...
    const rules = [
        ...
        ['cho_duyet_huy','cho_duyet_huy',false,0,1],
    ];
    for (const [f,t,d,dl,s] of rules) {
        await db.run(`INSERT INTO consult_flow_rules (from_status,to_type_key,is_default,delay_days,sort_order,crm_menu)
            VALUES ('${f}','${t}',${d},${dl},${s},'affiliate_hv')
            ON CONFLICT DO NOTHING`);
    }
    const c = await db.get(`SELECT COUNT(*) as c FROM consult_type_configs WHERE crm_menu='affiliate_hv'`);
    const r = await db.get(`SELECT COUNT(*) as c FROM consult_flow_rules WHERE crm_menu='affiliate_hv'`);
    console.log(`Done: ${c.c} buttons, ${r.c} rules for affiliate_hv`);
    process.exit(0);
})();
