...
async function _dhtShowCreateFree() {
    ...
    var body = '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-bottom:0">'
        ...
        +'<div class="form-group"><label>Khuyến Mãi Tặng Áo</label><input id="_co_promoGift" class="form-control" value="Không có" disabled style="'+_dis+';font-weight:700;color:#3b82f6"></div>'
        +'</div>'
        +'<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:8px">'
...
async function _dhtGoStep2() {
    ...
    if (isEdit) {
        if (isDraftOrder) {
            footer += '<button class="btn" onclick="_dhtSubmitEditV2(true)" style="background:linear-gradient(135deg,#f59e0b,#d97706);color:#fff;border:none;padding:8px 24px;border-radius:8px;font-weight:800;margin-left:8px">📝 Lưu Nháp</button>'
                   + '<button class="btn" onclick="_dhtSubmitEditV2(false)" style="background:linear-gradient(135deg,#059669,#10b981);color:#fff;border:none;padding:8px 24px;border-radius:8px;font-weight:800;margin-left:8px">💾 Lưu Chính Thức</button>';
        } else {
...
function _dhtSearchOrderCode() {
    ...
    _dhtCodeTimer = setTimeout(async function() {
        var q = (document.getElementById('_co_code')?.value || '').toLowerCase().trim();
        ...
        if (filteredCodes.length > 0) {
            html += '<div style="padding:4px 10px;background:#f8fafc;font-size:10px;font-weight:800;color:#64748b;border-bottom:1.5px solid #e2e8f0;letter-spacing:0.5px">MÃ ĐƠN CHỜ (CRM)</div>';
            html += filteredCodes.map(function(c) {
        ...
        if (customers.length > 0) {
            html += '<div style="padding:4px 10px;background:#fffbeb;font-size:10px;font-weight:800;color:#d97706;border-bottom:1.5px solid #fde68a;margin-top:4px;letter-spacing:0.5px">KHÁCH HÀNG (ĐỂ LƯU NHÁP)</div>';
            html += customers.map(function(c) {
                // escape quotes for JSON string
                var dataStr = JSON.stringify(c).replace(/'/g, '&#39;').replace(/"/g, '&quot;');
                return '<div style="padding:10px 12px;cursor:pointer;border-bottom:1px solid #f1f5f9;font-size:12px"'
                    +' onmouseover="this.style.background=\'#fef3c7\'" onmouseout="this.style.background=\'\'"'
                    +' onclick="_dhtPickCustomerForDraft(\''+dataStr+'\')"'
                    +'><b style="color:#d97706">👤 '+c.customer_name+'</b>'
                    +' — <span style="color:#2563eb">'+c.phone+'</span>'
                    +(c.source_name ? ' <span style="font-size:10px;color:#6b7280">('+c.source_name+')</span>' : '')+'</div>';
            }).join('');
        }

        if (filteredCodes.length === 0 && customers.length === 0) {
            html = '<div style="padding:12px;text-align:center;font-size:11px;color:#9ca3af">Không tìm thấy mã đơn chờ hoặc khách hàng.<br><span style="color:#dc2626;font-weight:600">Gõ từ khóa khác hoặc tạo Khách hàng ở CRM trước.</span></div>';
        }
...
function _dhtPickCustomerForDraft(cStr) {
    try {
        ...
        _dhtCreate.orderCode = '';
        _dhtCreate.isDraftSelected = true;
        
        var codeInput = document.getElementById('_co_code');
        if (codeInput) {
            codeInput.value = '📝 ĐƠN NHÁP (Chưa có mã)';
            codeInput.style.background = '#fffbeb';
            codeInput.style.fontWeight = '900';
            codeInput.style.fontSize = '14px';
            codeInput.style.color = '#d97706';
        }
        
        document.getElementById('_co_custId').value = c.id;
        document.getElementById('_co_phone').value = c.phone || '';
        document.getElementById('_co_name').value = c.customer_name || '';
        document.getElementById('_co_src').value = c.source_name || '';
        document.getElementById('_co_addr').value = c.address || '';
        ...
        var depInfo = document.getElementById('_co_depositInfo');
        if (depInfo) {
            depInfo.innerHTML = '💰 Chưa liên kết cọc (Lưu Nháp)';
            depInfo.style.color = '#d97706';
        }
        
        document.getElementById('_co_codeList').style.display = 'none';
        _dhtCalcTotal();
    } catch(e) {
        console.error('Error picking customer for draft:', e);
        showToast('Lỗi chọn khách hàng', 'error');
    }
}
...
async function _dhtAddItem(editIdx) {
    ...
    ov.innerHTML='<div style="background:#fff;border-radius:12px;padding:20px;width:500px;max-height:85vh;overflow-y:auto;box-shadow:0 8px 32px rgba(0,0,0,0.2)">'
        ...
        +'</div>'
        +'<div id="_pp_patternContainer" style="display:grid;grid-template-columns:1fr;gap:8px;margin-bottom:8px">'+sfPat+'</div>'
        ...
        +'<div id="_pp_sewMatContainer" style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:8px"><div><label style="font-size:11px;font-weight:700">✂️ Chi Tiết May Thêm</label><div id="_ppSewTags" style="display:flex;flex-wrap:wrap;gap:3px;min-height:24px;padding:4px;border:1px solid #e2e8f0;border-radius:4px;background:#f8fafc;margin-bottom:4px"></div>'+(isRestricted ? '' : '<button type="button" onclick="_ppOpenBgmPicker()" style="background:#6366f1;color:#fff;border:none;padding:3px 10px;border-radius:4px;font-size:10px;font-weight:700;cursor:pointer">➕ Chọn</button>')+'</div><div><label style="font-size:11px;font-weight:700">Vật Liệu Kèm</label><select id="_pp_extraMat" class="form-control" style="font-size:12px" multiple'+(isRestricted?' disabled':'')+'>'+( extOpts||noOpt)+'</select></div></div>'
    ...
    // Add change event listener to process steps for dynamic show/hide
...
async function _ppApplyGiftCode() {
    var inp = document.getElementById('_pp_promoGiftInput');
    var qtyHidden = document.getElementById('_pp_promoGiftQty');
    var codeHidden = document.getElementById('_pp_promoGiftCode');
    var msgEl = document.getElementById('_pp_giftPromoMsg');
    ...
    try {
        ...
        if (res && res.valid) {
            if (res.promo_type !== 'gift') {
                ...
                if (msgEl) {
                    msgEl.innerText = '❌ Mã giảm giá % (không áp dụng)';
                    msgEl.style.color = '#dc2626';
                }
                qtyHidden.value = 0;
                codeHidden.value = '';
            ...
            _ppCalc();
            showToast('✅ Áp dụng mã tặng áo thành công!');
        } else {
            showToast('❌ ' + (res.error || 'Mã không hợp lệ hoặc đã hết hạn.'), 'error');
            if (msgEl) {
                msgEl.innerText = '❌ Mã không hợp lệ';
                msgEl.style.color = '#dc2626';
            }
            qtyHidden.value = 0;
            codeHidden.value = '';
        ...
        console.error('Check gift promo error:', e);
...
function _dhtSavePhieu(idx) {
    ...
    var va=Math.round(raw*vp/100);
    var sewArr = isCutting ? ((window._ppSewItems || []).slice()) : [];
    var extArr = isCutting ? (Array.from(document.getElementById('_pp_extraMat')?.selectedOptions||[]).map(function(o){return o.value;})) : [];
    ...
    _dhtCreate.phieuItems[idx]={
        ...
        material_id:mainPair?mainPair.material_id:null,
        ...
        reminders:nnArr,
        accounting_notes:acctNotes,
        extra_materials:extArr,
        quantities:qtyPairs,
        vat_percent:vp,
        vat_amount:va,
        raw_total:raw,
        ...
        quantity:typedQty,
        unit_price:qtyPairs[0]?.price||0,
        promo_gift_quantity:giftQty,
        promo_gift_code:giftCode,
        promo_gift_apply_row_index:selectedRowIdx,
        has_fabric_called:!!existing.has_fabric_called,
        has_print_assignment:!!existing.has_print_assignment,
        has_cutting_started:!!existing.has_cutting_started,
...
function _dhtRenderPhieuRows() {
    ...
    _dhtCreate.phieuItems.forEach(function(p,i){
        ...
        var catSel = document.getElementById('_co_cat');
        var catName = catSel ? (catSel.options[catSel.selectedIndex]?.text || '') : '';
        var isOrderCategoryFree = (catName === 'PET' || catName === 'TEM');
        var isFree = isOrderCategoryFree && (!p.material_pairs || p.material_pairs.length === 0);
        ...
        var infoBadge = '';
        if (p.material_name || p.color_name) {
            infoBadge = ' <span style="font-size:10px;color:#6b7280">(' + (p.material_name || '') + '/' + (p.color_name || '') + ')</span>';
        } else {
            infoBadge = ' <span style="font-size:10px;color:#ef4444;font-weight:700">(Không Cắt)</span>';
        }
        var label = isFree
            ? '🐾 Phiếu ' + catName + ' #'+(i+1)+' — '+p.product_name + giftBadge
            : '📋 #'+(i+1)+' '+p.product_name + infoBadge + giftBadge;
...
async function _dhtSubmitCreateV2(isDraft) {
    ...
    if (data.success) {
        ...
        showToast('✅ ' + (isDraft ? 'Đã lưu đơn nháp thành công!' : (_dhtRepairData ? 'Đã tạo đơn sửa! Mã: ' + generatedCode : 'Đã tạo đơn thành công!' + (isFree ? ' Mã: ' + generatedCode : ''))));
...
async function _dhtSubmitEditV2(isDraft) {
    ...
    // Handle proof image for CHUẨN and validate time + image (skip for drafts)
