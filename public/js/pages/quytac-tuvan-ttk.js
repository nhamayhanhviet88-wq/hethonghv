...
let _qtTAllRules = {};
let _qtTStages = [];
let _qtTSections = [];      // dynamic sections with section_order
let _qtTUnsectioned = [];   // types without a section
let _qtTGroupMembers = [];  // buttons belonging to a group (section_order=0)
let _qtTRulePhases = [];    // dynamic phase groups (PHẦN 1, 2, 3...)
let _qtTIsGD = false;
let _qtTActiveTab = localStorage.getItem('qt_active_tab') || 'buttons';
let _qtTSortDebounce = null;

// ========== MAIN ENTRY ==========
async function renderQuyTacTuVanTtkPage(container) {
    _qtTIsGD = currentUser && currentUser.role === 'giam_doc';

    container.innerHTML = `
        <div class="qt-page">
            <a class="qt-back" href="/crm-tu-tim-kiem" onclick="event.preventDefault();navigate('crm-tu-tim-kiem')">← Quay lại CRM Tự Tìm Kiếm</a>
            <div class="qt-header">
                <h2 class="qt-header-title">⚙️ Quy Tắc Nút Tư Vấn</h2>
                <span class="qt-header-badge">
                    ${_qtTIsGD ? '🔓 Chế độ chỉnh sửa' : '👁️ Chế độ xem'}
                </span>
            </div>
            <div class="qt-tabs">
                <div class="qt-tab${_qtTActiveTab === 'buttons' ? ' active' : ''}" onclick="_qtTSwitchTab('buttons')" id="qtTabButtons">📋 Danh Sách Nút (0)</div>
                <div class="qt-tab${_qtTActiveTab === 'rules' ? ' active' : ''}" onclick="_qtTSwitchTab('rules')" id="qtTabRules">🔄 Quy Tắc Liên Kết</div>
            </div>
            <div class="qt-panel" id="qtPanel">${_qtTRenderSkeleton()}</div>
        </div>
    `;
...
// ========== DATA LOADING ==========
async function _qtTLoadData() {
    const [typesData, rulesData, stagesData, sectionsData, phasesData] = await Promise.all([
        apiCall('/api/consult-types?crm_menu=tu_tim_kiem'),
        apiCall('/api/consult-flow-rules?crm_menu=tu_tim_kiem'),
        apiCall('/api/consult-stages'),
        apiCall('/api/consult-sections?crm_menu=tu_tim_kiem'),
        apiCall('/api/consult-rule-phases')
    ]);
    _qtTAllTypes = typesData.types || [];
    _qtTAllRules = rulesData.rules || {};
    _qtTStages = stagesData.stages || [];
    _qtTSections = sectionsData.sections || [];
    _qtTUnsectioned = (sectionsData.unsectioned || []).filter(t => !['cap_cuu_sep', 'huy'].includes(t.key));
    _qtTGroupMembers = sectionsData.groupMembers || [];
    _qtTRulePhases = phasesData.phases || [];
    _qtTRulePhases.sort((a, b) => (a.sort_order || 0) - (b.sort_order || 0));
    // Sort stages by sort_order
    _qtTStages.sort((a, b) => (a.sort_order || 0) - (b.sort_order || 0));

    document.getElementById('qtTabButtons').innerHTML = `📋 Danh Sách Nút (${_qtTAllTypes.length})`;
    _qtTRenderActiveTab();
}

// ========== TAB SWITCHING ==========
function _qtTSwitchTab(tab) {
    _qtTActiveTab = tab;
...
function _qtTRenderActiveTab() {
    if (_qtTActiveTab === 'buttons') _qtTRenderButtons();
    else _qtTRenderRules();
}

// ========== TAB 1: BUTTONS (Dynamic Stages) ==========
function _qtTRenderButtons() {
    ...
    if (_qtTIsGD) {
        html += `<div class="qt-action-bar" style="gap:8px;">
            <button class="qt-flow-edit-btn" style="background:linear-gradient(135deg,#10b981,#059669);" onclick="_qtTShowAddStageModal()">📁 Thêm giai đoạn</button>
            <button class="qt-flow-edit-btn" onclick="_qtTShowAddTypeModal()">➕ Thêm nút mới</button>
        </div>`;
    }

    // Render each stage from dynamic config
    for (const stage of _qtTStages) {
        const stageTypes = _qtTAllTypes.filter(t => t.stage === stage.id);
        html += `
            <div class="qt-stage" data-stage-id="${stage.id}">
                <div class="qt-stage-header" style="background:${stage.gradient};">
                    ...
                    ${_qtTIsGD ? `
                        <button onclick="event.stopPropagation();_qtTShowEditStageModal('${stage.id}')" style="background:none;border:none;cursor:pointer;font-size:14px;margin-left:4px;" title="Sửa giai đoạn">✏️</button>
                        <button onclick="event.stopPropagation();_qtTDeleteStage('${stage.id}')" style="background:none;border:none;cursor:pointer;font-size:14px;" title="Xóa giai đoạn">🗑️</button>
                    ` : ''}
                </div>
                <div class="qt-btn-grid" data-stage="${stage.id}">
                    ${stageTypes.map(t => _qtTRenderButtonCard(t)).join('')}
                </div>
            </div>
        `;
    }
    ...
    const unassigned = _qtTAllTypes.filter(t => !t.stage || !_qtTStages.find(s => s.id === t.stage));
        ...
        html += `
            <div class="qt-stage" data-stage-id="_other">
                ...
                <div class="qt-btn-grid" data-stage="_other">
                    ${unassigned.map(t => _qtTRenderButtonCard(t)).join('')}
                </div>
            </div>
        `;
    }
    ...
    if (_qtTIsGD) {
        panel.addEventListener('click', function(e) {
            ...
            if (editBtn) {
                ...
                if (card && card.dataset.key) _qtTShowEditTypeModal(card.dataset.key);
                return;
            }
            ...
            if (delBtn) {
                ...
                if (card && card.dataset.key) _qtTDeleteType(card.dataset.key);
                return;
            }
        }, true);
    }
    ...
    if (_qtTIsGD && typeof Sortable !== 'undefined') {
        document.querySelectorAll('.qt-btn-grid').forEach(grid => {
            new Sortable(grid, {
                ...
                group: 'buttons',
                onEnd: _qtTOnButtonSortEnd
            });
        });
    }
}

function _qtTRenderButtonCard(t) {
    return `
        <div class="qt-btn-card ${t.is_active ? '' : 'inactive'}" data-key="${t.key}" style="--card-accent:${t.color}">
            ${_qtTIsGD ? `<span class="qt-drag-hint">⠿</span>` : ''}
            ${_qtTIsGD ? `<button class="qt-edit-btn" type="button">✏️</button>` : ''}
            ${_qtTIsGD ? `<button class="qt-del-btn" type="button">🗑️</button>` : ''}
...
function _qtTOnButtonSortEnd() {
    clearTimeout(_qtTSortDebounce);
    _qtTSortDebounce = setTimeout(() => {
        const orders = [];
        let globalOrder = 0;
        document.querySelectorAll('.qt-btn-grid').forEach(grid => {
        ...
        _qtTSaveSortOrder(orders);
    }, 500);
}

// ========== STAGE MANAGEMENT ==========
function _qtTShowAddStageModal() {
    const presetsHTML = TTK_STAGE_PRESETS.map((p, i) => `
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                ...
                <button class="qt-btn qt-btn-primary" onclick="_qtTAddStage()">📁 Thêm giai đoạn</button>
            </div>
        </div>
    `;
...
async function _qtTAddStage() {
    ...
    const preset = TTK_STAGE_PRESETS[presetIdx];
    ...
    const newStage = {
        ...
        sort_order: _qtTStages.length + 1
    };

    _qtTStages.push(newStage);
    await apiCall('/api/consult-stages', 'PUT', { stages: _qtTStages });
    document.querySelector('.qt-modal-overlay')?.remove();
    showToast('✅ Đã thêm giai đoạn!', 'success');
    _qtTRenderButtons();
}

function _qtTShowEditStageModal(stageId) {
    const stage = _qtTStages.find(s => s.id === stageId);
    if (!stage) return;
    ...
    const currentPresetIdx = TTK_STAGE_PRESETS.findIndex(p => p.gradient === stage.gradient);
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtTSaveStage('${stageId}')">💾 Lưu</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtTSaveStage(stageId) {
    const stage = _qtTStages.find(s => s.id === stageId);
    if (!stage) return;
...
async function _qtTDeleteStage(stageId) {
    const stage = _qtTStages.find(s => s.id === stageId);
    if (!stage) return;
    const count = _qtTAllTypes.filter(t => t.stage === stageId).length;
    if (!confirm(`Xóa giai đoạn "${stage.title}"?\n${count > 0 ? `⚠️ ${count} nút sẽ chuyển về "Chưa phân loại"` : ''}`)) return;

    // Remove stage from config
    _qtTStages = _qtTStages.filter(s => s.id !== stageId);
    await apiCall('/api/consult-stages', 'PUT', { stages: _qtTStages });

    // Clear stage from buttons
    const orders = _qtTAllTypes.filter(t => t.stage === stageId).map((t, i) => ({ key: t.key, sort_order: 999 + i, stage: null }));
    if (orders.length > 0) {
...
// ========== DELETE TYPE ==========
async function _qtTDeleteType(key) {
    const t = _qtTAllTypes.find(x => x.key === key);
    if (!t) return;
...
// ========== EDIT TYPE MODAL ==========
function _qtTShowEditTypeModal(key) {
    const t = _qtTAllTypes.find(x => x.key === key);
    if (!t) return;

    const stageOptions = _qtTStages.map(s => `<option value="${s.id}" ${t.stage === s.id ? 'selected' : ''}>${s.icon} ${s.title}</option>`).join('');
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtTSaveType('${key}')">💾 Lưu</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtTSaveType(key) {
    const data = {
...
function _qtTShowAddTypeModal() {
    const stageOptions = _qtTStages.map(s => `<option value="${s.id}">${s.icon} ${s.title}</option>`).join('');
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtTAddType()">➕ Thêm</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtTAddType() {
    ...
    await apiCall('/api/consult-types', 'POST', {...data, crm_menu: 'tu_tim_kiem'});
    document.querySelector('.qt-modal-overlay')?.remove();
...
function _qtTRenderRules() {
    const panel = document.getElementById('qtPanel');
    let html = _qtTRenderFlowchart();
    ...
    if (_qtTIsGD) {
        const unsecCount = _qtTUnsectioned.length;
        html += `<div class="qt-action-bar" style="gap:8px;">
            <button class="qt-flow-edit-btn" style="background:linear-gradient(135deg,#10b981,#059669);" onclick="_qtTShowAddPhaseModal()">📂 Thêm Phần</button>
            <button class="qt-flow-edit-btn" onclick="_qtTShowAddRuleGroupModal()">➕ Thêm nhóm quy tắc mới ${unsecCount > 0 ? '<span style="background:#ef4444;color:white;padding:1px 7px;border-radius:10px;font-size:10px;margin-left:4px;">' + unsecCount + '</span>' : ''}</button>
        </div>`;
    }

    // ★ DYNAMIC phases from API
    const renderedPhaseIds = new Set();
    for (const phase of _qtTRulePhases) {
        const phaseSections = _qtTSections.filter(s => s.rule_phase === phase.id);
        ...
        html += `
            <div class="qt-section-divider" style="background:${phase.gradient};border-left-color:${phase.color};">
                ...
                ${_qtTIsGD ? `<div style="margin-left:auto;display:flex;gap:6px;">
                    <button onclick="event.stopPropagation();_qtTEditPhase('${phase.id}')" style="background:none;border:none;cursor:pointer;font-size:14px;" title="Sửa phần">✏️</button>
                    <button onclick="event.stopPropagation();_qtTDeletePhase('${phase.id}')" style="background:none;border:none;cursor:pointer;font-size:14px;" title="Xóa phần">🗑️</button>
                </div>` : ''}
            </div>
        `;
        ...
        for (const sec of phaseSections) {
            ...
            html += _qtTRenderSectionAccordion(sec);
        }
    }

    // Sections without a phase
    const unphased = _qtTSections.filter(s => !s.rule_phase || !renderedPhaseIds.has(s.rule_phase));
    ...
    if (firstHeader) _qtTToggleSection(firstHeader);
}

// Render a single section accordion item
function _qtTRenderSectionAccordion(sec) {
    // Check if this is a group leader
    const isGroup = !!sec.section_group;
    const groupKeys = isGroup ? _qtTGetGroupKeys(sec.section_group) : [sec.key];
    const rules = _qtTAllRules[sec.key];
    ...
    if (isGroup && sec.section_group_label) {
        ...
        const memberLabels = groupKeys.map(k => {
            const t = _qtTAllTypes.find(x => x.key === k);
            return t ? `${t.icon} ${t.label}` : k;
        }).join(', ');
        tooltip = `title=\"Gồm: ${memberLabels}\"`;
    } else {
        const tp = _qtTAllTypes.find(x => x.key === sec.key);
    ...
    let html = `
        <div class="qt-flow-section" id="${sectionId}">
            <div class="qt-flow-header" onclick="_qtTToggleSection(this)">
                <div class="qt-flow-title">
                    <span class="qt-loai-badge" style="background:#3b82f6;cursor:${_qtTIsGD ? 'pointer' : 'default'};" ${_qtTIsGD ? `onclick="event.stopPropagation();_qtTEditSectionOrder('${sec.key}',${sec.section_order})" title="Click để đổi STT"` : ''}>Loại ${sec.section_order}</span>
                    ...
                    ${isGroup ? `<span style="font-size:10px;background:#8b5cf6;color:white;padding:1px 6px;border-radius:8px;">🔗 ${groupKeys.length} nút</span>
                        <span class="qt-group-members" style="display:inline-flex;gap:3px;flex-wrap:wrap;margin-left:4px;">${groupKeys.map(k => { const mt = _qtTAllTypes.find(x => x.key === k); return mt ? `<span style="font-size:9px;padding:1px 5px;border-radius:6px;background:${mt.color}20;color:${mt.color};font-weight:700;border:1px solid ${mt.color}30;white-space:nowrap;">${mt.icon} ${mt.label}</span>` : ''; }).join('')}</span>` : ''}
                    <span class="qt-flow-count">${rules.length} nút</span>
                    ${_qtTIsGD ? `<span style="font-size:10px;padding:1px 6px;border-radius:8px;background:${sec.max_appointment_days > 0 ? '#f59e0b20' : '#64748b20'};color:${sec.max_appointment_days > 0 ? '#f59e0b' : '#94a3b8'};cursor:pointer;border:1px solid ${sec.max_appointment_days > 0 ? '#f59e0b40' : '#64748b30'};margin-left:4px;" onclick="event.stopPropagation();_qtTEditMaxDays('${sec.key}',${sec.max_appointment_days || 0})" title="Giới hạn ngày hẹn tối đa">📅 ${sec.max_appointment_days > 0 ? sec.max_appointment_days + ' ngày' : 'Không giới hạn'}</span>` : (sec.max_appointment_days > 0 ? `<span style="font-size:10px;padding:1px 6px;border-radius:8px;background:#f59e0b20;color:#f59e0b;">📅 ${sec.max_appointment_days} ngày</span>` : '')}
                </div>
                <div style="display:flex;align-items:center;gap:8px;">
                    ${_qtTIsGD ? `<button class="qt-flow-edit-btn" onclick="event.stopPropagation();_qtTShowEditRulesModal('${sec.key}')">✏️ Sửa</button><button class="qt-flow-edit-btn" style="background:#ef4444;padding:4px 8px;min-width:0;" onclick="event.stopPropagation();_qtTDeleteSection('${sec.key}')" title="Xóa loại">🗑️</button>` : ''}
    ...
    if (!isWaitingStatus) {
        ...
        for (const r of rulesList) {
            const rtp = _qtTAllTypes.find(x => x.key === r.to_type_key);
...
async function _qtTEditMaxDays(key, currentVal) {
...
function _qtTGetGroupKeys(groupId) {
    if (!groupId) return [];
    const keys = [];
    // Leader (section_order > 0)
    const leader = _qtTSections.find(s => s.section_group === groupId);
    if (leader) keys.push(leader.key);
    // Members (section_order = 0)
    for (const m of _qtTGroupMembers) {
...
// ========== PHASE MANAGEMENT ==========
function _qtTShowAddPhaseModal() {
    const presetsHTML = TTK_STAGE_PRESETS.map((p, i) => `
    ...
    const nextOrder = _qtTRulePhases.length > 0 ? Math.max(..._qtTRulePhases.map(p => p.sort_order)) + 1 : 1;
        ...
        <div class="qt-modal">
            ...
            <div style="margin-top:8px;">
                ...
                <div id="qtPhaseSectionsList" style="max-height:200px;overflow-y:auto;margin-top:4px;">
                    ${_qtTSections.filter(s => !s.rule_phase).map(s => `
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtTAddPhase()">📂 Thêm Phần</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtTAddPhase() {
    ...
    await apiCall('/api/consult-rule-phases', 'PUT', { phases: _qtTRulePhases });
...
function _qtTEditPhase(phaseId) {
    const phase = _qtTRulePhases.find(p => p.id === phaseId);
    if (!phase) return;

    // Sections in this phase + available sections (no phase or different phase)
    const allSectioned = _qtTSections;
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtTSavePhase('${phaseId}')">💾 Lưu</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtTSavePhase(phaseId) {
    const phase = _qtTRulePhases.find(p => p.id === phaseId);
    if (!phase) return;
    ...
    items.forEach(item => {
        ...
        const sec = _qtTSections.find(s => s.key === key);
...
async function _qtTDeletePhase(phaseId) {
    const phase = _qtTRulePhases.find(p => p.id === phaseId);
    if (!phase) return;
    const count = _qtTSections.filter(s => s.rule_phase === phaseId).length;
    ...
    const updates = _qtTSections.filter(s => s.rule_phase === phaseId).map(s => ({ key: s.key, rule_phase: null }));
    if (updates.length > 0) await apiCall('/api/consult-types/batch/rule-phase', 'PATCH', { updates });

    _qtTRulePhases = _qtTRulePhases.filter(p => p.id !== phaseId);
    await apiCall('/api/consult-rule-phases', 'PUT', { phases: _qtTRulePhases });
...
function _qtTShowAddSectionModal() {
    if (_qtTUnsectioned.length === 0) return showToast('✅ Tất cả nút đã có loại!', 'success');
    const nextOrder = _qtTSections.length > 0 ? Math.max(..._qtTSections.map(s => s.section_order)) + 1 : 1;

    let listHTML = _qtTUnsectioned.map(t => `
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtTAddSections()">📌 Thêm loại</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtTAddSections() {
    ...
    for (const key of selected) {
        ...
        await apiCall(`/api/consult-flow-rules/${key}`, 'PUT', {
            rules: [{ to_type_key: key, is_default: true, delay_days: 0, sort_order: 1 }], crm_menu: 'tu_tim_kiem'
        });
        // Set section order
        await apiCall(`/api/consult-types/${key}/section-order`, 'PATCH', { section_order: startOrder, crm_menu: 'tu_tim_kiem' });
        startOrder++;
    }

    document.querySelector('.qt-modal-overlay')?.remove();
    showToast(`✅ Đã thêm ${selected.length} loại!`, 'success');
    await apiCall('/api/consult-sections/reindex', 'POST', { crm_menu: 'tu_tim_kiem' });
    await _qtTLoadData();
    _qtTSwitchTab('rules');
}

// ========== EDIT SECTION ORDER (click badge) ==========
function _qtTEditSectionOrder(key, currentOrder) {
    const tp = _qtTAllTypes.find(x => x.key === key);
    const label = tp ? `${tp.icon} ${tp.label}` : key;
    const sec = _qtTSections.find(s => s.key === key);
    const currentPhase = sec ? sec.rule_phase : '';

    const phaseOptions = _qtTRulePhases.map(p =>
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtTSaveSectionEdit('${key}',${currentOrder})">💾 Lưu</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtTSaveSectionEdit(key, oldOrder) {
    ...
    if (newOrder !== oldOrder) {
        promises.push(apiCall(`/api/consult-types/${key}/section-order`, 'PATCH', { section_order: newOrder, crm_menu: 'tu_tim_kiem' }));
    }
    const sec = _qtTSections.find(s => s.key === key);
    if (!sec || sec.rule_phase !== (newPhase || null)) {
        promises.push(apiCall(`/api/consult-types/${key}/rule-phase`, 'PATCH', { rule_phase: newPhase || null, crm_menu: 'tu_tim_kiem' }));
    }
...
async function _qtTDeleteSection(key) {
    const sec = _qtTSections.find(s => s.key === key);
    const isGroup = sec && sec.section_group;
    const groupKeys = isGroup ? _qtTGetGroupKeys(sec.section_group) : [key];
    const tp = _qtTAllTypes.find(x => x.key === key);
    const displayLabel = (isGroup && sec.section_group_label) ? sec.section_group_label : (tp ? `${tp.icon} ${tp.label}` : key);
    const ruleCount = (_qtTAllRules[key] || []).length;
    ...
    try {
        console.log('[_qtTDeleteSection] Deleting key:', key, 'groupKeys:', groupKeys);
        for (const k of groupKeys) {
            console.log('[_qtTDeleteSection] Processing key:', k);
            const r1 = await apiCall(`/api/consult-flow-rules/${k}?crm_menu=tu_tim_kiem`, 'DELETE');
            console.log('[_qtTDeleteSection] DELETE flow-rules:', k, r1);
            const r2 = await apiCall(`/api/consult-types/${k}/section-order`, 'PATCH', { section_order: 0, crm_menu: 'tu_tim_kiem' });
            console.log('[_qtTDeleteSection] PATCH section-order:', k, r2);
            const r3 = await apiCall(`/api/consult-types/${k}/rule-phase`, 'PATCH', { rule_phase: null, crm_menu: 'tu_tim_kiem' });
            console.log('[_qtTDeleteSection] PATCH rule-phase:', k, r3);
            // Clear group fields
            const r4 = await apiCall(`/api/consult-types/${k}`, 'PATCH', { section_group: null, section_group_label: null, crm_menu: 'tu_tim_kiem' });
            console.log('[_qtTDeleteSection] PATCH group fields:', k, r4);
        }
        ...
        console.error('[_qtTDeleteSection] ERROR:', e);
        showToast('❌ Lỗi xóa loại: ' + (e.message || ''), 'error');
    }
}

function _qtTToggleSection(header) {
...
function _qtTRenderFlowchart() {
    // Build journey nodes dynamically from _qtTSections (sorted by section_order)
    ...
    function renderNodeRow(sections) {
        ...
        for (let i = 0; i < sections.length; i++) {
            ...
            const t = _qtTAllTypes.find(x => x.key === sec.key);
            const icon = t ? t.icon : '📋';
            ...
            const isGroup = !!sec.section_group;
            const groupKeys = isGroup ? _qtTGetGroupKeys(sec.section_group) : [];
            ...
            nodesHtml += `
                <div class="qt-fc-node" onclick="_qtTScrollToRule('${sec.key}')" title="${displayName}${isGroup ? ' (nhóm ' + multiCount + ' nút: ' + groupKeys.map(k => { const gt = _qtTAllTypes.find(x => x.key === k); return gt ? gt.label : k; }).join(', ') + ')' : ''}">
...
// ========== EDIT RULES MODAL ==========
function _qtTShowEditRulesModal(fromStatus) {
    const rules = _qtTAllRules[fromStatus] || [];
    const statusLabel = TTK_FLOW_STATUS_LABELS[fromStatus] || _qtTGetTypeLabel(fromStatus) || fromStatus;

    // ★ Show ALL active types as eligible targets
    const eligibleTypes = _qtTAllTypes.filter(t => {
    ...
    const unsectionedKeys = new Set(_qtTUnsectioned.map(u => u.key));
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtTSaveRules('${fromStatus}')">💾 Lưu quy tắc</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtTSaveRules(fromStatus) {
    ...
    const sec = _qtTSections.find(s => s.key === fromStatus);
    const groupKeys = (sec && sec.section_group) ? _qtTGetGroupKeys(sec.section_group) : [fromStatus];
    for (const key of groupKeys) {
        await apiCall(`/api/consult-flow-rules/${key}`, 'PUT', { rules, crm_menu: 'tu_tim_kiem' });
    }
...
function _qtTShowAddRuleGroupModal() {
    // Show unsectioned types (exclude system keys that already have dedicated workflows)
    const SYSTEM_KEYS = ['cap_cuu_sep', 'huy'];
    const sectionKeys = new Set(_qtTSections.map(s => s.key));
    const available = _qtTAllTypes.filter(t => !sectionKeys.has(t.key) && t.is_active && !t.section_group && !SYSTEM_KEYS.includes(t.key));
        ...
        <div class="qt-modal">
            ...
            <div style="margin-top:12px;">
                ...
                <select id="qtNewRulePhase" style="margin-top:4px;">
                    <option value="">(Chưa phân phần)</option>
                    ${_qtTRulePhases.map(p => `<option value="${p.id}">${p.icon} PHẦN ${p.sort_order}: ${p.title}</option>`).join('')}
                </select>
            </div>
            <div style="margin-top:8px;">
                <label style="font-size:13px;font-weight:600;color:#334155;">Bắt đầu từ Loại số:</label>
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtTAddRuleGroup()">➕ Thêm</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtTAddRuleGroup() {
    ...
    if (isGroup) {
        ...
        for (const key of selected) {
            await apiCall(`/api/consult-flow-rules/${key}`, 'PUT', {
                rules: [{ to_type_key: key, is_default: key === leaderKey, delay_days: 0, sort_order: selected.indexOf(key) + 1 }], crm_menu: 'tu_tim_kiem'
            });
            // Set group for all
        ...
        await apiCall(`/api/consult-types/${leaderKey}/section-order`, 'PATCH', { section_order: startOrder, crm_menu: 'tu_tim_kiem' });
        if (selectedPhase) await apiCall(`/api/consult-types/${leaderKey}/rule-phase`, 'PATCH', { rule_phase: selectedPhase, crm_menu: 'tu_tim_kiem' });
        ...
        await apiCall(`/api/consult-types/${key}/section-order`, 'PATCH', { section_order: startOrder, crm_menu: 'tu_tim_kiem' });
        if (selectedPhase) await apiCall(`/api/consult-types/${key}/rule-phase`, 'PATCH', { rule_phase: selectedPhase, crm_menu: 'tu_tim_kiem' });
    }
    ...
    setTimeout(() => _qtTShowEditRulesModal(selected[0]), 300);
}

// ========== HELPERS ==========
function _qtTGetTypeLabel(key) {
    const t = _qtTAllTypes.find(x => x.key === key);
    return t ? `${t.icon} ${t.label}` : key;
}

async function _qtTSaveSortOrder(orders) {
    try {
        await apiCall('/api/consult-types/batch/sort-order', 'PATCH', { orders });
