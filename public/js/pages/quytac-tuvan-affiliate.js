...
let _qtAAllRules = {};
let _qtAStages = [];
let _qtASections = [];      // dynamic sections with section_order
let _qtAUnsectioned = [];   // types without a section
let _qtAGroupMembers = [];  // buttons belonging to a group (section_order=0)
let _qtARulePhases = [];    // dynamic phase groups (PHẦN 1, 2, 3...)
let _qtAIsGD = false;
let _qtAActiveTab = localStorage.getItem('qt_active_tab') || 'buttons';
let _qtASortDebounce = null;

// ========== MAIN ENTRY ==========
async function renderQuyTacTuVanAffPage(container) {
    _qtAIsGD = currentUser && currentUser.role === 'giam_doc';

    container.innerHTML = `
        <div class="qt-page">
            <a class="qt-back" href="/chamsocaffiliate" onclick="event.preventDefault();navigate('cham-soc-affiliate')">← Quay lại Chăm Sóc Affiliate</a>
            <div class="qt-header">
                <h2 class="qt-header-title">⚙️ Quy Tắc Nút Tư Vấn</h2>
                <span class="qt-header-badge">
                    ${_qtAIsGD ? '🔓 Chế độ chỉnh sửa' : '👁️ Chế độ xem'}
                </span>
            </div>
            <div class="qt-tabs">
                <div class="qt-tab${_qtAActiveTab === 'buttons' ? ' active' : ''}" onclick="_qtASwitchTab('buttons')" id="qtTabButtons">📋 Danh Sách Nút (0)</div>
                <div class="qt-tab${_qtAActiveTab === 'rules' ? ' active' : ''}" onclick="_qtASwitchTab('rules')" id="qtTabRules">🔄 Quy Tắc Liên Kết</div>
            </div>
            <div class="qt-panel" id="qtPanel">${_qtARenderSkeleton()}</div>
        </div>
    `;
...
// ========== DATA LOADING ==========
async function _qtALoadData() {
    const [typesData, rulesData, stagesData, sectionsData, phasesData] = await Promise.all([
        apiCall('/api/consult-types?crm_menu=affiliate'),
        apiCall('/api/consult-flow-rules?crm_menu=affiliate'),
        apiCall('/api/consult-stages'),
        apiCall('/api/consult-sections?crm_menu=affiliate'),
        apiCall('/api/consult-rule-phases')
    ]);
    _qtAAllTypes = typesData.types || [];
    _qtAAllRules = rulesData.rules || {};
    _qtAStages = stagesData.stages || [];
    _qtASections = sectionsData.sections || [];
    _qtAUnsectioned = (sectionsData.unsectioned || []).filter(t => !['huy', 'cap_cuu_sep'].includes(t.key));
    ...
    _qtAStages.sort((a, b) => (a.sort_order || 0) - (b.sort_order || 0));

    document.getElementById('qtTabButtons').innerHTML = `📋 Danh Sách Nút (${_qtAAllTypes.length})`;
    _qtARenderActiveTab();
}

// ========== TAB SWITCHING ==========
function _qtASwitchTab(tab) {
    _qtAActiveTab = tab;
...
function _qtARenderActiveTab() {
    if (_qtAActiveTab === 'buttons') _qtARenderButtons();
    else _qtARenderRules();
}

// ========== TAB 1: BUTTONS (Dynamic Stages) ==========
function _qtARenderButtons() {
    ...
    if (_qtAIsGD) {
        html += `<div class="qt-action-bar" style="gap:8px;">
            <button class="qt-flow-edit-btn" style="background:linear-gradient(135deg,#10b981,#059669);" onclick="_qtAShowAddStageModal()">📁 Thêm giai đoạn</button>
            <button class="qt-flow-edit-btn" onclick="_qtAShowAddTypeModal()">➕ Thêm nút mới</button>
        </div>`;
    }

    // Render each stage from dynamic config
    for (const stage of _qtAStages) {
        const stageTypes = _qtAAllTypes.filter(t => t.stage === stage.id);
        html += `
            <div class="qt-stage" data-stage-id="${stage.id}">
                <div class="qt-stage-header" style="background:${stage.gradient};">
                    ...
                    ${_qtAIsGD ? `
                        <button onclick="event.stopPropagation();_qtAShowEditStageModal('${stage.id}')" style="background:none;border:none;cursor:pointer;font-size:14px;margin-left:4px;" title="Sửa giai đoạn">✏️</button>
                        <button onclick="event.stopPropagation();_qtADeleteStage('${stage.id}')" style="background:none;border:none;cursor:pointer;font-size:14px;" title="Xóa giai đoạn">🗑️</button>
                    ` : ''}
                </div>
                <div class="qt-btn-grid" data-stage="${stage.id}">
                    ${stageTypes.map(t => _qtARenderButtonCard(t)).join('')}
                </div>
            </div>
        `;
    }
    ...
    const unassigned = _qtAAllTypes.filter(t => !t.stage || !_qtAStages.find(s => s.id === t.stage));
        ...
        html += `
            <div class="qt-stage" data-stage-id="_other">
                ...
                <div class="qt-btn-grid" data-stage="_other">
                    ${unassigned.map(t => _qtARenderButtonCard(t)).join('')}
                </div>
            </div>
        `;
    }
    ...
    if (_qtAIsGD && typeof Sortable !== 'undefined') {
...
function _qtAOnButtonSortEnd() {
    clearTimeout(_qtASortDebounce);
    _qtASortDebounce = setTimeout(() => {
        const orders = [];
        let globalOrder = 0;
        document.querySelectorAll('.qt-btn-grid').forEach(grid => {
        ...
        _qtASaveSortOrder(orders);
    }, 500);
}

// ========== STAGE MANAGEMENT ==========
function _qtAShowAddStageModal() {
    const presetsHTML = AFF_STAGE_PRESETS.map((p, i) => `
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                ...
                <button class="qt-btn qt-btn-primary" onclick="_qtAAddStage()">📁 Thêm giai đoạn</button>
            </div>
        </div>
    `;
...
async function _qtAAddStage() {
    ...
    const preset = AFF_STAGE_PRESETS[presetIdx];
    ...
    const newStage = {
        ...
        sort_order: _qtAStages.length + 1
    };

    _qtAStages.push(newStage);
    await apiCall('/api/consult-stages', 'PUT', { stages: _qtAStages });
    document.querySelector('.qt-modal-overlay')?.remove();
    showToast('✅ Đã thêm giai đoạn!', 'success');
    _qtARenderButtons();
}

function _qtAShowEditStageModal(stageId) {
    const stage = _qtAStages.find(s => s.id === stageId);
    if (!stage) return;
    ...
    const currentPresetIdx = AFF_STAGE_PRESETS.findIndex(p => p.gradient === stage.gradient);
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtASaveStage('${stageId}')">💾 Lưu</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtASaveStage(stageId) {
    const stage = _qtAStages.find(s => s.id === stageId);
    if (!stage) return;
...
async function _qtADeleteStage(stageId) {
    const stage = _qtAStages.find(s => s.id === stageId);
    if (!stage) return;
    const count = _qtAAllTypes.filter(t => t.stage === stageId).length;
    if (!confirm(`Xóa giai đoạn "${stage.title}"?\n${count > 0 ? `⚠️ ${count} nút sẽ chuyển về "Chưa phân loại"` : ''}`)) return;

    // Remove stage from config
    _qtAStages = _qtAStages.filter(s => s.id !== stageId);
    await apiCall('/api/consult-stages', 'PUT', { stages: _qtAStages });

    // Clear stage from buttons
    const orders = _qtAAllTypes.filter(t => t.stage === stageId).map((t, i) => ({ key: t.key, sort_order: 999 + i, stage: null }));
    if (orders.length > 0) {
...
// ========== EDIT TYPE MODAL ==========
function _qtAShowEditTypeModal(key) {
    const t = _qtAAllTypes.find(x => x.key === key);
    if (!t) return;

    const stageOptions = _qtAStages.map(s => `<option value="${s.id}" ${t.stage === s.id ? 'selected' : ''}>${s.icon} ${s.title}</option>`).join('');
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtASaveType('${key}')">💾 Lưu</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtASaveType(key) {
    const data = {
...
function _qtAShowAddTypeModal() {
    const stageOptions = _qtAStages.map(s => `<option value="${s.id}">${s.icon} ${s.title}</option>`).join('');
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtAAddType()">➕ Thêm</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtAAddType() {
    ...
    await apiCall('/api/consult-types', 'POST', {...data, crm_menu: 'affiliate'});
    document.querySelector('.qt-modal-overlay')?.remove();
...
function _qtARenderRules() {
    const panel = document.getElementById('qtPanel');
    let html = _qtARenderFlowchart();
    ...
    if (_qtAIsGD) {
        const unsecCount = _qtAUnsectioned.length;
        html += `<div class="qt-action-bar" style="gap:8px;">
            <button class="qt-flow-edit-btn" style="background:linear-gradient(135deg,#10b981,#059669);" onclick="_qtAShowAddPhaseModal()">📂 Thêm Phần</button>
            <button class="qt-flow-edit-btn" onclick="_qtAShowAddRuleGroupModal()">➕ Thêm nhóm quy tắc mới ${unsecCount > 0 ? '<span style="background:#ef4444;color:white;padding:1px 7px;border-radius:10px;font-size:10px;margin-left:4px;">' + unsecCount + '</span>' : ''}</button>
        </div>`;
    }
    ...
    for (const phase of _qtARulePhases) {
        const phaseSections = _qtASections.filter(s => s.rule_phase === phase.id);
        ...
        html += `
            <div class="qt-section-divider" style="background:${phase.gradient};border-left-color:${phase.color};">
                ...
                ${_qtAIsGD ? `<div style="margin-left:auto;display:flex;gap:6px;">
                    <button onclick="event.stopPropagation();_qtAEditPhase('${phase.id}')" style="background:none;border:none;cursor:pointer;font-size:14px;" title="Sửa phần">✏️</button>
                    <button onclick="event.stopPropagation();_qtADeletePhase('${phase.id}')" style="background:none;border:none;cursor:pointer;font-size:14px;" title="Xóa phần">🗑️</button>
                </div>` : ''}
            </div>
        `;
        ...
        for (const sec of phaseSections) {
            ...
            html += _qtARenderSectionAccordion(sec);
        }
    }

    // Sections without a phase
    const unphased = _qtASections.filter(s => !s.rule_phase || !renderedPhaseIds.has(s.rule_phase));
    ...
    if (firstHeader) _qtAToggleSection(firstHeader);
}

// Render a single section accordion item
function _qtARenderSectionAccordion(sec) {
    // Check if this is a group leader
    const isGroup = !!sec.section_group;
    const groupKeys = isGroup ? _qtAGetGroupKeys(sec.section_group) : [sec.key];
    const rules = _qtAAllRules[sec.key];
    ...
    if (isGroup && sec.section_group_label) {
        ...
        const memberLabels = groupKeys.map(k => {
            const t = _qtAAllTypes.find(x => x.key === k);
    ...
    let html = `
        <div class="qt-flow-section" id="${sectionId}">
            <div class="qt-flow-header" onclick="_qtAToggleSection(this)">
                <div class="qt-flow-title">
                    <span class="qt-loai-badge" style="background:#3b82f6;cursor:${_qtAIsGD ? 'pointer' : 'default'};" ${_qtAIsGD ? `onclick="event.stopPropagation();_qtAEditSectionOrder('${sec.key}',${sec.section_order})" title="Click để đổi STT"` : ''}>Loại ${sec.section_order}</span>
                    ...
                    ${isGroup ? `<span style="font-size:10px;background:#8b5cf6;color:white;padding:1px 6px;border-radius:8px;">🔗 ${groupKeys.length} nút</span>
                        <span class="qt-group-members" style="display:inline-flex;gap:3px;flex-wrap:wrap;margin-left:4px;">${groupKeys.map(k => { const mt = _qtAAllTypes.find(x => x.key === k); return mt ? `<span style="font-size:9px;padding:1px 5px;border-radius:6px;background:${mt.color}20;color:${mt.color};font-weight:700;border:1px solid ${mt.color}30;white-space:nowrap;">${mt.icon} ${mt.label}</span>` : ''; }).join('')}</span>` : ''}
    ...
    if (!isWaitingStatus) {
        ...
        for (const r of rulesList) {
            const rtp = _qtAAllTypes.find(x => x.key === r.to_type_key);
...
async function _qtAEditMaxDays(key, currentVal) {
...
function _qtAGetGroupKeys(groupId) {
    if (!groupId) return [];
    const keys = [];
    // Leader (section_order > 0)
    const leader = _qtASections.find(s => s.section_group === groupId);
    if (leader) keys.push(leader.key);
    // Members (section_order = 0)
    for (const m of _qtAGroupMembers) {
...
// ========== PHASE MANAGEMENT ==========
function _qtAShowAddPhaseModal() {
    const presetsHTML = AFF_STAGE_PRESETS.map((p, i) => `
    ...
    const nextOrder = _qtARulePhases.length > 0 ? Math.max(..._qtARulePhases.map(p => p.sort_order)) + 1 : 1;
        ...
        <div class="qt-modal">
            ...
            <div style="margin-top:8px;">
                ...
                <div id="qtPhaseSectionsList" style="max-height:200px;overflow-y:auto;margin-top:4px;">
                    ${_qtASections.filter(s => !s.rule_phase).map(s => `
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtAAddPhase()">📂 Thêm Phần</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtAAddPhase() {
    ...
    await apiCall('/api/consult-rule-phases', 'PUT', { phases: _qtARulePhases });
...
function _qtAEditPhase(phaseId) {
    const phase = _qtARulePhases.find(p => p.id === phaseId);
    if (!phase) return;

    // Sections in this phase + available sections (no phase or different phase)
    const allSectioned = _qtASections;
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtASavePhase('${phaseId}')">💾 Lưu</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtASavePhase(phaseId) {
    const phase = _qtARulePhases.find(p => p.id === phaseId);
    if (!phase) return;
...
async function _qtADeletePhase(phaseId) {
    const phase = _qtARulePhases.find(p => p.id === phaseId);
    if (!phase) return;
    const count = _qtASections.filter(s => s.rule_phase === phaseId).length;
    ...
    const updates = _qtASections.filter(s => s.rule_phase === phaseId).map(s => ({ key: s.key, rule_phase: null }));
    if (updates.length > 0) await apiCall('/api/consult-types/batch/rule-phase', 'PATCH', { updates });

    _qtARulePhases = _qtARulePhases.filter(p => p.id !== phaseId);
    await apiCall('/api/consult-rule-phases', 'PUT', { phases: _qtARulePhases });
...
function _qtAShowAddSectionModal() {
    if (_qtAUnsectioned.length === 0) return showToast('✅ Tất cả nút đã có loại!', 'success');
    const nextOrder = _qtASections.length > 0 ? Math.max(..._qtASections.map(s => s.section_order)) + 1 : 1;

    let listHTML = _qtAUnsectioned.map(t => `
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtAAddSections()">📌 Thêm loại</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtAAddSections() {
    ...
    for (const key of selected) {
        ...
        await apiCall(`/api/consult-flow-rules/${key}`, 'PUT', {
            rules: [{ to_type_key: key, is_default: true, delay_days: 0, sort_order: 1 }], crm_menu: 'affiliate'
        });
        // Set section order
        await apiCall(`/api/consult-types/${key}/section-order`, 'PATCH', { section_order: startOrder, crm_menu: 'affiliate' });
        startOrder++;
    }
...
function _qtAEditSectionOrder(key, currentOrder) {
    const tp = _qtAAllTypes.find(x => x.key === key);
    const label = tp ? `${tp.icon} ${tp.label}` : key;
    const sec = _qtASections.find(s => s.key === key);
    const currentPhase = sec ? sec.rule_phase : '';

    const phaseOptions = _qtARulePhases.map(p =>
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtASaveSectionEdit('${key}',${currentOrder})">💾 Lưu</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtASaveSectionEdit(key, oldOrder) {
    ...
    if (newOrder !== oldOrder) {
        promises.push(apiCall(`/api/consult-types/${key}/section-order`, 'PATCH', { section_order: newOrder, crm_menu: 'affiliate' }));
    }
    const sec = _qtASections.find(s => s.key === key);
    if (!sec || sec.rule_phase !== (newPhase || null)) {
        promises.push(apiCall(`/api/consult-types/${key}/rule-phase`, 'PATCH', { rule_phase: newPhase || null, crm_menu: 'affiliate' }));
    }
...
async function _qtADeleteSection(key) {
    const sec = _qtASections.find(s => s.key === key);
    const isGroup = sec && sec.section_group;
    const groupKeys = isGroup ? _qtAGetGroupKeys(sec.section_group) : [key];
    const tp = _qtAAllTypes.find(x => x.key === key);
    const displayLabel = (isGroup && sec.section_group_label) ? sec.section_group_label : (tp ? `${tp.icon} ${tp.label}` : key);
    const ruleCount = (_qtAAllRules[key] || []).length;
    ...
    try {
        console.log('[_qtADeleteSection] Deleting key:', key, 'groupKeys:', groupKeys);
        for (const k of groupKeys) {
            console.log('[_qtADeleteSection] Processing key:', k);
            const r1 = await apiCall(`/api/consult-flow-rules/${k}?crm_menu=affiliate`, 'DELETE');
            console.log('[_qtADeleteSection] DELETE flow-rules:', k, r1);
            const r2 = await apiCall(`/api/consult-types/${k}/section-order`, 'PATCH', { section_order: 0, crm_menu: 'affiliate' });
            console.log('[_qtADeleteSection] PATCH section-order:', k, r2);
            const r3 = await apiCall(`/api/consult-types/${k}/rule-phase`, 'PATCH', { rule_phase: null, crm_menu: 'affiliate' });
            console.log('[_qtADeleteSection] PATCH rule-phase:', k, r3);
            // Clear group fields
            const r4 = await apiCall(`/api/consult-types/${k}`, 'PATCH', { section_group: null, section_group_label: null, crm_menu: 'affiliate' });
            console.log('[_qtADeleteSection] PATCH group fields:', k, r4);
        }
        ...
        console.error('[_qtADeleteSection] ERROR:', e);
        showToast('❌ Lỗi xóa loại: ' + (e.message || ''), 'error');
    }
}

function _qtAToggleSection(header) {
...
function _qtARenderFlowchart() {
    // Build journey nodes dynamically from _qtASections (sorted by section_order)
    ...
    function renderNodeRow(sections) {
        ...
        for (let i = 0; i < sections.length; i++) {
            ...
            const t = _qtAAllTypes.find(x => x.key === sec.key);
            const icon = t ? t.icon : '📋';
            ...
            const isGroup = !!sec.section_group;
            const groupKeys = isGroup ? _qtAGetGroupKeys(sec.section_group) : [];
            ...
            nodesHtml += `
                <div class="qt-fc-node" onclick="_qtAScrollToRule('${sec.key}')" title="${displayName}${isGroup ? ' (nhóm ' + multiCount + ' nút: ' + groupKeys.map(k => { const gt = _qtAAllTypes.find(x => x.key === k); return gt ? gt.label : k; }).join(', ') + ')' : ''}">
...
// ========== EDIT RULES MODAL ==========
function _qtAShowEditRulesModal(fromStatus) {
    const rules = _qtAAllRules[fromStatus] || [];
    const statusLabel = AFF_FLOW_STATUS_LABELS[fromStatus] || _qtAGetTypeLabel(fromStatus) || fromStatus;

    // ★ Show ALL active types as eligible targets
    const eligibleTypes = _qtAAllTypes.filter(t => {
    ...
    const unsectionedKeys = new Set(_qtAUnsectioned.map(u => u.key));
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtASaveRules('${fromStatus}')">💾 Lưu quy tắc</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtASaveRules(fromStatus) {
    ...
    const sec = _qtASections.find(s => s.key === fromStatus);
    const groupKeys = (sec && sec.section_group) ? _qtAGetGroupKeys(sec.section_group) : [fromStatus];
    for (const key of groupKeys) {
        await apiCall(`/api/consult-flow-rules/${key}`, 'PUT', { rules, crm_menu: 'affiliate' });
    }
...
function _qtAShowAddRuleGroupModal() {
    ...
    const SYSTEM_KEYS = ['huy', 'cap_cuu_sep'];
        ...
        <div class="qt-modal">
            ...
            <div style="margin-top:12px;">
                ...
                <select id="qtNewRulePhase" style="margin-top:4px;">
                    <option value="">(Chưa phân phần)</option>
                    ${_qtARulePhases.map(p => `<option value="${p.id}">${p.icon} PHẦN ${p.sort_order}: ${p.title}</option>`).join('')}
                </select>
            </div>
            <div style="margin-top:8px;">
                <label style="font-size:13px;font-weight:600;color:#334155;">Bắt đầu từ Loại số:</label>
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtAAddRuleGroup()">➕ Thêm</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtAAddRuleGroup() {
    ...
    if (isGroup) {
        ...
        for (const key of selected) {
            await apiCall(`/api/consult-flow-rules/${key}`, 'PUT', {
                rules: [{ to_type_key: key, is_default: key === leaderKey, delay_days: 0, sort_order: selected.indexOf(key) + 1 }], crm_menu: 'affiliate'
            });
            // Set group for all
        ...
        await apiCall(`/api/consult-types/${leaderKey}/section-order`, 'PATCH', { section_order: startOrder, crm_menu: 'affiliate' });
        if (selectedPhase) await apiCall(`/api/consult-types/${leaderKey}/rule-phase`, 'PATCH', { rule_phase: selectedPhase, crm_menu: 'affiliate' });
        ...
        await apiCall(`/api/consult-types/${key}/section-order`, 'PATCH', { section_order: startOrder, crm_menu: 'affiliate' });
        if (selectedPhase) await apiCall(`/api/consult-types/${key}/rule-phase`, 'PATCH', { rule_phase: selectedPhase, crm_menu: 'affiliate' });
    }
    ...
    setTimeout(() => _qtAShowEditRulesModal(selected[0]), 300);
}

// ========== MERGE INTO EXISTING GROUP ==========
function _qtAShowMergeModal(sectionKey) {
    const sec = _qtASections.find(s => s.key === sectionKey);
    if (!sec) return;
    const isGroup = !!sec.section_group;
    const groupKeys = isGroup ? _qtAGetGroupKeys(sec.section_group) : [sec.key];
    const tp = _qtAAllTypes.find(x => x.key === sec.key);
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div style="display:flex;flex-wrap:wrap;gap:4px;margin-bottom:12px;">
                ${groupKeys.map(k => { const mt = _qtAAllTypes.find(x => x.key === k); return mt ? `<span style="font-size:10px;padding:2px 8px;border-radius:6px;background:${mt.color}20;color:${mt.color};font-weight:700;border:1px solid ${mt.color}30;">${mt.icon} ${mt.label}</span>` : ''; }).join('')}
            </div>
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtAMergeIntoGroup('${sectionKey}')">➕ Gom vào nhóm</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtAMergeIntoGroup(sectionKey) {
    const sec = _qtASections.find(s => s.key === sectionKey);
    if (!sec) return;
    ...
    const leaderRules = _qtAAllRules[leaderKey] || [];
...
// ========== UNGROUP ==========
function _qtAShowUngroupModal(sectionKey) {
    const sec = _qtASections.find(s => s.key === sectionKey);
    if (!sec || !sec.section_group) return;
    const groupKeys = _qtAGetGroupKeys(sec.section_group);
    ...
    let listHTML = groupKeys.map(k => {
        const mt = _qtAAllTypes.find(x => x.key === k); const isLeader = k === sec.key;
    ...
    overlay.innerHTML = `<div class="qt-modal"><h3>🔓 Hủy gom nhóm: ${currentLabel}</h3>
        ...
        <div class="qt-actions" style="gap:8px;"><button class="qt-btn" style="background:linear-gradient(135deg,#ef4444,#dc2626);color:white;box-shadow:0 2px 8px rgba(239,68,68,.3);" onclick="_qtADisbandGroup('${sectionKey}')">💥 Giải tán toàn bộ</button><div style="flex:1;"></div><button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button><button class="qt-btn" style="background:linear-gradient(135deg,#f59e0b,#d97706);color:white;box-shadow:0 2px 8px rgba(245,158,11,.3);" onclick="_qtAUngroupSelected('${sectionKey}')">🔓 Bỏ ra khỏi nhóm</button></div></div>`;
    document.body.appendChild(overlay);
...
async function _qtAUngroupSelected(sectionKey) {
    const sec = _qtASections.find(s => s.key === sectionKey); if (!sec || !sec.section_group) return;
    const groupKeys = _qtAGetGroupKeys(sec.section_group);
    ...
    document.querySelector('.qt-modal-overlay')?.remove(); showToast(`✅ Đã bỏ ${toRemove.length} nút ra khỏi nhóm!`, 'success'); await _qtALoadData(); _qtASwitchTab('rules');
}
async function _qtADisbandGroup(sectionKey) {
    const sec = _qtASections.find(s => s.key === sectionKey); if (!sec || !sec.section_group) return;
    const groupKeys = _qtAGetGroupKeys(sec.section_group); const label = sec.section_group_label || sec.key;
    ...
    document.querySelector('.qt-modal-overlay')?.remove(); showToast(`✅ Đã giải tán nhóm "${label}"!`, 'success'); await _qtALoadData(); _qtASwitchTab('rules');
}

// ========== HELPERS ==========
function _qtAGetTypeLabel(key) {
    const t = _qtAAllTypes.find(x => x.key === key);
    return t ? `${t.icon} ${t.label}` : key;
}

async function _qtASaveSortOrder(orders) {
    try {
        await apiCall('/api/consult-types/batch/sort-order', 'PATCH', { orders });
