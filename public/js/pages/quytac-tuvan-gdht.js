...
let _qtGAllRules = {};
let _qtGStages = [];
let _qtGSections = [];      // dynamic sections with section_order
let _qtGUnsectioned = [];   // types without a section
let _qtGGroupMembers = [];  // buttons belonging to a group (section_order=0)
let _qtGRulePhases = [];    // dynamic phase groups (PHẦN 1, 2, 3...)
let _qtGIsGD = false;
let _qtGActiveTab = localStorage.getItem('qt_active_tab') || 'buttons';
let _qtGSortDebounce = null;

// ========== MAIN ENTRY ==========
async function renderQuyTacTuVanGdhtPage(container) {
    _qtGIsGD = currentUser && currentUser.role === 'giam_doc';

    container.innerHTML = `
        <div class="qt-page">
            <a class="qt-back" href="/crm-goi-hop-tac" onclick="event.preventDefault();navigate('crm-goi-hop-tac')">← Quay lại CRM Gọi Điện Hợp Tác</a>
            <div class="qt-header">
                <h2 class="qt-header-title">⚙️ Quy Tắc Nút Tư Vấn</h2>
                <span class="qt-header-badge">
                    ${_qtGIsGD ? '🔓 Chế độ chỉnh sửa' : '👁️ Chế độ xem'}
                </span>
            </div>
            <div class="qt-tabs">
                <div class="qt-tab${_qtGActiveTab === 'buttons' ? ' active' : ''}" onclick="_qtGSwitchTab('buttons')" id="qtTabButtons">📋 Danh Sách Nút (0)</div>
                <div class="qt-tab${_qtGActiveTab === 'rules' ? ' active' : ''}" onclick="_qtGSwitchTab('rules')" id="qtTabRules">🔄 Quy Tắc Liên Kết</div>
            </div>
            <div class="qt-panel" id="qtPanel">${_qtGRenderSkeleton()}</div>
        </div>
    `;
...
// ========== DATA LOADING ==========
async function _qtGLoadData() {
    const [typesData, rulesData, stagesData, sectionsData, phasesData] = await Promise.all([
        apiCall('/api/consult-types?crm_menu=goi_hop_tac'),
        apiCall('/api/consult-flow-rules?crm_menu=goi_hop_tac'),
        apiCall('/api/consult-stages'),
        apiCall('/api/consult-sections?crm_menu=goi_hop_tac'),
        apiCall('/api/consult-rule-phases')
    ]);
    _qtGAllTypes = typesData.types || [];
    _qtGAllRules = rulesData.rules || {};
    _qtGStages = stagesData.stages || [];
    _qtGSections = sectionsData.sections || [];
    _qtGUnsectioned = (sectionsData.unsectioned || []).filter(t => !['cap_cuu_sep', 'huy'].includes(t.key));
    _qtGGroupMembers = sectionsData.groupMembers || [];
    _qtGRulePhases = phasesData.phases || [];
    _qtGRulePhases.sort((a, b) => (a.sort_order || 0) - (b.sort_order || 0));
    // Sort stages by sort_order
    _qtGStages.sort((a, b) => (a.sort_order || 0) - (b.sort_order || 0));

    document.getElementById('qtTabButtons').innerHTML = `📋 Danh Sách Nút (${_qtGAllTypes.length})`;
    _qtGRenderActiveTab();
}

// ========== TAB SWITCHING ==========
function _qtGSwitchTab(tab) {
    _qtGActiveTab = tab;
...
function _qtGRenderActiveTab() {
    if (_qtGActiveTab === 'buttons') _qtGRenderButtons();
    else _qtGRenderRules();
}

// ========== TAB 1: BUTTONS (Dynamic Stages) ==========
function _qtGRenderButtons() {
    ...
    if (_qtGIsGD) {
        html += `<div class="qt-action-bar" style="gap:8px;">
            <button class="qt-flow-edit-btn" style="background:linear-gradient(135deg,#10b981,#059669);" onclick="_qtGShowAddStageModal()">📁 Thêm giai đoạn</button>
            <button class="qt-flow-edit-btn" onclick="_qtGShowAddTypeModal()">➕ Thêm nút mới</button>
        </div>`;
    }

    // Render each stage from dynamic config
    for (const stage of _qtGStages) {
        const stageTypes = _qtGAllTypes.filter(t => t.stage === stage.id);
        html += `
            <div class="qt-stage" data-stage-id="${stage.id}">
                <div class="qt-stage-header" style="background:${stage.gradient};">
                    ...
                    ${_qtGIsGD ? `
                        <button onclick="event.stopPropagation();_qtGShowEditStageModal('${stage.id}')" style="background:none;border:none;cursor:pointer;font-size:14px;margin-left:4px;" title="Sửa giai đoạn">✏️</button>
                        <button onclick="event.stopPropagation();_qtGDeleteStage('${stage.id}')" style="background:none;border:none;cursor:pointer;font-size:14px;" title="Xóa giai đoạn">🗑️</button>
                    ` : ''}
                </div>
                <div class="qt-btn-grid" data-stage="${stage.id}">
                    ${stageTypes.map(t => _qtGRenderButtonCard(t)).join('')}
                </div>
            </div>
        `;
    }
    ...
    const unassigned = _qtGAllTypes.filter(t => !t.stage || !_qtGStages.find(s => s.id === t.stage));
        ...
        html += `
            <div class="qt-stage" data-stage-id="_other">
                ...
                <div class="qt-btn-grid" data-stage="_other">
                    ${unassigned.map(t => _qtGRenderButtonCard(t)).join('')}
                </div>
            </div>
        `;
    }
    ...
    if (_qtGIsGD) {
        panel.addEventListener('click', function(e) {
            ...
            if (editBtn) {
                ...
                if (card && card.dataset.key) _qtGShowEditTypeModal(card.dataset.key);
                return;
            }
            ...
            if (delBtn) {
                ...
                if (card && card.dataset.key) _qtGDeleteType(card.dataset.key);
                return;
            }
        }, true);
    }
    ...
    if (_qtGIsGD && typeof Sortable !== 'undefined') {
        document.querySelectorAll('.qt-btn-grid').forEach(grid => {
            new Sortable(grid, {
                ...
                group: 'buttons',
                onEnd: _qtGOnButtonSortEnd
            });
        });
    }
}

function _qtGRenderButtonCard(t) {
    return `
        <div class="qt-btn-card ${t.is_active ? '' : 'inactive'}" data-key="${t.key}" style="--card-accent:${t.color}">
            ${_qtGIsGD ? `<span class="qt-drag-hint">⠿</span>` : ''}
            ${_qtGIsGD ? `<button class="qt-edit-btn" type="button">✏️</button>` : ''}
            ${_qtGIsGD ? `<button class="qt-del-btn" type="button">🗑️</button>` : ''}
...
function _qtGOnButtonSortEnd() {
    clearTimeout(_qtGSortDebounce);
    _qtGSortDebounce = setTimeout(() => {
        const orders = [];
        let globalOrder = 0;
        document.querySelectorAll('.qt-btn-grid').forEach(grid => {
        ...
        _qtGSaveSortOrder(orders);
    }, 500);
}

// ========== STAGE MANAGEMENT ==========
function _qtGShowAddStageModal() {
    const presetsHTML = GDHT_STAGE_PRESETS.map((p, i) => `
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                ...
                <button class="qt-btn qt-btn-primary" onclick="_qtGAddStage()">📁 Thêm giai đoạn</button>
            </div>
        </div>
    `;
...
async function _qtGAddStage() {
    ...
    const preset = GDHT_STAGE_PRESETS[presetIdx];
    ...
    const newStage = {
        ...
        sort_order: _qtGStages.length + 1
    };

    _qtGStages.push(newStage);
    await apiCall('/api/consult-stages', 'PUT', { stages: _qtGStages });
    document.querySelector('.qt-modal-overlay')?.remove();
    showToast('✅ Đã thêm giai đoạn!', 'success');
    _qtGRenderButtons();
}

function _qtGShowEditStageModal(stageId) {
    const stage = _qtGStages.find(s => s.id === stageId);
    if (!stage) return;
    ...
    const currentPresetIdx = GDHT_STAGE_PRESETS.findIndex(p => p.gradient === stage.gradient);
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtGSaveStage('${stageId}')">💾 Lưu</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtGSaveStage(stageId) {
    const stage = _qtGStages.find(s => s.id === stageId);
    if (!stage) return;
...
async function _qtGDeleteStage(stageId) {
    const stage = _qtGStages.find(s => s.id === stageId);
    if (!stage) return;
    const count = _qtGAllTypes.filter(t => t.stage === stageId).length;
    if (!confirm(`Xóa giai đoạn "${stage.title}"?\n${count > 0 ? `⚠️ ${count} nút sẽ chuyển về "Chưa phân loại"` : ''}`)) return;

    // Remove stage from config
    _qtGStages = _qtGStages.filter(s => s.id !== stageId);
    await apiCall('/api/consult-stages', 'PUT', { stages: _qtGStages });

    // Clear stage from buttons
    const orders = _qtGAllTypes.filter(t => t.stage === stageId).map((t, i) => ({ key: t.key, sort_order: 999 + i, stage: null }));
    if (orders.length > 0) {
...
// ========== DELETE TYPE ==========
async function _qtGDeleteType(key) {
    const t = _qtGAllTypes.find(x => x.key === key);
    if (!t) return;
...
// ========== EDIT TYPE MODAL ==========
function _qtGShowEditTypeModal(key) {
    const t = _qtGAllTypes.find(x => x.key === key);
    if (!t) return;

    const stageOptions = _qtGStages.map(s => `<option value="${s.id}" ${t.stage === s.id ? 'selected' : ''}>${s.icon} ${s.title}</option>`).join('');
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtGSaveType('${key}')">💾 Lưu</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtGSaveType(key) {
    const data = {
...
function _qtGShowAddTypeModal() {
    const stageOptions = _qtGStages.map(s => `<option value="${s.id}">${s.icon} ${s.title}</option>`).join('');
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtGAddType()">➕ Thêm</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtGAddType() {
    ...
    await apiCall('/api/consult-types', 'POST', {...data, crm_menu: 'goi_hop_tac'});
    document.querySelector('.qt-modal-overlay')?.remove();
...
function _qtGRenderRules() {
    const panel = document.getElementById('qtPanel');
    let html = _qtGRenderFlowchart();
    ...
    if (_qtGIsGD) {
        const unsecCount = _qtGUnsectioned.length;
        html += `<div class="qt-action-bar" style="gap:8px;">
            <button class="qt-flow-edit-btn" style="background:linear-gradient(135deg,#10b981,#059669);" onclick="_qtGShowAddPhaseModal()">📂 Thêm Phần</button>
            <button class="qt-flow-edit-btn" onclick="_qtGShowAddRuleGroupModal()">➕ Thêm nhóm quy tắc mới ${unsecCount > 0 ? '<span style="background:#ef4444;color:white;padding:1px 7px;border-radius:10px;font-size:10px;margin-left:4px;">' + unsecCount + '</span>' : ''}</button>
        </div>`;
    }

    // ★ DYNAMIC phases from API
    const renderedPhaseIds = new Set();
    for (const phase of _qtGRulePhases) {
        const phaseSections = _qtGSections.filter(s => s.rule_phase === phase.id);
        ...
        html += `
            <div class="qt-section-divider" style="background:${phase.gradient};border-left-color:${phase.color};">
                ...
                ${_qtGIsGD ? `<div style="margin-left:auto;display:flex;gap:6px;">
                    <button onclick="event.stopPropagation();_qtGEditPhase('${phase.id}')" style="background:none;border:none;cursor:pointer;font-size:14px;" title="Sửa phần">✏️</button>
                    <button onclick="event.stopPropagation();_qtGDeletePhase('${phase.id}')" style="background:none;border:none;cursor:pointer;font-size:14px;" title="Xóa phần">🗑️</button>
                </div>` : ''}
            </div>
        `;
        ...
        for (const sec of phaseSections) {
            ...
            html += _qtGRenderSectionAccordion(sec);
        }
    }

    // Sections without a phase
    const unphased = _qtGSections.filter(s => !s.rule_phase || !renderedPhaseIds.has(s.rule_phase));
    ...
    if (firstHeader) _qtGToggleSection(firstHeader);
}

// Render a single section accordion item
function _qtGRenderSectionAccordion(sec) {
    // Check if this is a group leader
    const isGroup = !!sec.section_group;
    const groupKeys = isGroup ? _qtGGetGroupKeys(sec.section_group) : [sec.key];
    const rules = _qtGAllRules[sec.key];
    ...
    if (isGroup && sec.section_group_label) {
        ...
        const memberLabels = groupKeys.map(k => {
            const t = _qtGAllTypes.find(x => x.key === k);
            return t ? `${t.icon} ${t.label}` : k;
        }).join(', ');
        tooltip = `title=\"Gồm: ${memberLabels}\"`;
    } else {
        const tp = _qtGAllTypes.find(x => x.key === sec.key);
    ...
    let html = `
        <div class="qt-flow-section" id="${sectionId}">
            <div class="qt-flow-header" onclick="_qtGToggleSection(this)">
                <div class="qt-flow-title">
                    <span class="qt-loai-badge" style="background:#3b82f6;cursor:${_qtGIsGD ? 'pointer' : 'default'};" ${_qtGIsGD ? `onclick="event.stopPropagation();_qtGEditSectionOrder('${sec.key}',${sec.section_order})" title="Click để đổi STT"` : ''}>Loại ${sec.section_order}</span>
                    ...
                    ${isGroup ? `<span style="font-size:10px;background:#8b5cf6;color:white;padding:1px 6px;border-radius:8px;">🔗 ${groupKeys.length} nút</span>
                        <span class="qt-group-members" style="display:inline-flex;gap:3px;flex-wrap:wrap;margin-left:4px;">${groupKeys.map(k => { const mt = _qtGAllTypes.find(x => x.key === k); return mt ? `<span style="font-size:9px;padding:1px 5px;border-radius:6px;background:${mt.color}20;color:${mt.color};font-weight:700;border:1px solid ${mt.color}30;white-space:nowrap;">${mt.icon} ${mt.label}</span>` : ''; }).join('')}</span>` : ''}
                    <span class="qt-flow-count">${rules.length} nút</span>
                    ${_qtGIsGD ? `<span style="font-size:10px;padding:1px 6px;border-radius:8px;background:${sec.max_appointment_days > 0 ? '#f59e0b20' : '#64748b20'};color:${sec.max_appointment_days > 0 ? '#f59e0b' : '#94a3b8'};cursor:pointer;border:1px solid ${sec.max_appointment_days > 0 ? '#f59e0b40' : '#64748b30'};margin-left:4px;" onclick="event.stopPropagation();_qtGEditMaxDays('${sec.key}',${sec.max_appointment_days || 0})" title="Giới hạn ngày hẹn tối đa">📅 ${sec.max_appointment_days > 0 ? sec.max_appointment_days + ' ngày' : 'Không giới hạn'}</span>` : (sec.max_appointment_days > 0 ? `<span style="font-size:10px;padding:1px 6px;border-radius:8px;background:#f59e0b20;color:#f59e0b;">📅 ${sec.max_appointment_days} ngày</span>` : '')}
                </div>
                <div style="display:flex;align-items:center;gap:8px;">
                    ${_qtGIsGD ? `<button class="qt-flow-edit-btn" onclick="event.stopPropagation();_qtGShowEditRulesModal('${sec.key}')">✏️ Sửa</button><button class="qt-flow-edit-btn" style="background:#ef4444;padding:4px 8px;min-width:0;" onclick="event.stopPropagation();_qtGDeleteSection('${sec.key}')" title="Xóa loại">🗑️</button>` : ''}
    ...
    if (!isWaitingStatus) {
        ...
        for (const r of rulesList) {
            const rtp = _qtGAllTypes.find(x => x.key === r.to_type_key);
...
async function _qtGEditMaxDays(key, currentVal) {
...
function _qtGGetGroupKeys(groupId) {
    if (!groupId) return [];
    const keys = [];
    // Leader (section_order > 0)
    const leader = _qtGSections.find(s => s.section_group === groupId);
    if (leader) keys.push(leader.key);
    // Members (section_order = 0)
    for (const m of _qtGGroupMembers) {
...
// ========== PHASE MANAGEMENT ==========
function _qtGShowAddPhaseModal() {
    const presetsHTML = GDHT_STAGE_PRESETS.map((p, i) => `
    ...
    const nextOrder = _qtGRulePhases.length > 0 ? Math.max(..._qtGRulePhases.map(p => p.sort_order)) + 1 : 1;
        ...
        <div class="qt-modal">
            ...
            <div style="margin-top:8px;">
                ...
                <div id="qtPhaseSectionsList" style="max-height:200px;overflow-y:auto;margin-top:4px;">
                    ${_qtGSections.filter(s => !s.rule_phase).map(s => `
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtGAddPhase()">📂 Thêm Phần</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtGAddPhase() {
    ...
    await apiCall('/api/consult-rule-phases', 'PUT', { phases: _qtGRulePhases });
...
function _qtGEditPhase(phaseId) {
    const phase = _qtGRulePhases.find(p => p.id === phaseId);
    if (!phase) return;

    // Sections in this phase + available sections (no phase or different phase)
    const allSectioned = _qtGSections;
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtGSavePhase('${phaseId}')">💾 Lưu</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtGSavePhase(phaseId) {
    const phase = _qtGRulePhases.find(p => p.id === phaseId);
    if (!phase) return;
...
async function _qtGDeletePhase(phaseId) {
    const phase = _qtGRulePhases.find(p => p.id === phaseId);
    if (!phase) return;
    const count = _qtGSections.filter(s => s.rule_phase === phaseId).length;
    ...
    const updates = _qtGSections.filter(s => s.rule_phase === phaseId).map(s => ({ key: s.key, rule_phase: null }));
    if (updates.length > 0) await apiCall('/api/consult-types/batch/rule-phase', 'PATCH', { updates });

    _qtGRulePhases = _qtGRulePhases.filter(p => p.id !== phaseId);
    await apiCall('/api/consult-rule-phases', 'PUT', { phases: _qtGRulePhases });
...
function _qtGShowAddSectionModal() {
    if (_qtGUnsectioned.length === 0) return showToast('✅ Tất cả nút đã có loại!', 'success');
    const nextOrder = _qtGSections.length > 0 ? Math.max(..._qtGSections.map(s => s.section_order)) + 1 : 1;

    let listHTML = _qtGUnsectioned.map(t => `
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtGAddSections()">📌 Thêm loại</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtGAddSections() {
...
function _qtGEditSectionOrder(key, currentOrder) {
    const tp = _qtGAllTypes.find(x => x.key === key);
    const label = tp ? `${tp.icon} ${tp.label}` : key;
    const sec = _qtGSections.find(s => s.key === key);
    const currentPhase = sec ? sec.rule_phase : '';

    const phaseOptions = _qtGRulePhases.map(p =>
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtGSaveSectionEdit('${key}',${currentOrder})">💾 Lưu</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtGSaveSectionEdit(key, oldOrder) {
...
async function _qtGDeleteSection(key) {
    const sec = _qtGSections.find(s => s.key === key);
    const isGroup = sec && sec.section_group;
    const groupKeys = isGroup ? _qtGGetGroupKeys(sec.section_group) : [key];
    const tp = _qtGAllTypes.find(x => x.key === key);
    const displayLabel = (isGroup && sec.section_group_label) ? sec.section_group_label : (tp ? `${tp.icon} ${tp.label}` : key);
    const ruleCount = (_qtGAllRules[key] || []).length;
    ...
    try {
        console.log('[_qtGDeleteSection] Deleting key:', key, 'groupKeys:', groupKeys);
        for (const k of groupKeys) {
            console.log('[_qtGDeleteSection] Processing key:', k);
            const r1 = await apiCall(`/api/consult-flow-rules/${k}`, 'DELETE');
            console.log('[_qtGDeleteSection] DELETE flow-rules:', k, r1);
            const r2 = await apiCall(`/api/consult-types/${k}/section-order`, 'PATCH', { section_order: 0 });
            console.log('[_qtGDeleteSection] PATCH section-order:', k, r2);
            const r3 = await apiCall(`/api/consult-types/${k}/rule-phase`, 'PATCH', { rule_phase: null });
            console.log('[_qtGDeleteSection] PATCH rule-phase:', k, r3);
            // Clear group fields
            const r4 = await apiCall(`/api/consult-types/${k}`, 'PATCH', { section_group: null, section_group_label: null });
            console.log('[_qtGDeleteSection] PATCH group fields:', k, r4);
        }
        ...
        console.error('[_qtGDeleteSection] ERROR:', e);
        showToast('❌ Lỗi xóa loại: ' + (e.message || ''), 'error');
    }
}

function _qtGToggleSection(header) {
...
function _qtGRenderFlowchart() {
    // Build journey nodes dynamically from _qtGSections (sorted by section_order)
    ...
    function renderNodeRow(sections) {
        ...
        for (let i = 0; i < sections.length; i++) {
            ...
            const t = _qtGAllTypes.find(x => x.key === sec.key);
            const icon = t ? t.icon : '📋';
            ...
            const isGroup = !!sec.section_group;
            const groupKeys = isGroup ? _qtGGetGroupKeys(sec.section_group) : [];
            ...
            nodesHtml += `
                <div class="qt-fc-node" onclick="_qtGScrollToRule('${sec.key}')" title="${displayName}${isGroup ? ' (nhóm ' + multiCount + ' nút: ' + groupKeys.map(k => { const gt = _qtGAllTypes.find(x => x.key === k); return gt ? gt.label : k; }).join(', ') + ')' : ''}">
...
// ========== EDIT RULES MODAL ==========
function _qtGShowEditRulesModal(fromStatus) {
    const rules = _qtGAllRules[fromStatus] || [];
    const statusLabel = GDHT_FLOW_STATUS_LABELS[fromStatus] || _qtGGetTypeLabel(fromStatus) || fromStatus;

    // ★ Show ALL active types as eligible targets
    const eligibleTypes = _qtGAllTypes.filter(t => {
    ...
    const unsectionedKeys = new Set(_qtGUnsectioned.map(u => u.key));
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtGSaveRules('${fromStatus}')">💾 Lưu quy tắc</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtGSaveRules(fromStatus) {
    ...
    const sec = _qtGSections.find(s => s.key === fromStatus);
    const groupKeys = (sec && sec.section_group) ? _qtGGetGroupKeys(sec.section_group) : [fromStatus];
    for (const key of groupKeys) {
        await apiCall(`/api/consult-flow-rules/${key}`, 'PUT', { rules, crm_menu: 'goi_hop_tac' });
    }
...
function _qtGShowAddRuleGroupModal() {
    // Show unsectioned types (exclude system keys that already have dedicated workflows)
    const SYSTEM_KEYS = ['cap_cuu_sep', 'huy'];
    const sectionKeys = new Set(_qtGSections.map(s => s.key));
    const available = _qtGAllTypes.filter(t => !sectionKeys.has(t.key) && t.is_active && !t.section_group && !SYSTEM_KEYS.includes(t.key));
        ...
        <div class="qt-modal">
            ...
            <div style="margin-top:12px;">
                ...
                <select id="qtNewRulePhase" style="margin-top:4px;">
                    <option value="">(Chưa phân phần)</option>
                    ${_qtGRulePhases.map(p => `<option value="${p.id}">${p.icon} PHẦN ${p.sort_order}: ${p.title}</option>`).join('')}
                </select>
            </div>
            <div style="margin-top:8px;">
                <label style="font-size:13px;font-weight:600;color:#334155;">Bắt đầu từ Loại số:</label>
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtGAddRuleGroup()">➕ Thêm</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtGAddRuleGroup() {
    ...
    if (isGroup) {
        ...
        for (const key of selected) {
            await apiCall(`/api/consult-flow-rules/${key}`, 'PUT', {
                rules: [{ to_type_key: key, is_default: key === leaderKey, delay_days: 0, sort_order: selected.indexOf(key) + 1 }], crm_menu: 'goi_hop_tac'
            });
            // Set group for all
    ...
    setTimeout(() => _qtGShowEditRulesModal(selected[0]), 300);
}

// ========== HELPERS ==========
function _qtGGetTypeLabel(key) {
    const t = _qtGAllTypes.find(x => x.key === key);
    return t ? `${t.icon} ${t.label}` : key;
}

async function _qtGSaveSortOrder(orders) {
    try {
        await apiCall('/api/consult-types/batch/sort-order', 'PATCH', { orders });
