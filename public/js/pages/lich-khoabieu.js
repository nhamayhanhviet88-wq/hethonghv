...
let _kbLockTasks = [], _kbLockCompletions = {}, _kbLockHolidays = new Set(), _kbDeptJoinedAt = null, _kbUserCreatedAt = null; // CV Khóa data
...
// ★ Cache pending review data globally — no API call needed for modal
var _kbLockReviewCache = {};
var _kbChainReviewCache = {};

// ========== LOCK TASK REVIEW MODAL ==========
function _kbShowLockReviewModal(completionId) {
    const comp = _kbLockReviewCache[completionId];
            ...
            <div style="background:linear-gradient(135deg,#991b1b,#dc2626);padding:18px 24px;border-radius:16px 16px 0 0;">
                <div style="display:flex;justify-content:space-between;align-items:center;">
                    <div>
                        ...
                        <div style="font-size:16px;color:white;font-weight:800;margin-top:4px;">${comp.task_name || 'N/A'}</div>
                    </div>
...
function _kbShowChainReviewModal(completionId) {
    const comp = _kbChainReviewCache[completionId];
    if (!comp) { alert('Không tìm thấy báo cáo CV Chuỗi này'); return; }

    const dateF = (comp.deadline || '').split('-').reverse().join('/');
    const createdAt = comp.created_at ? new Date(comp.created_at).toLocaleString('vi-VN') : '';
    const proofHtml = comp.proof_url ? '<div style="margin-top:10px;"><img src="' + comp.proof_url + '" style="max-width:100%;max-height:300px;border-radius:8px;border:2px solid #e5e7eb;cursor:pointer;" onclick="window.open(this.src,\'_blank\')" /></div>' : '';
            ...
            <div style="background:linear-gradient(135deg,#1e40af,#2563eb);padding:18px 24px;border-radius:16px 16px 0 0;">
                <div style="display:flex;justify-content:space-between;align-items:center;">
                    <div>
                        ...
                        <div style="font-size:16px;color:white;font-weight:800;margin-top:4px;">${comp.task_name || 'N/A'}</div>
                        <div style="font-size:11px;color:rgba(255,255,255,0.7);margin-top:2px;">${comp.chain_name || ''}</div>
                    </div>
            ...
            <div style="padding:20px 24px;">
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:16px;">
                    ...
                    <div style="background:#f8fafc;border-radius:10px;padding:12px;">
                        ...
                        <div style="font-size:14px;font-weight:700;color:#1e293b;margin-top:4px;">${comp.quantity_done || 0}/${comp.min_quantity || 1}</div>
                    </div>
                </div>
                ${comp.content ? '<div style="background:#fffbeb;border:1px solid #fde68a;border-radius:10px;padding:14px;margin-bottom:14px;"><div style="font-size:10px;color:#92400e;font-weight:700;text-transform:uppercase;margin-bottom:6px;">📝 Nội dung báo cáo</div><div style="font-size:13px;color:#1e293b;line-height:1.6;white-space:pre-wrap;">' + comp.content + '</div></div>' : ''}
                ${proofHtml ? '<div style="background:#f0fdf4;border:1px solid #a7f3d0;border-radius:10px;padding:14px;"><div style="font-size:10px;color:#065f46;font-weight:700;text-transform:uppercase;margin-bottom:6px;">📸 Ảnh minh chứng</div>' + proofHtml + '</div>' : ''}
...
async function _kbLoadSchedule() {
    ...
    try {
        ...
        window._kbLockSupportRequests = {}; _kbDeptJoinedAt = null; _kbUserCreatedAt = null;
    }
        ...
        sortedSlots.forEach((slot, idx) => {
            ...
            for (let d = 1; d <= 7; d++) {
                ...
                if (_linkedPage && canReport && !_isPastDay) {
                    ...
                    if (report && report.status === 'approved' && report.points_earned > 0) {
    ...
    if (_kbLockTasks && _kbLockTasks.length > 0) {
        ...
        _kbLockTasks.forEach(lt => {
            ...
            for (let d = 1; d <= 7; d++) {
                ...
                // ★ Skip dates before user was assigned this task OR joined department OR account created
                // Priority: assigned_at (ngày giao CV) > department_joined_at (ngày vào phòng) > user_created_at (ngày tạo TK) > created_at (ngày tạo task)
                let effectiveStartDate = '';
                const _rawStart = lt.assigned_at || _kbDeptJoinedAt || _kbUserCreatedAt || lt.created_at;
                if (_rawStart) {
...
async function _kbLoadApprovalPanel() {
    ...
    try {
        ...
        // ★ Cache data for review modals (no extra API call needed)
        _kbLockReviewCache = {};
        lockReviews.forEach(r => { _kbLockReviewCache[r.id] = r; });
        _kbChainReviewCache = {};
        chainReviews.forEach(r => { _kbChainReviewCache[r.id] = r; });
        ...
        lockReviews.forEach(r => {
            ...
            rows += `<tr style="border-bottom:1px solid #f1f5f9;${lockOverdue ? 'background:#fef2f2;' : lockUrgent ? 'background:#fffbeb;' : ''}">
                ...
                <td style="padding:8px 12px;text-align:center;">
                    <span onclick="_kbShowLockReviewModal(${r.id})" style="background:#fef2f2;border:1px solid #fecaca;padding:4px 10px;border-radius:6px;cursor:pointer;font-size:14px;display:inline-flex;align-items:center;gap:4px;" title="Xem chi tiết báo cáo">👁️ Xem</span>
                </td>
        ...
        chainReviews.forEach(r => {
            ...
            rows += `<tr style="border-bottom:1px solid #f1f5f9;${isOverdue ? 'background:#fef2f2;' : isUrgent ? 'background:#fffbeb;' : ''}">
                ...
                <td style="padding:8px 12px;text-align:center;">
                    <span onclick="_kbShowChainReviewModal(${r.id})" style="background:#eff6ff;border:1px solid #bfdbfe;padding:4px 10px;border-radius:6px;cursor:pointer;font-size:14px;display:inline-flex;align-items:center;gap:4px;" title="Xem chi tiết báo cáo">👁️ Xem</span>
                </td>
...
function _kbRenderDangContentMini(el, res) {
    ...
    if (isPending) {
        ...
        const _dcColor = done ? '#059669' : '#8b5cf6';
        el.innerHTML = `<div style="margin-top:4px;"><div style="font-size:9px;margin-bottom:2px;">${done ? `<span style="color:#059669;font-weight:800;">✅ ${count}/${target}</span>` : `<span style="color:#8b5cf6;font-weight:700;">✍️ ${count}/${target}</span>`}</div><div style="background:#e5e7eb;border-radius:4px;height:5px;overflow:hidden;"><div style="background:${_dcColor};height:100%;width:${pct}%;border-radius:4px;transition:width .5s;"></div></div></div>`;
    }
}
...
function _kbRenderDangGroupMini(el, res) {
    ...
    if (isPending) {
        ...
        const _dgColor = done ? '#059669' : '#0891b2';
        el.innerHTML = `<div style="margin-top:4px;"><div style="font-size:9px;margin-bottom:2px;">${done ? `<span style="color:#059669;font-weight:800;">✅ ${count}/${target}</span>` : `<span style="color:#0891b2;font-weight:700;">📢 ${count}/${target}</span>`}</div><div style="background:#e5e7eb;border-radius:4px;height:5px;overflow:hidden;"><div style="background:${_dgColor};height:100%;width:${pct}%;border-radius:4px;transition:width .5s;"></div></div></div>`;
    }
}
...
function _kbRenderTuyenDungMini(el, res) {
    ...
    if (isPending) {
        ...
        const _tdColor = done ? '#059669' : '#be185d';
        el.innerHTML = `<div style="margin-top:4px;"><div style="font-size:9px;margin-bottom:2px;">${done ? `<span style="color:#059669;font-weight:800;">✅ ${count}/${target}</span>` : `<span style="color:#be185d;font-weight:700;">🎓 ${count}/${target}</span>`}</div><div style="background:#e5e7eb;border-radius:4px;height:5px;overflow:hidden;"><div style="background:${_tdColor};height:100%;width:${pct}%;border-radius:4px;transition:width .5s;"></div></div></div>`;
    }
}
...
function _kbRenderSeddingMini(el, res) {
    ...
    if (isPending) {
        ...
        const _sdColor = done ? '#059669' : '#ea580c';
        el.innerHTML = `<div style="margin-top:4px;"><div style="font-size:9px;margin-bottom:2px;">${done ? `<span style="color:#059669;font-weight:800;">✅ ${count}/${target}</span>` : `<span style="color:#ea580c;font-weight:700;">🌐 ${count}/${target}</span>`}</div><div style="background:#e5e7eb;border-radius:4px;height:5px;overflow:hidden;"><div style="background:${_sdColor};height:100%;width:${pct}%;border-radius:4px;transition:width .5s;"></div></div></div>`;
    }
}
...
function _kbRenderZaloMini(el, res) {
    ...
    if (isPending) {
        ...
        const _zlColor = done ? '#059669' : '#0284c7';
        el.innerHTML = `<div style="margin-top:4px;"><div style="font-size:9px;margin-bottom:2px;">${done ? `<span style="color:#059669;font-weight:800;">✅ ${count}/${target}</span>` : `<span style="color:#0284c7;font-weight:700;">🔍 ${count}/${target}</span>`}</div><div style="background:#e5e7eb;border-radius:4px;height:5px;overflow:hidden;"><div style="background:${_zlColor};height:100%;width:${pct}%;border-radius:4px;transition:width .5s;"></div></div></div>`;
    }
}
...
function _kbRenderDangBTMini(el, res) {
    ...
    if (isPending) {
        ...
        const _btColor = done ? '#059669' : '#d946ef';
        el.innerHTML = `<div style="margin-top:4px;"><div style="font-size:9px;margin-bottom:2px;">${done ? `<span style="color:#059669;font-weight:800;">✅ ${count}/${target}</span>` : `<span style="color:#a21caf;font-weight:700;">📸 ${count}/${target}</span>`}</div><div style="background:#e5e7eb;border-radius:4px;height:5px;overflow:hidden;"><div style="background:${_btColor};height:100%;width:${pct}%;border-radius:4px;transition:width .5s;"></div></div></div>`;
    }
}
