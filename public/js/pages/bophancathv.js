...
+'.bpc-sb-total{background:#be123c;color:#fff;border-left:5px solid #9f1239;padding:12px 16px 12px 11px;font-size:13px;font-weight:800;display:flex;justify-content:space-between;align-items:center;cursor:pointer;position:relative;overflow:hidden;border-bottom:1px solid #be123c;transition:all 0.2s}'
+'.bpc-sb-total:hover{background:#9f1239}'
+'.bpc-sb-total.active{background:linear-gradient(135deg,#be123c,#e11d48);color:#fff;border-left-color:#fbbf24;box-shadow:0 2px 8px rgba(190,18,60,0.4);font-weight:900}'
...
+'.bpc-sb-uncut{background:#ea580c;color:#fff;border-left:5px solid #c2410c;padding:11px 16px 11px 11px;font-size:12px;font-weight:800;display:flex;justify-content:space-between;align-items:center;cursor:pointer;border-bottom:1px solid #ea580c;transition:all 0.2s}'
+'.bpc-sb-uncut:hover{background:#c2410c}'
+'.bpc-sb-uncut.active{background:linear-gradient(135deg,#c2410c,#f97316);color:#fff;border-left-color:#fbbf24;box-shadow:0 2px 8px rgba(234,88,12,0.4);font-weight:900}'
...
+'.bpc-sb-year{padding:9px 16px 9px 11px;font-weight:800;font-size:12px;color:#002277;cursor:pointer;display:flex;justify-content:space-between;align-items:center;background:#cce0ff;border-bottom:1px solid #cce0ff;transition:all 0.15s;border-left:5px solid #60a5fa}'
...
+'.bpc-icon-btn{width:26px;height:26px;border-radius:6px;border:1.5px solid #e2e8f0;background:#fff;cursor:pointer;display:inline-flex;align-items:center;justify-content:center;font-size:12px;transition:all .15s;margin:0 1px;font-family:"Inter",system-ui,sans-serif}'
...
+'.bpc-icon-btn.on-pending{background:#fbbf24;border-color:#d97706;color:#78350f;font-weight:800;width:auto !important;padding:0 8px;font-size:11px;animation:bpcPulseAnimation 1.5s infinite;font-family:"Inter",system-ui,sans-serif;letter-spacing:0.3px}'
            ...
            try {
                ...
                var unassignedRecords = filteredUnassigned.map(function(ur) {
                    ...
                    return {
                        ...
                        cutter_name: null,
                        cutter_id: null,
                        printing_contractor_id: ur.printing_contractor_id || null,
                        contractor_has_shelf: ur.contractor_has_shelf || false,
                        is_cutting: false,
                        is_cut_done: false,
...
function _bpcFmtTimeDateNoYear(d) {
    if (!d) return '—';
    try {
        var str = '';
...
function _bpcMapRecordRow(r, i) {
    ...
    return '<tr>'
        ...
        +'<td style="text-align:center;font-weight:800;color:'+ratioColor+'">'+(r.is_cut_done && r.cut_ratio ? r.cut_ratio + ' sp/' + (r.fabric_unit || 'kg') : '—')+'</td>'
...
function _bpcBuildUnassignedTableHtml(all) {
    ...
    if (!all.length) {
        ...
        all.forEach(function(r, i) {
            ...
            var claimTd = '<td colspan="4" style="text-align:center;vertical-align:middle;border-left:2px solid #e2e8f0">'+claimHtml+'</td>';
...
async function _bpcClaimOrder(orderId, itemId, phoiIndex, orderCode) {
    ...
    h += '<div class="bpc-modal-header"><div class="m-icon">✂️</div><div><div class="m-title">Nhận Đơn Cắt</div><div class="m-sub">Xác nhận nhận phối cắt này</div></div></div>';
    ...
    // Phối details
    if (rows.length > 0) {
        h += '<div class="bpc-modal-phoi"><div class="bpc-modal-phoi-title">📦 Chi tiết phối</div>';
...
async function _bpcDoClaimOrder(orderId, itemId, phoiIndex, orderCode) {
    if (window._bpcBusy) return;
    window._bpcBusy = true;
...
async function _bpcOpenCutModal(recordId) {
    ...
    h += '<div class="bpc-modal-actions" id="_bpcCutActions" style="display:none"><button class="bpc-modal-btn cancel" onclick="_bpcCloseCutModal()">Hủy</button><button class="bpc-modal-btn confirm" id="_bpcCutConfirmBtn" disabled style="opacity:0.5;cursor:not-allowed;" onclick="_bpcDoCut('+recordId+')">✂️ XÁC NHẬN CẮT</button></div>';
    h += '</div></div>';
    document.body.insertAdjacentHTML('beforeend', h);
    ...
    try {
        ...
        if (cutReminders.length > 0) {
            ...
            cutReminders.forEach(function(rem, remIdx) {
                ...
                bh += '    <div style="display:flex;align-items:center;gap:10px;background:#fff;border:1.5px solid ' + (isViewed ? '#059669' : '#fca5a5') + ';border-radius:10px;padding:10px 12px;transition:all 0.3s">';
                bh += '       <input type="checkbox" class="bpc-reminder-cb" data-reminder-id="' + remId + '" ' + (isViewed ? 'checked' : '') + ' style="display:none">';
                bh += '       <div style="flex:1;font-size:12px;font-weight:700;color:#7f1d1d;line-height:1.4">' + rem.replace(/</g, '&lt;').replace(/>/g, '&gt;') + '</div>';
                if (isViewed) {
...
function _bpcToggleReminder(btn) {
    var remId = btn.getAttribute('data-reminder-id');
    var cb = document.querySelector('.bpc-reminder-cb[data-reminder-id="' + remId + '"]');
    if (!cb) return;
...
// Add CSS animation for cutting reminder pulse
(function() {
    if (document.getElementById('bpcReminderStyles')) return;
    var style = document.createElement('style');
    style.id = 'bpcReminderStyles';
    style.textContent = '@keyframes bpcReminderPulse { 0%,100%{transform:scale(1);box-shadow:0 0 0 0 rgba(220,38,38,0.4)} 50%{transform:scale(1.05);box-shadow:0 0 0 8px rgba(220,38,38,0)} }';
...
async function _bpcOpenDetail(recordId) {
    ...
    try {
        ...
        try {
            ...
            console.error('[BPC] Load reminders error:', e);
        }
        var rolls = [];
...
async function _bpcOpenDoneModal(recordId, isRefresh = false) {
        ...
        rolls = snapRolls.map(sr => {
            ...
            if (live) {
                return {
                    ...
                    is_reserved_for_this_order: live.is_reserved_for_this_order || false
                };
            } else {
                return {
                    roll_id: sr.roll_id,
                    ...
                    is_newly_synced: isNewlySynced,
                    is_added_roll: sr.is_added_roll || false,
                    is_reserved_for_this_order: false
                };
            }
        });
    } catch(e) {
    ...
    if (rolls.length) {
        rolls.forEach(function(rl, idx) {
            ...
            var isArrivedCalled = (rl.is_added_roll || rl.is_newly_synced) && rl.is_reserved_for_this_order;
...
async function _bpcSubmitDone(recordId) {
    ...
    var body = { action: 'cut_done', cut_quantity: qty, roll_remains: rollRemains, need_compensate: needCompensate };
...
function _bpcGDoneRecalc() {
    ...
    d.records.forEach(function(r) {
        var inp = document.getElementById('_bpcGQ_' + r.id);
        var q = inp ? parseInt(inp.value) || 0 : 0;
        
        // Show/hide group compensation area
        var max = r.order_quantity || 0;
        var comp = document.getElementById('_bpcGroupCompensate_' + r.id);
        if (comp && inp) {
            if (q < max && !inp.readOnly) {
        ...
        qtys.push({ id: r.id, qty: q, name: r.product_name || r.order_code });
        totalQty += q;
    });
    var dh = '';
    var combinedRatio = 0;
...
async function _bpcSubmitGDone() {
    ...
    for (var i = 0; i < d.records.length; i++) {
        ...
        var compSelect = document.getElementById('_bpcGroupCompSelect_' + r.id);
        var needCompensate = compSelect ? (compSelect.value === 'true') : false;
        items.push({ record_id: r.id, cut_quantity: qty, need_compensate: needCompensate });
        totalQty += qty;
    }
