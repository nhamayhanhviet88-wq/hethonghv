...
var _bpc = { records: [], tree: null, filter: { year: null, month: null, cutter_id: null, status: null }, search: '', page: 1, pageSize: 100 };
var _bpcOpen = {};
...
+'.bpc-sb-month{padding:6px 16px 6px 28px;font-size:11px;font-weight:700;cursor:pointer;display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid #f0f0f0;color:#dc2626}'
+'.bpc-sb-month:hover{background:#fef2f2}.bpc-sb-month.active{background:#fee2e2;font-weight:800}'
+'.bpc-sb-cutter{padding:5px 16px 5px 42px;font-size:10px;font-weight:600;cursor:pointer;display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid #fafafa;color:#64748b}'
+'.bpc-sb-cutter:hover{background:#fef2f2}.bpc-sb-cutter.active{background:#fee2e2;color:#dc2626;font-weight:800}'
...
+'@media(max-width:768px){.bpc-sidebar{display:none}}';
    ...
    content.innerHTML = '<div class="bpc-wrap"><div class="bpc-sidebar" id="bpcSidebar"><div style="padding:20px;text-align:center;color:var(--gray-400);font-size:12px">Đang tải...</div></div><div class="bpc-main">'
        ...
        +'<div class="card"><div class="card-body" style="overflow-x:auto;padding:8px"><table class="table" style="font-size:11px;white-space:nowrap" id="bpcTable"><thead><tr style="background:var(--gray-800)">'
        ...
        +'</tr></thead><tbody id="bpcTbody"><tr><td colspan="21" style="text-align:center;padding:40px">⏳</td></tr></tbody></table></div></div>'
    ...
    var _st; document.getElementById('bpcSearch').addEventListener('input', function() {
        clearTimeout(_st); _st = setTimeout(function() { _bpc.search = document.getElementById('bpcSearch').value || ''; _bpc.page = 1; _bpcRenderTable(); }, 300);
    });
    _bpcLoadAll();
}
        ...
        t.tree.forEach(function(yr) {
            ...
            if (yOpen && yr.months) {
                yr.months.forEach(function(mo) {
                    var mKey = 'm'+yr.year+'_'+mo.month;
                    var mOpen = !!_bpcOpen[mKey];
                    var mAct = f.year==yr.year && f.month==mo.month && !f.cutter_id;
                    h += '<div class="bpc-sb-month'+(mAct?' active':'')+'" onclick="event.stopPropagation();_bpcToggle(\''+mKey+'\');_bpcFilter('+yr.year+','+mo.month+')"><span>'+(mOpen?'▼':'▶')+' T'+String(mo.month).padStart(2,'0')+'</span><span>'+mo.count+'</span></div>';
                    if (mOpen && mo.cutters) {
                        mo.cutters.forEach(function(c) {
                            var cAct = f.year==yr.year && f.month==mo.month && f.cutter_id==c.id;
                            h += '<div class="bpc-sb-cutter'+(cAct?' active':'')+'" onclick="event.stopPropagation();_bpcFilter('+yr.year+','+mo.month+','+c.id+')"><span>👤 '+(c.name||'Chưa PC')+'</span><span>'+c.count+'</span></div>';
                        });
                    }
                });
            }
...
function _bpcFilter(year, month, cutterId) {
    _bpc.filter = { year: year||null, month: month||null, cutter_id: cutterId||null };
    _bpc.page = 1;
    _bpcRenderSidebar();
    _bpcLoadRecords();
}
...
function _bpcRenderRows(paged) {
    ...
    if (!paged.length) { tbody.innerHTML = '<tr><td colspan="21"><div class="empty-state"><div class="icon">✂️</div><h3>Chưa có đơn cắt nào</h3><p>Bấm nút + để tạo đơn cắt mới</p></div></td></tr>'; return; }
...
function _bpcReportError(id) {
    // Navigate to Đơn Lỗi Khách & Nội Bộ
