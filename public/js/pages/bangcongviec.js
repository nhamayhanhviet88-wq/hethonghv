                        ...
                        ${checklist.map(function(item) {
                            ...
                            return '<div class="bcv-cl-card ' + (item.is_done ? 'done' : '') + '" data-cl-id="' + item.id + '">' +
                                ...
                                (canEditReport ? '<div class="bcv-cl-card-body" id="bcvClForm_' + item.id + '" style="display:' + (hasData ? 'none' : 'block') + '">' +
                                    ...
                                    '<textarea class="bcv-cl-input" id="bcvClContent_' + item.id + '" placeholder="Nhập ghi chú chi tiết..." style="min-height:50px;resize:vertical">' + _esc(item.content || '') + '</textarea>' +
                                    ...
                                    '<input type="text" class="bcv-cl-input" id="bcvClLink_' + item.id + '" value="' + _escAttr(item.link || '') + '" placeholder="https://...">' +
                                    '<div style="display:flex;align-items:center;justify-content:flex-end;margin-top:6px">' +
                                        '<button class="bcv-btn" data-no-debounce="true" onclick="_bcvSaveChecklistDetail(' + task.id + ',' + item.id + ')" style="padding:6px 14px;font-size:11px;font-weight:800;background:linear-gradient(135deg,#2563eb,#1d4ed8);color:#fff;border-radius:8px;border:none;cursor:pointer;box-shadow:0 2px 6px rgba(37,99,235,.25);display:inline-flex;align-items:center;gap:4px">💾 Lưu</button>' +
                                    '</div>' +
                                '</div>' : '') +
                            '</div>';
                        }).join('')}
