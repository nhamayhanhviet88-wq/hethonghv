...
async function _wtViewDetail(id){
    try{
        ...
        replies.forEach(function(rp){
            ...
            var rpImgs2='';
            rAttach.forEach(function(a){if(a.type==='image'&&a.url)rpImgs2+='<img src="'+a.url+'" style="max-width:180px;max-height:110px;border-radius:6px;margin-top:4px;cursor:pointer;border:1px solid '+(isCreator2?'#bfdbfe':'#bbf7d0')+'" onclick="window.open(this.src,\'_blank\')">';if(a.type==='video'&&a.url)rpImgs2+='<div style="margin-top:4px"><video src="'+a.url+'" controls style="max-width:240px;max-height:140px;border-radius:6px;border:1px solid '+(isCreator2?'#bfdbfe':'#bbf7d0')+'"></video></div>';});
            if(isCreator2){
...
function _wtReplySelectPL(k,reqImg){
    ...
    // Khẩn Cấp bắt buộc hình ảnh, các mức khác tùy chọn
    window._wtReplyReqImg=(k==='urgent');
    var reqEl=document.getElementById('wtReplyImgReq'),optEl=document.getElementById('wtReplyImgOpt');
    if(k==='urgent'){
        if(reqEl)reqEl.style.display='inline';if(optEl)optEl.style.display='none';
    } else {
...
async function _wtReply(id){
    ...
    // Khẩn Cấp bắt buộc ảnh
    if(window._wtReplyReqImg&&!window._wtReplyImg){
        showToast('🔴 Mức độ Khẩn Cấp bắt buộc phải có hình ảnh!','error');
        var pz=document.getElementById('wtReplyPaste');
        if(pz){pz.style.borderColor='#dc2626';pz.style.background='#fef2f2';}
        return;
    }
