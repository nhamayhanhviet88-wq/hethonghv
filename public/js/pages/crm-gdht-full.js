// ========== CRM G?i Ði?n H?p Tác C?U — 14-column layout with consultation system ==========
...
function _gdhtRenderCustomerRow(c, stats, stt) {
    ...
    return `<tr class="${_pinClass}">
        <td style="text-align:center;padding:4px 2px;">
            ${!c.readonly ? `<span class="crm-pin-btn ${c.is_pinned ? 'active' : ''}" onclick="event.stopPropagation();_gdhtTogglePin(${c.id})" title="${c.is_pinned ? 'B? pin' : 'Pin khách'}">${c.is_pinned ? '??' : '<span style="opacity:0.3">??</span>'}</span>` : ''}
        </td>
        ...
        <td>
            ${!c.readonly ? '<button class="btn btn-sm" onclick="event.stopPropagation();_gdhtOpenCustomerInfo(' + c.id + ')" style="font-size:9px;padding:1px 5px;margin-right:4px;background:var(--gray-700);color:var(--gold);" title="C?p nh?t thông tin">??</button>' : ''}
            ${(() => {
                const _colors = [
...
async function _gdhtOpenConsultModal(customerId) {
    ...
    const footerHTML = `
        ...
        <button class="btn btn-primary" id="consultSubmitBtn" onclick="_gdhtSubmitConsultLog(${customerId})" style="width:auto;">?? GHI NH?N</button>
    `;
...
async function _gdhtSubmitConsultLog(customerId) {
    ...
    if (log_type === 'huy') {
        ...
        if (!reason) { showToast('Vui lòng nh?p lý do h?y!', 'error'); _gdhtEnableSubmitBtn(); return; }
        try {
            ...
            else { showToast(data.error || 'L?i!', 'error'); _gdhtEnableSubmitBtn(); }
        } catch (err) { showToast('L?i k?t n?i!', 'error'); _gdhtEnableSubmitBtn(); }
        return;
    }
    ...
    if (log_type === 'cap_cuu_sep') {
        ...
        if (!content) { showToast('Vui lòng nh?p n?i dung tình hu?ng!', 'error'); _gdhtEnableSubmitBtn(); return; }
        if (!handler_id) { showToast('Vui lòng ch?n S?p x? lý!', 'error'); _gdhtEnableSubmitBtn(); return; }
        try {
    ...
    if (log_type === 'dat_coc') {
        ...
        if (depositAmount <= 0) {
            showToast('Vui lòng nh?p s? ti?n d?t c?c!', 'error'); _gdhtEnableSubmitBtn(); return;
        }
        ...
        try {
            ...
            if (data.success) {
                showToast('? Ð?t c?c thành công!'); closeModal(); window._consultImageBlob = null; loadCrmGdhtData();
            } else { showToast(data.error || 'L?i!', 'error'); _gdhtEnableSubmitBtn(); }
        } catch (err) { showToast('L?i k?t n?i!', 'error'); _gdhtEnableSubmitBtn(); }
        return;
    }
    ...
    if (log_type === 'chot_don') {
        ...
        if (!address) { showToast('Vui lòng nh?p d?a ch?!', 'error'); _gdhtEnableSubmitBtn(); return; }
        if (!city) { showToast('Vui lòng ch?n thành ph?!', 'error'); _gdhtEnableSubmitBtn(); return; }
        if (!sbhDate) { showToast('Vui lòng ch?n ngày h?n sau bán hàng!', 'error'); _gdhtEnableSubmitBtn(); return; }

        // Phone validate
        if (phone && !/^\d{10}$/.test(phone)) {
            showToast('SÐT ph?i dúng 10 ch? s?', 'error'); _gdhtEnableSubmitBtn(); return;
        }

        // Collect order items
        const rows = document.querySelectorAll('#consultOrderTable tbody tr');
        if (rows.length === 0) { showToast('Vui lòng thêm ít nh?t 1 s?n ph?m!', 'error'); _gdhtEnableSubmitBtn(); return; }
        const items = [];
        ...
        if (items.length === 0) { showToast('Vui lòng nh?p s?n ph?m h?p l?!', 'error'); _gdhtEnableSubmitBtn(); return; }
        ...
        try {
            ...
            const data = await res.json();
            if (data.success) {
                showToast('? Ch?t don thành công! Chuy?n sang Sau Bán Hàng.'); closeModal(); window._consultImageBlob = null; loadCrmGdhtData();
            } else { showToast(data.error || 'L?i!', 'error'); _gdhtEnableSubmitBtn(); }
        } catch (err) { showToast('L?i k?t n?i!', 'error'); _gdhtEnableSubmitBtn(); }
        return;
    }
    ...
    if (!content) { showToast('Vui lòng nh?p n?i dung tu v?n!', 'error'); _gdhtEnableSubmitBtn(); return; }
    ...
    if (imageRequiredTypes.includes(log_type) && !window._consultImageBlob) {
        showToast('Vui lòng dán hình ?nh (Ctrl+V)!', 'error'); _gdhtEnableSubmitBtn(); return;
    }
    if (!appointment_date) { showToast('Vui lòng ch?n ngày h?n!', 'error'); _gdhtEnableSubmitBtn(); return; }
...
async function _gdhtOpenConsultHistory(customerId) {
    ...
    let bodyHTML = `
        <div style="display:flex;gap:8px;margin-bottom:16px;">
            ...
            <button class="btn btn-sm tab-btn" onclick="_gdhtSwitchConsultTab('history', this)" style="font-size:12px;">?? L?ch S? (${logs.length})</button>
            <button class="btn btn-sm tab-btn" onclick="_gdhtSwitchConsultTab('order', this)" style="font-size:12px;">?? Ðon Hàng</button>
        </div>
        ...
        <div id="tabInfo">
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;font-size:13px;">
                ...
                ${(c.referrer_name || c.referrer_customer_name) ? `<div><strong>Ngu?i GT:</strong> <span style="cursor:pointer;text-decoration:underline;color:var(--info);" onclick="_gdhtOpenAffiliateDetail(${c.referrer_id})">${c.referrer_name || c.referrer_customer_name}</span></div>` : ''}
    ...
    const footerHTML = `
        ...
        <button class="btn btn-primary" onclick="_gdhtOpenConsultModal(${c.id})" style="width:auto;">?? Tu V?n</button>
        <button class="btn btn-primary" onclick="_gdhtSaveOrderItems(${c.id})" style="width:auto;">?? Luu Ðon</button>
        <button class="btn" onclick="_gdhtRequestCancel(${c.id})" style="width:auto;background:var(--danger);color:white;">? H?y KH</button>
    `;
...
async function _gdhtOpenReferrerSearch(customerId) {
    const bodyHTML = `
        <div class="form-group">
            ...
            <input type="text" id="referrerSearchInput" class="form-control" placeholder="Nh?p tên ho?c SÐT d? l?c..." oninput="_gdhtFilterReferrerList(${customerId})">
        </div>
...
async function _gdhtOpenCustomerInfo(customerId) {
    ...
    const bodyHTML = `
        ...
        <div class="form-group">
            ...
            <button class="btn btn-sm" onclick="_gdhtAddHolidayRow()" style="font-size:12px;margin-top:6px;">? Thêm ngày l?</button>
        </div>
    `;

    const footerHTML = `
        <button class="btn btn-secondary" onclick="closeModal()">H?y</button>
        <button class="btn btn-primary" onclick="_gdhtSaveCustomerInfo(${customerId})" style="width:auto;">?? LUU</button>
    `;
...
const GDHT_CRM_LABELS_AFF = { nhu_cau: 'Cham Sóc KH Nhu C?u', ctv: 'Cham Sóc CTV', tu_tim_kiem: 'CRM T? Tìm Ki?m', goi_hop_tac: 'CRM G?i Ði?n H?p Tác', goi_ban_hang: 'CRM G?i Ði?n Bán Hàng', koc_tiktok: 'CRM KOL/KOC Tiktok' };
const GDHT_ROLE_LABELS_AFF = { giam_doc:'Giám Ð?c', quan_ly_cap_cao:'Qu?n Lý C?p Cao', quan_ly:'Qu?n Lý', truong_phong:'Tru?ng Phòng', nhan_vien:'Nhân Viên', part_time:'Part Time', hoa_hong:'Hoa H?ng', ctv:'CTV', nuoi_duong:'Nuôi Du?ng', sinh_vien:'Sinh Viên', tkaffiliate:'TK Affiliate' };
...
async function _gdhtOpenEditAffiliateFromCrm(userId) {
    try {
        ...
        const footerHTML = `<button class="btn" onclick="_gdhtSubmitEditAffFromCrm(${u.id})" style="background:var(--gold);color:#122546;padding:8px 24px;border-radius:8px;font-weight:600;">?? Luu thay d?i</button>`;
