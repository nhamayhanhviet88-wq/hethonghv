...
async function _qtLoadData() {
    ...
    _qtUnsectioned = (sectionsData.unsectioned || []).filter(t => !['huy', 'cap_cuu_sep'].includes(t.key));
    ...
    if (_qtIsGD) {
        panel.addEventListener('click', function(e) {
            ...
            if (toggleWrap) {
                ...
                if (card && card.dataset.key) _qtToggleActive(card.dataset.key, card);
                return;
            }
        }, true);
    }
...
// ========== TOGGLE ACTIVE ==========
async function _qtToggleActive(key, cardEl) {
    ...
    try {
        await apiCall(`/api/consult-types/${key}`, 'PUT', {
            ...
            is_active: newActive, stage: t.stage || null
        });
...
// Toggle active from section accordion (rules tab)
async function _qtToggleSection_Active(key, toggleWrapEl) {
    const t = _qtAllTypes.find(x => x.key === key);
    if (!t) return;
    const newActive = !t.is_active;
        ...
        <div class="qt-flow-section" id="${sectionId}">
            <div class="qt-flow-header" onclick="_qtToggleSection(this)">
                ...
                <div style="display:flex;align-items:center;gap:8px;">
                    ${_qtIsGD ? (() => {
                        const secType = _qtAllTypes.find(x => x.key === sec.key);
                        const secActive = secType ? secType.is_active : true;
                        return `<div class="qt-toggle-wrap" style="margin:0;gap:4px;" onclick="event.stopPropagation();_qtToggleSection_Active('${sec.key}',this)" title="${secActive ? 'Click để tắt loại này' : 'Click để bật lại loại này'}">
                        ...
                        </div><button class="qt-flow-edit-btn" onclick="event.stopPropagation();_qtShowEditRulesModal('${sec.key}')">✏️ Sửa</button><button class="qt-flow-edit-btn" style="background:#ef4444;padding:4px 8px;min-width:0;" onclick="event.stopPropagation();_qtDeleteSection('${sec.key}')" title="Xóa loại">🗑️</button><button class="qt-flow-edit-btn" style="background:linear-gradient(135deg,#8b5cf6,#7c3aed);padding:4px 8px;min-width:0;" onclick="event.stopPropagation();_qtShowMergeModal('${sec.key}')" title="Gom thêm nút vào loại này">➕</button>${isGroup ? `<button class="qt-flow-edit-btn" style="background:linear-gradient(135deg,#f59e0b,#d97706);padding:4px 8px;min-width:0;" onclick="event.stopPropagation();_qtShowUngroupModal('${sec.key}')" title="Hủy gom nhóm">🔓</button>` : ''}<button class="qt-flow-edit-btn" style="background:linear-gradient(135deg,#0ea5e9,#0284c7);padding:4px 8px;min-width:0;position:relative;" onclick="event.stopPropagation();_qtShowChangePhaseDropdown('${sec.key}',this)" title="Đổi thuộc Phần">▼</button>`;
                    })() : ''}
...
function _qtShowChangePhaseDropdown(key, btnEl) {
    ...
    // "(Chưa phân phần)" option
    const isNone = !currentPhase;
    ddHtml += `<div class="qt-phase-dd-item" style="padding:8px 14px;cursor:pointer;display:flex;align-items:center;gap:8px;font-size:12px;font-weight:${isNone ? '800' : '600'};color:${isNone ? '#2563eb' : '#64748b'};background:${isNone ? '#eff6ff' : 'transparent'};transition:background .15s;border-left:3px solid ${isNone ? '#2563eb' : 'transparent'};" onmouseover="this.style.background='#f1f5f9'" onmouseout="this.style.background='${isNone ? '#eff6ff' : 'transparent'}'" onclick="event.stopPropagation();_qtChangePhase('${key}',null)">
        <span style="font-size:14px;">📋</span>
        <span>(Chưa phân phần)</span>
        ${isNone ? '<span style="margin-left:auto;color:#2563eb;font-size:11px;">✓</span>' : ''}
    </div>`;

    // Each phase
    for (const phase of _qtRulePhases) {
        const isCurrent = currentPhase === phase.id;
        ddHtml += `<div class="qt-phase-dd-item" style="padding:8px 14px;cursor:pointer;display:flex;align-items:center;gap:8px;font-size:12px;font-weight:${isCurrent ? '800' : '600'};color:${isCurrent ? phase.color : '#334155'};background:${isCurrent ? phase.gradient : 'transparent'};transition:background .15s;border-left:3px solid ${isCurrent ? phase.color : 'transparent'};" onmouseover="if(!${isCurrent})this.style.background='#f1f5f9'" onmouseout="if(!${isCurrent})this.style.background='transparent'" onclick="event.stopPropagation();_qtChangePhase('${key}','${phase.id}')">
            <span style="font-size:14px;">${phase.icon}</span>
            <span>PHẦN ${phase.sort_order}: ${phase.title}</span>
            ${isCurrent ? `<span style="margin-left:auto;color:${phase.color};font-size:11px;">✓</span>` : ''}
        </div>`;
    }
...
async function _qtChangePhase(key, phaseId) {
    document.querySelectorAll('.qt-phase-dropdown').forEach(d => d.remove());
    try {
        await apiCall(`/api/consult-types/${key}/rule-phase`, 'PATCH', { rule_phase: phaseId || null });
        // Auto-reindex sections after phase change
...
function _qtShowMergeModal(sectionKey) {
    const sec = _qtSections.find(s => s.key === sectionKey);
    if (!sec) return;
    const isGroup = !!sec.section_group;
    const groupKeys = isGroup ? _qtGetGroupKeys(sec.section_group) : [sec.key];
    const tp = _qtAllTypes.find(x => x.key === sec.key);
    ...
    const SYSTEM_KEYS = ['huy', 'cap_cuu_sep'];
    const sectionKeys = new Set(_qtSections.map(s => s.key));
    const groupMemberKeys = new Set(_qtGroupMembers.map(m => m.key));
    const available = _qtAllTypes.filter(t =>
        t.is_active &&
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div style="display:flex;flex-wrap:wrap;gap:4px;margin-bottom:12px;">
                ${groupKeys.map(k => { const mt = _qtAllTypes.find(x => x.key === k); return mt ? `<span style="font-size:10px;padding:2px 8px;border-radius:6px;background:${mt.color}20;color:${mt.color};font-weight:700;border:1px solid ${mt.color}30;">${mt.icon} ${mt.label}</span>` : ''; }).join('')}
            </div>
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtMergeIntoGroup('${sectionKey}')">➕ Gom vào nhóm</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtMergeIntoGroup(sectionKey) {
    const sec = _qtSections.find(s => s.key === sectionKey);
    if (!sec) return;
    ...
    const leaderRules = _qtAllRules[leaderKey] || [];
...
// ========== UNGROUP ==========
function _qtShowUngroupModal(sectionKey) {
    const sec = _qtSections.find(s => s.key === sectionKey);
    if (!sec || !sec.section_group) return;
    const groupKeys = _qtGetGroupKeys(sec.section_group);
    if (groupKeys.length < 2) return showToast('⚠️ Nhóm chỉ có 1 nút, không cần hủy gom!', 'warning');

    const currentLabel = sec.section_group_label || sec.label || sec.key;

    let listHTML = groupKeys.map(k => {
        const mt = _qtAllTypes.find(x => x.key === k);
        const isLeader = k === sec.key;
        return `
        <div class="qt-rule-item ${isLeader ? 'qt-ri-active' : ''}" data-key="${k}" style="cursor:pointer;" onclick="if(!this.querySelector('.qt-ri-leader')){this.querySelector('input[type=checkbox]').click()}">
            ${isLeader
                ? `<span class="qt-ri-leader" style="width:18px;height:18px;display:flex;align-items:center;justify-content:center;font-size:12px;" title="Nút chủ nhóm — không thể bỏ chọn riêng">👑</span>`
                : `<input type="checkbox" class="qt-ri-check" onclick="event.stopPropagation()">`}
            <div class="qt-ri-info">
                <span class="qt-ri-icon">${mt ? mt.icon : '📋'}</span>
                <span class="qt-ri-label">${mt ? mt.label : k}</span>
            </div>
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            <h3>🔓 Hủy gom nhóm: ${currentLabel}</h3>
            ...
            <div style="padding:8px 12px;background:#fff7ed;border:1px solid #f59e0b30;border-radius:10px;margin-bottom:12px;font-size:11px;color:#92400e;">
                ⚠️ Nút bị bỏ ra sẽ trở thành loại riêng (mất quy tắc liên kết hiện tại).<br>
                👑 Nút chủ nhóm không thể bỏ — dùng "Giải tán toàn bộ" nếu muốn xóa nhóm.
            </div>
            ...
            <div class="qt-actions" style="gap:8px;">
                <button class="qt-btn" style="background:linear-gradient(135deg,#ef4444,#dc2626);color:white;box-shadow:0 2px 8px rgba(239,68,68,.3);" onclick="_qtDisbandGroup('${sectionKey}')">💥 Giải tán toàn bộ</button>
                <div style="flex:1;"></div>
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn" style="background:linear-gradient(135deg,#f59e0b,#d97706);color:white;box-shadow:0 2px 8px rgba(245,158,11,.3);" onclick="_qtUngroupSelected('${sectionKey}')">🔓 Bỏ ra khỏi nhóm</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
    ...
    const updatePreview = () => {
        const checked = overlay.querySelectorAll('#qtUngroupList .qt-ri-check:checked').length;
        const el = document.getElementById('qtUngroupPreview');
        if (el) el.innerHTML = checked > 0 ? `⚡ Sẽ bỏ <b>${checked} nút</b> — nhóm còn lại <b>${groupKeys.length - checked} nút</b>` : '';
    };
    overlay.querySelectorAll('.qt-ri-check').forEach(cb => cb.addEventListener('change', updatePreview));
}

async function _qtUngroupSelected(sectionKey) {
    const sec = _qtSections.find(s => s.key === sectionKey);
    if (!sec || !sec.section_group) return;
    const groupKeys = _qtGetGroupKeys(sec.section_group);

    const items = document.querySelectorAll('#qtUngroupList .qt-rule-item');
    const toRemove = [];
    items.forEach(item => {
        const cb = item.querySelector('.qt-ri-check');
        if (cb && cb.checked) toRemove.push(item.dataset.key);
    });
    if (toRemove.length === 0) return showToast('❌ Chọn ít nhất 1 nút để bỏ ra!', 'error');
    ...
    // If only leader left, dissolve group entirely
    const remaining = groupKeys.filter(k => !toRemove.includes(k));
    if (remaining.length <= 1 && remaining[0] === sec.key) {
        await apiCall(`/api/consult-types/${sec.key}`, 'PATCH', {
            section_group: null,
            section_group_label: null
        });
    }
    ...
    document.querySelector('.qt-modal-overlay')?.remove();
    showToast(`✅ Đã bỏ ${toRemove.length} nút ra khỏi nhóm!`, 'success');
    await _qtLoadData();
    _qtSwitchTab('rules');
}

async function _qtDisbandGroup(sectionKey) {
    const sec = _qtSections.find(s => s.key === sectionKey);
    if (!sec || !sec.section_group) return;
    const groupKeys = _qtGetGroupKeys(sec.section_group);
    const label = sec.section_group_label || sec.key;

    if (!confirm(`💥 Giải tán nhóm "${label}"?\n\n${groupKeys.length} nút sẽ tách ra thành các loại riêng biệt.\n⚠️ Quy tắc liên kết của các nút thành viên sẽ bị xóa!`)) return;
    ...
    document.querySelector('.qt-modal-overlay')?.remove();
    showToast(`✅ Đã giải tán nhóm "${label}" (${groupKeys.length} nút)!`, 'success');
    await _qtLoadData();
    _qtSwitchTab('rules');
}
