    ...
    function renderDeptBlock(dept, isRoot) {
        ...
        <div class="dept-row ${isRoot ? 'root' : ''}" onclick="affToggleDept(${dept.id})">
            ...
            <div class="dept-stats">
                ...
                ${!_affHideFinancials() ? `<span style="background:#fef3c7;color:#92400e;padding:5px 14px;border-radius:10px;font-size:14px;font-weight:800;">💰 ${affFormatMoney(deptRevenue)}</span>` : ''}
...
async function affShowStatModal(type, page = 1) {
    ...
    try {
        ...
        if (type === 'affiliates') {
            const _hideRevCol = _affHideFinancials();
            tableHead = `<tr><th>#</th><th>Tên Affiliate</th><th>Tài khoản</th><th>NV Quản Lý</th><th>CRM</th>${!_hideRevCol ? '<th>Doanh Số</th>' : ''}<th>Ngày Tạo TK</th><th>Trạng Thái</th></tr>`;
            ...
            data.forEach((r, i) => {
                ...
                tableBody += `<tr>
                    ...
                    ${!_hideRevCol ? `<td style="font-weight:700;color:#059669;">${affFormatMoney(r.total_revenue || 0)}</td>` : ''}
...
async function affOpenTransferModal(empId, empName) {
    ...
    const managerIds = new Set(affiliates.map(a => a.managed_by_user_id).filter(Boolean));
    const affDeptIds = new Set();
    employees.forEach(e => { if (managerIds.has(e.id)) affDeptIds.add(e.department_id); });

    // Get list of active employees — only from departments that manage affiliates
    const AFF_ROLES = ['tkaffiliate', 'hoa_hong', 'ctv', 'nuoi_duong', 'sinh_vien'];
    const staffList = employees.filter(e => e.id !== empId && !AFF_ROLES.includes(e.role) && affDeptIds.has(e.department_id));
