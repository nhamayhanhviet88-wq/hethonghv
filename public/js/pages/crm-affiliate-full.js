...
function _affUpdateDateFilterCounts() {
    const cat = _affActiveCat;
    if (cat !== 'cho_xu_ly' && cat !== 'huy_khach' && cat !== 'xu_ly_tre') return;
    const catCustomers = _affAllCustomers.filter(c => _affGetCategory(c, _affAllStats) === cat);
...
async function _affOpenEditAffiliateFromCrm(userId) {
    try {
        ...
        const bodyHTML = `
            <form id="editAffCrmForm" style="max-width:520px;"><div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;">
