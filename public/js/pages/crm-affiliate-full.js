...
function _affRenderCustomerRow(c, stats, stt) {
    ...
    const isMoiChuyen = _affIsMoiChuyenClientSide(c, [s.lastLog].filter(Boolean)) && !['giam_doc', 'quan_ly_cap_cao', 'quan_ly', 'truong_phong'].includes(currentUser?.role);
...
async function _affSubmitConsultLog(customerId) {
    ...
    if (log_type === 'chot_don') {
        ...
        if (phone && !/^\d{10}$/.test(phone)) {
            showToast('SĐT phải đúng 10 chữ số', 'error'); _affEnableSubmitBtn(); return;
        }
