...
function _affRenderCustomerRow(c, stats, stt) {
    ...
    return `<tr class="${_pinClass}" data-customer-id="${c.id}">
        ...
        <td>${c.readonly ? '<span style="color:var(--gray-400)">' + c.phone + '</span>' : '<a href="tel:' + c.phone + '" style="color:var(--info)">' + c.phone + '</a>'}${c.phone ? '<span onclick="event.stopPropagation();_crmCopyText(\'' + c.phone + '\',this,\'SĐT\')" style="cursor:pointer;font-size:11px;color:#94a3b8;margin-left:4px;transition:color 0.2s;" onmouseover="this.style.color=\'#3b82f6\'" onmouseout="this.style.color=\'#94a3b8\'" title="Copy SĐT">📋</span>' : ''}</td>
    ...
    const bodyHTML = `
        ...
        <div class="form-group" id="consultDepositGroup" style="display:none;">
            ...
            <input type="text" id="consultDepositAmount" class="form-control" placeholder="Nhập số tiền đặt cọc..." 
                ...
                oninput="_affFormatDepositInput(this)">
        </div>
...
async function _affSubmitConsultLog(customerId) {
    ...
    if (log_type === 'dat_coc') {
        ...
        if (depositAmount <= 0) {
            showToast('Vui lòng nhập số tiền đặt cọc!', 'error'); _affEnableSubmitBtn(); return;
        }
        ...
        try {
            ...
            if (data.success) {
                showToast('✅ Đặt cọc thành công!'); closeModal(); window._consultImageBlob = null; loadCrmAffData();
            } else { showToast(data.error || 'Lỗi!', 'error'); _affEnableSubmitBtn(); }
        } catch (err) { showToast('Lỗi kết nối!', 'error'); _affEnableSubmitBtn(); }
        return;
    }
    ...
    if (log_type === 'chot_don') {
        ...
        if (rows.length === 0) { showToast('Vui lòng thêm ít nhất 1 sản phẩm!', 'error'); _affEnableSubmitBtn(); return; }
        const items = [];
        ...
        if (items.length === 0) { showToast('Vui lòng nhập sản phẩm hợp lệ!', 'error'); _affEnableSubmitBtn(); return; }
        ...
        try {
            ...
            const data = await res.json();
            if (data.success) {
                showToast('✅ Chốt đơn thành công! Chuyển sang Sau Bán Hàng.'); closeModal(); window._consultImageBlob = null; loadCrmAffData();
            } else { showToast(data.error || 'Lỗi!', 'error'); _affEnableSubmitBtn(); }
        } catch (err) { showToast('Lỗi kết nối!', 'error'); _affEnableSubmitBtn(); }
        return;
    }
