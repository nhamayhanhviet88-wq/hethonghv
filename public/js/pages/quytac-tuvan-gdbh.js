...
let _qtBAllRules = {};
let _qtBStages = [];
let _qtBSections = [];      // dynamic sections with section_order
let _qtBUnsectioned = [];   // types without a section
let _qtBGroupMembers = [];  // buttons belonging to a group (section_order=0)
let _qtBRulePhases = [];    // dynamic phase groups (PHẦN 1, 2, 3...)
let _qtBIsGD = false;
let _qtBActiveTab = localStorage.getItem('qt_active_tab') || 'buttons';
let _qtBSortDebounce = null;

// ========== MAIN ENTRY ==========
async function renderQuyTacTuVanGdbhPage(container) {
    _qtBIsGD = currentUser && currentUser.role === 'giam_doc';

    container.innerHTML = `
        <div class="qt-page">
            <a class="qt-back" href="/crm-goi-ban-hang" onclick="event.preventDefault();navigate('crm-goi-ban-hang')">← Quay lại CRM Gọi Điện Bán Hàng</a>
            <div class="qt-header">
                <h2 class="qt-header-title">⚙️ Quy Tắc Nút Tư Vấn</h2>
                <span class="qt-header-badge">
                    ${_qtBIsGD ? '🔓 Chế độ chỉnh sửa' : '👁️ Chế độ xem'}
                </span>
            </div>
            <div class="qt-tabs">
                <div class="qt-tab${_qtBActiveTab === 'buttons' ? ' active' : ''}" onclick="_qtBSwitchTab('buttons')" id="qtTabButtons">📋 Danh Sách Nút (0)</div>
                <div class="qt-tab${_qtBActiveTab === 'rules' ? ' active' : ''}" onclick="_qtBSwitchTab('rules')" id="qtTabRules">🔄 Quy Tắc Liên Kết</div>
            </div>
            <div class="qt-panel" id="qtPanel">${_qtBRenderSkeleton()}</div>
        </div>
    `;
...
// ========== DATA LOADING ==========
async function _qtBLoadData() {
    const [typesData, rulesData, stagesData, sectionsData, phasesData] = await Promise.all([
        apiCall('/api/consult-types?crm_menu=goi_ban_hang'),
        apiCall('/api/consult-flow-rules?crm_menu=goi_ban_hang'),
        apiCall('/api/consult-stages'),
        apiCall('/api/consult-sections?crm_menu=goi_ban_hang'),
        apiCall('/api/consult-rule-phases')
    ]);
    _qtBAllTypes = typesData.types || [];
    _qtBAllRules = rulesData.rules || {};
    _qtBStages = stagesData.stages || [];
    _qtBSections = sectionsData.sections || [];
    _qtBUnsectioned = (sectionsData.unsectioned || []).filter(t => !['cap_cuu_sep', 'huy'].includes(t.key));
    _qtBGroupMembers = sectionsData.groupMembers || [];
    _qtBRulePhases = phasesData.phases || [];
    _qtBRulePhases.sort((a, b) => (a.sort_order || 0) - (b.sort_order || 0));
    // Sort stages by sort_order
    _qtBStages.sort((a, b) => (a.sort_order || 0) - (b.sort_order || 0));

    document.getElementById('qtTabButtons').innerHTML = `📋 Danh Sách Nút (${_qtBAllTypes.length})`;
    _qtBRenderActiveTab();
}

// ========== TAB SWITCHING ==========
function _qtBSwitchTab(tab) {
    _qtBActiveTab = tab;
...
function _qtBRenderActiveTab() {
    if (_qtBActiveTab === 'buttons') _qtBRenderButtons();
    else _qtBRenderRules();
}

// ========== TAB 1: BUTTONS (Dynamic Stages) ==========
function _qtBRenderButtons() {
    ...
    if (_qtBIsGD) {
        html += `<div class="qt-action-bar" style="gap:8px;">
            <button class="qt-flow-edit-btn" style="background:linear-gradient(135deg,#10b981,#059669);" onclick="_qtBShowAddStageModal()">📁 Thêm giai đoạn</button>
            <button class="qt-flow-edit-btn" onclick="_qtBShowAddTypeModal()">➕ Thêm nút mới</button>
        </div>`;
    }

    // Render each stage from dynamic config
    for (const stage of _qtBStages) {
        const stageTypes = _qtBAllTypes.filter(t => t.stage === stage.id);
        html += `
            <div class="qt-stage" data-stage-id="${stage.id}">
                <div class="qt-stage-header" style="background:${stage.gradient};">
                    ...
                    ${_qtBIsGD ? `
                        <button onclick="event.stopPropagation();_qtBShowEditStageModal('${stage.id}')" style="background:none;border:none;cursor:pointer;font-size:14px;margin-left:4px;" title="Sửa giai đoạn">✏️</button>
                        <button onclick="event.stopPropagation();_qtBDeleteStage('${stage.id}')" style="background:none;border:none;cursor:pointer;font-size:14px;" title="Xóa giai đoạn">🗑️</button>
                    ` : ''}
                </div>
                <div class="qt-btn-grid" data-stage="${stage.id}">
                    ${stageTypes.map(t => _qtBRenderButtonCard(t)).join('')}
                </div>
            </div>
        `;
    }
    ...
    const unassigned = _qtBAllTypes.filter(t => !t.stage || !_qtBStages.find(s => s.id === t.stage));
        ...
        html += `
            <div class="qt-stage" data-stage-id="_other">
                ...
                <div class="qt-btn-grid" data-stage="_other">
                    ${unassigned.map(t => _qtBRenderButtonCard(t)).join('')}
                </div>
            </div>
        `;
    }
    ...
    if (_qtBIsGD) {
        panel.addEventListener('click', function(e) {
            ...
            if (editBtn) {
                ...
                if (card && card.dataset.key) _qtBShowEditTypeModal(card.dataset.key);
                return;
            }
            ...
            if (delBtn) {
                ...
                if (card && card.dataset.key) _qtBDeleteType(card.dataset.key);
                return;
            }
        }, true);
    }
    ...
    if (_qtBIsGD && typeof Sortable !== 'undefined') {
        document.querySelectorAll('.qt-btn-grid').forEach(grid => {
            new Sortable(grid, {
                ...
                group: 'buttons',
                onEnd: _qtBOnButtonSortEnd
            });
        });
    }
}

function _qtBRenderButtonCard(t) {
    return `
        <div class="qt-btn-card ${t.is_active ? '' : 'inactive'}" data-key="${t.key}" style="--card-accent:${t.color}">
            ${_qtBIsGD ? `<span class="qt-drag-hint">⠿</span>` : ''}
            ${_qtBIsGD ? `<button class="qt-edit-btn" type="button">✏️</button>` : ''}
            ${_qtBIsGD ? `<button class="qt-del-btn" type="button">🗑️</button>` : ''}
...
function _qtBOnButtonSortEnd() {
    clearTimeout(_qtBSortDebounce);
    _qtBSortDebounce = setTimeout(() => {
        const orders = [];
        let globalOrder = 0;
        document.querySelectorAll('.qt-btn-grid').forEach(grid => {
        ...
        _qtBSaveSortOrder(orders);
    }, 500);
}

// ========== STAGE MANAGEMENT ==========
function _qtBShowAddStageModal() {
    const presetsHTML = GDBH_STAGE_PRESETS.map((p, i) => `
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                ...
                <button class="qt-btn qt-btn-primary" onclick="_qtBAddStage()">📁 Thêm giai đoạn</button>
            </div>
        </div>
    `;
...
async function _qtBAddStage() {
    ...
    const preset = GDBH_STAGE_PRESETS[presetIdx];
    ...
    const newStage = {
        ...
        sort_order: _qtBStages.length + 1
    };

    _qtBStages.push(newStage);
    await apiCall('/api/consult-stages', 'PUT', { stages: _qtBStages });
    document.querySelector('.qt-modal-overlay')?.remove();
    showToast('✅ Đã thêm giai đoạn!', 'success');
    _qtBRenderButtons();
}

function _qtBShowEditStageModal(stageId) {
    const stage = _qtBStages.find(s => s.id === stageId);
    if (!stage) return;
    ...
    const currentPresetIdx = GDBH_STAGE_PRESETS.findIndex(p => p.gradient === stage.gradient);
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtBSaveStage('${stageId}')">💾 Lưu</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtBSaveStage(stageId) {
    const stage = _qtBStages.find(s => s.id === stageId);
    if (!stage) return;
...
async function _qtBDeleteStage(stageId) {
    const stage = _qtBStages.find(s => s.id === stageId);
    if (!stage) return;
    const count = _qtBAllTypes.filter(t => t.stage === stageId).length;
    if (!confirm(`Xóa giai đoạn "${stage.title}"?\n${count > 0 ? `⚠️ ${count} nút sẽ chuyển về "Chưa phân loại"` : ''}`)) return;

    // Remove stage from config
    _qtBStages = _qtBStages.filter(s => s.id !== stageId);
    await apiCall('/api/consult-stages', 'PUT', { stages: _qtBStages });

    // Clear stage from buttons
    const orders = _qtBAllTypes.filter(t => t.stage === stageId).map((t, i) => ({ key: t.key, sort_order: 999 + i, stage: null }));
    if (orders.length > 0) {
...
// ========== DELETE TYPE ==========
async function _qtBDeleteType(key) {
    const t = _qtBAllTypes.find(x => x.key === key);
    if (!t) return;
...
// ========== EDIT TYPE MODAL ==========
function _qtBShowEditTypeModal(key) {
    const t = _qtBAllTypes.find(x => x.key === key);
    if (!t) return;

    const stageOptions = _qtBStages.map(s => `<option value="${s.id}" ${t.stage === s.id ? 'selected' : ''}>${s.icon} ${s.title}</option>`).join('');
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtBSaveType('${key}')">💾 Lưu</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtBSaveType(key) {
    const data = {
...
function _qtBShowAddTypeModal() {
    const stageOptions = _qtBStages.map(s => `<option value="${s.id}">${s.icon} ${s.title}</option>`).join('');
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtBAddType()">➕ Thêm</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtBAddType() {
    ...
    await apiCall('/api/consult-types', 'POST', {...data, crm_menu: 'goi_ban_hang'});
    document.querySelector('.qt-modal-overlay')?.remove();
...
function _qtBRenderRules() {
    const panel = document.getElementById('qtPanel');
    let html = _qtBRenderFlowchart();
    ...
    if (_qtBIsGD) {
        const unsecCount = _qtBUnsectioned.length;
        html += `<div class="qt-action-bar" style="gap:8px;">
            <button class="qt-flow-edit-btn" style="background:linear-gradient(135deg,#10b981,#059669);" onclick="_qtBShowAddPhaseModal()">📂 Thêm Phần</button>
            <button class="qt-flow-edit-btn" onclick="_qtBShowAddRuleGroupModal()">➕ Thêm nhóm quy tắc mới ${unsecCount > 0 ? '<span style="background:#ef4444;color:white;padding:1px 7px;border-radius:10px;font-size:10px;margin-left:4px;">' + unsecCount + '</span>' : ''}</button>
        </div>`;
    }

    // ★ DYNAMIC phases from API
    const renderedPhaseIds = new Set();
    for (const phase of _qtBRulePhases) {
        const phaseSections = _qtBSections.filter(s => s.rule_phase === phase.id);
        ...
        html += `
            <div class="qt-section-divider" style="background:${phase.gradient};border-left-color:${phase.color};">
                ...
                ${_qtBIsGD ? `<div style="margin-left:auto;display:flex;gap:6px;">
                    <button onclick="event.stopPropagation();_qtBEditPhase('${phase.id}')" style="background:none;border:none;cursor:pointer;font-size:14px;" title="Sửa phần">✏️</button>
                    <button onclick="event.stopPropagation();_qtBDeletePhase('${phase.id}')" style="background:none;border:none;cursor:pointer;font-size:14px;" title="Xóa phần">🗑️</button>
                </div>` : ''}
            </div>
        `;
        ...
        for (const sec of phaseSections) {
            ...
            html += _qtBRenderSectionAccordion(sec);
        }
    }

    // Sections without a phase
    const unphased = _qtBSections.filter(s => !s.rule_phase || !renderedPhaseIds.has(s.rule_phase));
    ...
    if (firstHeader) _qtBToggleSection(firstHeader);
}

// Render a single section accordion item
function _qtBRenderSectionAccordion(sec) {
    // Check if this is a group leader
    const isGroup = !!sec.section_group;
    const groupKeys = isGroup ? _qtBGetGroupKeys(sec.section_group) : [sec.key];
    const rules = _qtBAllRules[sec.key];
    ...
    if (isGroup && sec.section_group_label) {
        ...
        const memberLabels = groupKeys.map(k => {
            const t = _qtBAllTypes.find(x => x.key === k);
            return t ? `${t.icon} ${t.label}` : k;
        }).join(', ');
        tooltip = `title=\"Gồm: ${memberLabels}\"`;
    } else {
        const tp = _qtBAllTypes.find(x => x.key === sec.key);
    ...
    let html = `
        <div class="qt-flow-section" id="${sectionId}">
            <div class="qt-flow-header" onclick="_qtBToggleSection(this)">
                <div class="qt-flow-title">
                    <span class="qt-loai-badge" style="background:#3b82f6;cursor:${_qtBIsGD ? 'pointer' : 'default'};" ${_qtBIsGD ? `onclick="event.stopPropagation();_qtBEditSectionOrder('${sec.key}',${sec.section_order})" title="Click để đổi STT"` : ''}>Loại ${sec.section_order}</span>
                    ...
                    ${isGroup ? `<span style="font-size:10px;background:#8b5cf6;color:white;padding:1px 6px;border-radius:8px;">🔗 ${groupKeys.length} nút</span>
                        <span class="qt-group-members" style="display:inline-flex;gap:3px;flex-wrap:wrap;margin-left:4px;">${groupKeys.map(k => { const mt = _qtBAllTypes.find(x => x.key === k); return mt ? `<span style="font-size:9px;padding:1px 5px;border-radius:6px;background:${mt.color}20;color:${mt.color};font-weight:700;border:1px solid ${mt.color}30;white-space:nowrap;">${mt.icon} ${mt.label}</span>` : ''; }).join('')}</span>` : ''}
                    <span class="qt-flow-count">${rules.length} nút</span>
                    ${_qtBIsGD ? `<span style="font-size:10px;padding:1px 6px;border-radius:8px;background:${sec.max_appointment_days > 0 ? '#f59e0b20' : '#64748b20'};color:${sec.max_appointment_days > 0 ? '#f59e0b' : '#94a3b8'};cursor:pointer;border:1px solid ${sec.max_appointment_days > 0 ? '#f59e0b40' : '#64748b30'};margin-left:4px;" onclick="event.stopPropagation();_qtBEditMaxDays('${sec.key}',${sec.max_appointment_days || 0})" title="Giới hạn ngày hẹn tối đa">📅 ${sec.max_appointment_days > 0 ? sec.max_appointment_days + ' ngày' : 'Không giới hạn'}</span>` : (sec.max_appointment_days > 0 ? `<span style="font-size:10px;padding:1px 6px;border-radius:8px;background:#f59e0b20;color:#f59e0b;">📅 ${sec.max_appointment_days} ngày</span>` : '')}
                </div>
                <div style="display:flex;align-items:center;gap:8px;">
                    ${_qtBIsGD ? `<button class="qt-flow-edit-btn" onclick="event.stopPropagation();_qtBShowEditRulesModal('${sec.key}')">✏️ Sửa</button><button class="qt-flow-edit-btn" style="background:#ef4444;padding:4px 8px;min-width:0;" onclick="event.stopPropagation();_qtBDeleteSection('${sec.key}')" title="Xóa loại">🗑️</button>` : ''}
    ...
    if (!isWaitingStatus) {
        ...
        for (const r of rulesList) {
            const rtp = _qtBAllTypes.find(x => x.key === r.to_type_key);
...
async function _qtBEditMaxDays(key, currentVal) {
...
function _qtBGetGroupKeys(groupId) {
    if (!groupId) return [];
    const keys = [];
    // Leader (section_order > 0)
    const leader = _qtBSections.find(s => s.section_group === groupId);
    if (leader) keys.push(leader.key);
    // Members (section_order = 0)
    for (const m of _qtBGroupMembers) {
...
// ========== PHASE MANAGEMENT ==========
function _qtBShowAddPhaseModal() {
    const presetsHTML = GDBH_STAGE_PRESETS.map((p, i) => `
    ...
    const nextOrder = _qtBRulePhases.length > 0 ? Math.max(..._qtBRulePhases.map(p => p.sort_order)) + 1 : 1;
        ...
        <div class="qt-modal">
            ...
            <div style="margin-top:8px;">
                ...
                <div id="qtPhaseSectionsList" style="max-height:200px;overflow-y:auto;margin-top:4px;">
                    ${_qtBSections.filter(s => !s.rule_phase).map(s => `
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtBAddPhase()">📂 Thêm Phần</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtBAddPhase() {
    ...
    await apiCall('/api/consult-rule-phases', 'PUT', { phases: _qtBRulePhases });
...
function _qtBEditPhase(phaseId) {
    const phase = _qtBRulePhases.find(p => p.id === phaseId);
    if (!phase) return;

    // Sections in this phase + available sections (no phase or different phase)
    const allSectioned = _qtBSections;
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtBSavePhase('${phaseId}')">💾 Lưu</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtBSavePhase(phaseId) {
    const phase = _qtBRulePhases.find(p => p.id === phaseId);
    if (!phase) return;
    ...
    items.forEach(item => {
        ...
        const sec = _qtBSections.find(s => s.key === key);
...
async function _qtBDeletePhase(phaseId) {
    const phase = _qtBRulePhases.find(p => p.id === phaseId);
    if (!phase) return;
    const count = _qtBSections.filter(s => s.rule_phase === phaseId).length;
    ...
    const updates = _qtBSections.filter(s => s.rule_phase === phaseId).map(s => ({ key: s.key, rule_phase: null }));
    if (updates.length > 0) await apiCall('/api/consult-types/batch/rule-phase', 'PATCH', { updates });

    _qtBRulePhases = _qtBRulePhases.filter(p => p.id !== phaseId);
    await apiCall('/api/consult-rule-phases', 'PUT', { phases: _qtBRulePhases });
...
function _qtBShowAddSectionModal() {
    if (_qtBUnsectioned.length === 0) return showToast('✅ Tất cả nút đã có loại!', 'success');
    const nextOrder = _qtBSections.length > 0 ? Math.max(..._qtBSections.map(s => s.section_order)) + 1 : 1;

    let listHTML = _qtBUnsectioned.map(t => `
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtBAddSections()">📌 Thêm loại</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtBAddSections() {
    ...
    for (const key of selected) {
        ...
        await apiCall(`/api/consult-flow-rules/${key}`, 'PUT', {
            rules: [{ to_type_key: key, is_default: true, delay_days: 0, sort_order: 1 }], crm_menu: 'goi_ban_hang'
        });
        // Set section order
        await apiCall(`/api/consult-types/${key}/section-order`, 'PATCH', { section_order: startOrder, crm_menu: 'goi_ban_hang' });
        startOrder++;
    }

    document.querySelector('.qt-modal-overlay')?.remove();
    showToast(`✅ Đã thêm ${selected.length} loại!`, 'success');
    await apiCall('/api/consult-sections/reindex', 'POST', { crm_menu: 'goi_ban_hang' });
    await _qtBLoadData();
    _qtBSwitchTab('rules');
}

// ========== EDIT SECTION ORDER (click badge) ==========
function _qtBEditSectionOrder(key, currentOrder) {
    const tp = _qtBAllTypes.find(x => x.key === key);
    const label = tp ? `${tp.icon} ${tp.label}` : key;
    const sec = _qtBSections.find(s => s.key === key);
    const currentPhase = sec ? sec.rule_phase : '';

    const phaseOptions = _qtBRulePhases.map(p =>
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtBSaveSectionEdit('${key}',${currentOrder})">💾 Lưu</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtBSaveSectionEdit(key, oldOrder) {
    ...
    if (newOrder !== oldOrder) {
        promises.push(apiCall(`/api/consult-types/${key}/section-order`, 'PATCH', { section_order: newOrder, crm_menu: 'goi_ban_hang' }));
    }
    const sec = _qtBSections.find(s => s.key === key);
    if (!sec || sec.rule_phase !== (newPhase || null)) {
        promises.push(apiCall(`/api/consult-types/${key}/rule-phase`, 'PATCH', { rule_phase: newPhase || null, crm_menu: 'goi_ban_hang' }));
    }
...
async function _qtBDeleteSection(key) {
    const sec = _qtBSections.find(s => s.key === key);
    const isGroup = sec && sec.section_group;
    const groupKeys = isGroup ? _qtBGetGroupKeys(sec.section_group) : [key];
    const tp = _qtBAllTypes.find(x => x.key === key);
    const displayLabel = (isGroup && sec.section_group_label) ? sec.section_group_label : (tp ? `${tp.icon} ${tp.label}` : key);
    const ruleCount = (_qtBAllRules[key] || []).length;
    ...
    try {
        console.log('[_qtBDeleteSection] Deleting key:', key, 'groupKeys:', groupKeys);
        for (const k of groupKeys) {
            console.log('[_qtBDeleteSection] Processing key:', k);
            const r1 = await apiCall(`/api/consult-flow-rules/${k}`, 'DELETE');
            console.log('[_qtBDeleteSection] DELETE flow-rules:', k, r1);
            const r2 = await apiCall(`/api/consult-types/${k}/section-order`, 'PATCH', { section_order: 0 });
            console.log('[_qtBDeleteSection] PATCH section-order:', k, r2);
            const r3 = await apiCall(`/api/consult-types/${k}/rule-phase`, 'PATCH', { rule_phase: null });
            console.log('[_qtBDeleteSection] PATCH rule-phase:', k, r3);
            // Clear group fields
            const r4 = await apiCall(`/api/consult-types/${k}`, 'PATCH', { section_group: null, section_group_label: null });
            console.log('[_qtBDeleteSection] PATCH group fields:', k, r4);
        }
        ...
        console.error('[_qtBDeleteSection] ERROR:', e);
        showToast('❌ Lỗi xóa loại: ' + (e.message || ''), 'error');
    }
}

function _qtBToggleSection(header) {
...
function _qtBRenderFlowchart() {
    // Build journey nodes dynamically from _qtBSections (sorted by section_order)
    ...
    function renderNodeRow(sections) {
        ...
        for (let i = 0; i < sections.length; i++) {
            ...
            const t = _qtBAllTypes.find(x => x.key === sec.key);
            const icon = t ? t.icon : '📋';
            ...
            const isGroup = !!sec.section_group;
            const groupKeys = isGroup ? _qtBGetGroupKeys(sec.section_group) : [];
            ...
            nodesHtml += `
                <div class="qt-fc-node" onclick="_qtBScrollToRule('${sec.key}')" title="${displayName}${isGroup ? ' (nhóm ' + multiCount + ' nút: ' + groupKeys.map(k => { const gt = _qtBAllTypes.find(x => x.key === k); return gt ? gt.label : k; }).join(', ') + ')' : ''}">
...
// ========== EDIT RULES MODAL ==========
function _qtBShowEditRulesModal(fromStatus) {
    const rules = _qtBAllRules[fromStatus] || [];
    const statusLabel = GDBH_FLOW_STATUS_LABELS[fromStatus] || _qtBGetTypeLabel(fromStatus) || fromStatus;

    // ★ Show ALL active types as eligible targets
    const eligibleTypes = _qtBAllTypes.filter(t => {
    ...
    const unsectionedKeys = new Set(_qtBUnsectioned.map(u => u.key));
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtBSaveRules('${fromStatus}')">💾 Lưu quy tắc</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtBSaveRules(fromStatus) {
    ...
    const sec = _qtBSections.find(s => s.key === fromStatus);
    const groupKeys = (sec && sec.section_group) ? _qtBGetGroupKeys(sec.section_group) : [fromStatus];
    for (const key of groupKeys) {
        await apiCall(`/api/consult-flow-rules/${key}`, 'PUT', { rules, crm_menu: 'goi_ban_hang' });
    }
...
function _qtBShowAddRuleGroupModal() {
    // Show unsectioned types (exclude system keys that already have dedicated workflows)
    const SYSTEM_KEYS = ['cap_cuu_sep', 'huy'];
    const sectionKeys = new Set(_qtBSections.map(s => s.key));
    const available = _qtBAllTypes.filter(t => !sectionKeys.has(t.key) && t.is_active && !t.section_group && !SYSTEM_KEYS.includes(t.key));
        ...
        <div class="qt-modal">
            ...
            <div style="margin-top:12px;">
                ...
                <select id="qtNewRulePhase" style="margin-top:4px;">
                    <option value="">(Chưa phân phần)</option>
                    ${_qtBRulePhases.map(p => `<option value="${p.id}">${p.icon} PHẦN ${p.sort_order}: ${p.title}</option>`).join('')}
                </select>
            </div>
            <div style="margin-top:8px;">
                <label style="font-size:13px;font-weight:600;color:#334155;">Bắt đầu từ Loại số:</label>
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtBAddRuleGroup()">➕ Thêm</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtBAddRuleGroup() {
    ...
    if (isGroup) {
        ...
        for (const key of selected) {
            await apiCall(`/api/consult-flow-rules/${key}`, 'PUT', {
                rules: [{ to_type_key: key, is_default: key === leaderKey, delay_days: 0, sort_order: selected.indexOf(key) + 1 }], crm_menu: 'goi_ban_hang'
            });
            // Set group for all
        ...
        await apiCall(`/api/consult-types/${leaderKey}/section-order`, 'PATCH', { section_order: startOrder, crm_menu: 'goi_ban_hang' });
        if (selectedPhase) await apiCall(`/api/consult-types/${leaderKey}/rule-phase`, 'PATCH', { rule_phase: selectedPhase, crm_menu: 'goi_ban_hang' });
        ...
        await apiCall(`/api/consult-types/${key}/section-order`, 'PATCH', { section_order: startOrder, crm_menu: 'goi_ban_hang' });
        if (selectedPhase) await apiCall(`/api/consult-types/${key}/rule-phase`, 'PATCH', { rule_phase: selectedPhase, crm_menu: 'goi_ban_hang' });
    }
    ...
    setTimeout(() => _qtBShowEditRulesModal(selected[0]), 300);
}

// ========== HELPERS ==========
function _qtBGetTypeLabel(key) {
    const t = _qtBAllTypes.find(x => x.key === key);
    return t ? `${t.icon} ${t.label}` : key;
}

async function _qtBSaveSortOrder(orders) {
    try {
        await apiCall('/api/consult-types/batch/sort-order', 'PATCH', { orders });
