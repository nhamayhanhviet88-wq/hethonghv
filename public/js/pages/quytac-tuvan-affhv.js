...
let _qtHAllRules = {};
let _qtHStages = [];
let _qtHSections = [];      // dynamic sections with section_order
let _qtHUnsectioned = [];   // types without a section
let _qtHGroupMembers = [];  // buttons belonging to a group (section_order=0)
let _qtHRulePhases = [];    // dynamic phase groups (PHẦN 1, 2, 3...)
let _qtHIsGD = false;
let _qtHActiveTab = localStorage.getItem('qt_active_tab') || 'buttons';
let _qtHSortDebounce = null;

// ========== MAIN ENTRY ==========
async function renderQuyTacTuVanAffHvPage(container) {
    _qtHIsGD = currentUser && currentUser.role === 'giam_doc';

    container.innerHTML = `
        <div class="qt-page">
            <a class="qt-back" href="/affiliate-hv" onclick="event.preventDefault();navigate('affiliate-hv')">← Quay lại CRM Affiliate Giới Thiệu</a>
            <div class="qt-header">
                <h2 class="qt-header-title">⚙️ Quy Tắc Nút Tư Vấn</h2>
                <span class="qt-header-badge">
                    ${_qtHIsGD ? '🔓 Chế độ chỉnh sửa' : '👁️ Chế độ xem'}
                </span>
            </div>
            <div class="qt-tabs">
                <div class="qt-tab${_qtHActiveTab === 'buttons' ? ' active' : ''}" onclick="_qtHSwitchTab('buttons')" id="qtTabButtons">📋 Danh Sách Nút (0)</div>
                <div class="qt-tab${_qtHActiveTab === 'rules' ? ' active' : ''}" onclick="_qtHSwitchTab('rules')" id="qtTabRules">🔄 Quy Tắc Liên Kết</div>
            </div>
            <div class="qt-panel" id="qtPanel">${_qtHRenderSkeleton()}</div>
        </div>
    `;
...
// ========== DATA LOADING ==========
async function _qtHLoadData() {
    const [typesData, rulesData, stagesData, sectionsData, phasesData] = await Promise.all([
        apiCall('/api/consult-types?crm_menu=affiliate_hv'),
        apiCall('/api/consult-flow-rules?crm_menu=affiliate_hv'),
        apiCall('/api/consult-stages'),
        apiCall('/api/consult-sections?crm_menu=affiliate_hv'),
        apiCall('/api/consult-rule-phases')
    ]);
    _qtHAllTypes = typesData.types || [];
    _qtHAllRules = rulesData.rules || {};
    _qtHStages = stagesData.stages || [];
    _qtHSections = sectionsData.sections || [];
    _qtHUnsectioned = (sectionsData.unsectioned || []).filter(t => !['cap_cuu_sep', 'huy'].includes(t.key));
    _qtHGroupMembers = sectionsData.groupMembers || [];
    _qtHRulePhases = phasesData.phases || [];
    _qtHRulePhases.sort((a, b) => (a.sort_order || 0) - (b.sort_order || 0));
    // Sort stages by sort_order
    _qtHStages.sort((a, b) => (a.sort_order || 0) - (b.sort_order || 0));

    document.getElementById('qtTabButtons').innerHTML = `📋 Danh Sách Nút (${_qtHAllTypes.length})`;
    _qtHRenderActiveTab();
}

// ========== TAB SWITCHING ==========
function _qtHSwitchTab(tab) {
    _qtHActiveTab = tab;
...
function _qtHRenderActiveTab() {
    if (_qtHActiveTab === 'buttons') _qtHRenderButtons();
    else _qtHRenderRules();
}

// ========== TAB 1: BUTTONS (Dynamic Stages) ==========
function _qtHRenderButtons() {
    ...
    if (_qtHIsGD) {
        html += `<div class="qt-action-bar" style="gap:8px;">
            <button class="qt-flow-edit-btn" style="background:linear-gradient(135deg,#10b981,#059669);" onclick="_qtHShowAddStageModal()">📁 Thêm giai đoạn</button>
            <button class="qt-flow-edit-btn" onclick="_qtHShowAddTypeModal()">➕ Thêm nút mới</button>
        </div>`;
    }

    // Render each stage from dynamic config
    for (const stage of _qtHStages) {
        const stageTypes = _qtHAllTypes.filter(t => t.stage === stage.id);
        html += `
            <div class="qt-stage" data-stage-id="${stage.id}">
                <div class="qt-stage-header" style="background:${stage.gradient};">
                    ...
                    ${_qtHIsGD ? `
                        <button onclick="event.stopPropagation();_qtHShowEditStageModal('${stage.id}')" style="background:none;border:none;cursor:pointer;font-size:14px;margin-left:4px;" title="Sửa giai đoạn">✏️</button>
                        <button onclick="event.stopPropagation();_qtHDeleteStage('${stage.id}')" style="background:none;border:none;cursor:pointer;font-size:14px;" title="Xóa giai đoạn">🗑️</button>
                    ` : ''}
                </div>
                <div class="qt-btn-grid" data-stage="${stage.id}">
                    ${stageTypes.map(t => _qtHRenderButtonCard(t)).join('')}
                </div>
            </div>
        `;
    }
    ...
    const unassigned = _qtHAllTypes.filter(t => !t.stage || !_qtHStages.find(s => s.id === t.stage));
        ...
        html += `
            <div class="qt-stage" data-stage-id="_other">
                ...
                <div class="qt-btn-grid" data-stage="_other">
                    ${unassigned.map(t => _qtHRenderButtonCard(t)).join('')}
                </div>
            </div>
        `;
    }
    ...
    if (_qtHIsGD) {
        panel.addEventListener('click', function(e) {
            ...
            if (editBtn) {
                ...
                if (card && card.dataset.key) _qtHShowEditTypeModal(card.dataset.key);
                return;
            }
            ...
            if (delBtn) {
                ...
                if (card && card.dataset.key) _qtHDeleteType(card.dataset.key);
                return;
            }
        }, true);
    }
    ...
    if (_qtHIsGD && typeof Sortable !== 'undefined') {
        document.querySelectorAll('.qt-btn-grid').forEach(grid => {
            new Sortable(grid, {
                ...
                group: 'buttons',
                onEnd: _qtHOnButtonSortEnd
            });
        });
    }
}

function _qtHRenderButtonCard(t) {
    return `
        <div class="qt-btn-card ${t.is_active ? '' : 'inactive'}" data-key="${t.key}" style="--card-accent:${t.color}">
            ${_qtHIsGD ? `<span class="qt-drag-hint">⠿</span>` : ''}
            ${_qtHIsGD ? `<button class="qt-edit-btn" type="button">✏️</button>` : ''}
            ${_qtHIsGD ? `<button class="qt-del-btn" type="button">🗑️</button>` : ''}
...
function _qtHOnButtonSortEnd() {
    clearTimeout(_qtHSortDebounce);
    _qtHSortDebounce = setTimeout(() => {
        const orders = [];
        let globalOrder = 0;
        document.querySelectorAll('.qt-btn-grid').forEach(grid => {
        ...
        _qtHSaveSortOrder(orders);
    }, 500);
}

// ========== STAGE MANAGEMENT ==========
function _qtHShowAddStageModal() {
    const presetsHTML = AHV_STAGE_PRESETS.map((p, i) => `
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                ...
                <button class="qt-btn qt-btn-primary" onclick="_qtHAddStage()">📁 Thêm giai đoạn</button>
            </div>
        </div>
    `;
...
async function _qtHAddStage() {
    ...
    const preset = AHV_STAGE_PRESETS[presetIdx];
    ...
    const newStage = {
        ...
        sort_order: _qtHStages.length + 1
    };

    _qtHStages.push(newStage);
    await apiCall('/api/consult-stages', 'PUT', { stages: _qtHStages });
    document.querySelector('.qt-modal-overlay')?.remove();
    showToast('✅ Đã thêm giai đoạn!', 'success');
    _qtHRenderButtons();
}

function _qtHShowEditStageModal(stageId) {
    const stage = _qtHStages.find(s => s.id === stageId);
    if (!stage) return;
    ...
    const currentPresetIdx = AHV_STAGE_PRESETS.findIndex(p => p.gradient === stage.gradient);
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtHSaveStage('${stageId}')">💾 Lưu</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtHSaveStage(stageId) {
    const stage = _qtHStages.find(s => s.id === stageId);
    if (!stage) return;
...
async function _qtHDeleteStage(stageId) {
    const stage = _qtHStages.find(s => s.id === stageId);
    if (!stage) return;
    const count = _qtHAllTypes.filter(t => t.stage === stageId).length;
    if (!confirm(`Xóa giai đoạn "${stage.title}"?\n${count > 0 ? `⚠️ ${count} nút sẽ chuyển về "Chưa phân loại"` : ''}`)) return;

    // Remove stage from config
    _qtHStages = _qtHStages.filter(s => s.id !== stageId);
    await apiCall('/api/consult-stages', 'PUT', { stages: _qtHStages });

    // Clear stage from buttons
    const orders = _qtHAllTypes.filter(t => t.stage === stageId).map((t, i) => ({ key: t.key, sort_order: 999 + i, stage: null }));
    if (orders.length > 0) {
...
// ========== DELETE TYPE ==========
async function _qtHDeleteType(key) {
    const t = _qtHAllTypes.find(x => x.key === key);
    if (!t) return;
...
// ========== EDIT TYPE MODAL ==========
function _qtHShowEditTypeModal(key) {
    const t = _qtHAllTypes.find(x => x.key === key);
    if (!t) return;

    const stageOptions = _qtHStages.map(s => `<option value="${s.id}" ${t.stage === s.id ? 'selected' : ''}>${s.icon} ${s.title}</option>`).join('');
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtHSaveType('${key}')">💾 Lưu</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtHSaveType(key) {
    const data = {
...
function _qtHShowAddTypeModal() {
    const stageOptions = _qtHStages.map(s => `<option value="${s.id}">${s.icon} ${s.title}</option>`).join('');
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtHAddType()">➕ Thêm</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtHAddType() {
    ...
    await apiCall('/api/consult-types', 'POST', {...data, crm_menu: 'affiliate_hv'});
    document.querySelector('.qt-modal-overlay')?.remove();
...
function _qtHRenderRules() {
    const panel = document.getElementById('qtPanel');
    let html = _qtHRenderFlowchart();
    ...
    if (_qtHIsGD) {
        const unsecCount = _qtHUnsectioned.length;
        html += `<div class="qt-action-bar" style="gap:8px;">
            <button class="qt-flow-edit-btn" style="background:linear-gradient(135deg,#10b981,#059669);" onclick="_qtHShowAddPhaseModal()">📂 Thêm Phần</button>
            <button class="qt-flow-edit-btn" onclick="_qtHShowAddRuleGroupModal()">➕ Thêm nhóm quy tắc mới ${unsecCount > 0 ? '<span style="background:#ef4444;color:white;padding:1px 7px;border-radius:10px;font-size:10px;margin-left:4px;">' + unsecCount + '</span>' : ''}</button>
        </div>`;
    }

    // ★ DYNAMIC phases from API
    const renderedPhaseIds = new Set();
    for (const phase of _qtHRulePhases) {
        const phaseSections = _qtHSections.filter(s => s.rule_phase === phase.id);
        ...
        html += `
            <div class="qt-section-divider" style="background:${phase.gradient};border-left-color:${phase.color};">
                ...
                ${_qtHIsGD ? `<div style="margin-left:auto;display:flex;gap:6px;">
                    <button onclick="event.stopPropagation();_qtHEditPhase('${phase.id}')" style="background:none;border:none;cursor:pointer;font-size:14px;" title="Sửa phần">✏️</button>
                    <button onclick="event.stopPropagation();_qtHDeletePhase('${phase.id}')" style="background:none;border:none;cursor:pointer;font-size:14px;" title="Xóa phần">🗑️</button>
                </div>` : ''}
            </div>
        `;
        ...
        for (const sec of phaseSections) {
            ...
            html += _qtHRenderSectionAccordion(sec);
        }
    }

    // Sections without a phase
    const unphased = _qtHSections.filter(s => !s.rule_phase || !renderedPhaseIds.has(s.rule_phase));
    ...
    if (firstHeader) _qtHToggleSection(firstHeader);
}

// Render a single section accordion item
function _qtHRenderSectionAccordion(sec) {
    // Check if this is a group leader
    const isGroup = !!sec.section_group;
    const groupKeys = isGroup ? _qtHGetGroupKeys(sec.section_group) : [sec.key];
    const rules = _qtHAllRules[sec.key];
    ...
    if (isGroup && sec.section_group_label) {
        ...
        const memberLabels = groupKeys.map(k => {
            const t = _qtHAllTypes.find(x => x.key === k);
            return t ? `${t.icon} ${t.label}` : k;
        }).join(', ');
        tooltip = `title=\"Gồm: ${memberLabels}\"`;
    } else {
        const tp = _qtHAllTypes.find(x => x.key === sec.key);
    ...
    let html = `
        <div class="qt-flow-section" id="${sectionId}">
            <div class="qt-flow-header" onclick="_qtHToggleSection(this)">
                <div class="qt-flow-title">
                    <span class="qt-loai-badge" style="background:#3b82f6;cursor:${_qtHIsGD ? 'pointer' : 'default'};" ${_qtHIsGD ? `onclick="event.stopPropagation();_qtHEditSectionOrder('${sec.key}',${sec.section_order})" title="Click để đổi STT"` : ''}>Loại ${sec.section_order}</span>
                    ...
                    ${isGroup ? `<span style="font-size:10px;background:#8b5cf6;color:white;padding:1px 6px;border-radius:8px;">🔗 ${groupKeys.length} nút</span>
                        <span class="qt-group-members" style="display:inline-flex;gap:3px;flex-wrap:wrap;margin-left:4px;">${groupKeys.map(k => { const mt = _qtHAllTypes.find(x => x.key === k); return mt ? `<span style="font-size:9px;padding:1px 5px;border-radius:6px;background:${mt.color}20;color:${mt.color};font-weight:700;border:1px solid ${mt.color}30;white-space:nowrap;">${mt.icon} ${mt.label}</span>` : ''; }).join('')}</span>` : ''}
                    <span class="qt-flow-count">${rules.length} nút</span>
                    ${_qtHIsGD ? `<span style="font-size:10px;padding:1px 6px;border-radius:8px;background:${sec.max_appointment_days > 0 ? '#f59e0b20' : '#64748b20'};color:${sec.max_appointment_days > 0 ? '#f59e0b' : '#94a3b8'};cursor:pointer;border:1px solid ${sec.max_appointment_days > 0 ? '#f59e0b40' : '#64748b30'};margin-left:4px;" onclick="event.stopPropagation();_qtHEditMaxDays('${sec.key}',${sec.max_appointment_days || 0})" title="Giới hạn ngày hẹn tối đa">📅 ${sec.max_appointment_days > 0 ? sec.max_appointment_days + ' ngày' : 'Không giới hạn'}</span>` : (sec.max_appointment_days > 0 ? `<span style="font-size:10px;padding:1px 6px;border-radius:8px;background:#f59e0b20;color:#f59e0b;">📅 ${sec.max_appointment_days} ngày</span>` : '')}
                </div>
                <div style="display:flex;align-items:center;gap:8px;">
                    ${_qtHIsGD ? `<button class="qt-flow-edit-btn" onclick="event.stopPropagation();_qtHShowEditRulesModal('${sec.key}')">✏️ Sửa</button><button class="qt-flow-edit-btn" style="background:#ef4444;padding:4px 8px;min-width:0;" onclick="event.stopPropagation();_qtHDeleteSection('${sec.key}')" title="Xóa loại">🗑️</button>` : ''}
    ...
    if (!isWaitingStatus) {
        ...
        for (const r of rulesList) {
            const rtp = _qtHAllTypes.find(x => x.key === r.to_type_key);
...
async function _qtHEditMaxDays(key, currentVal) {
...
function _qtHGetGroupKeys(groupId) {
    if (!groupId) return [];
    const keys = [];
    // Leader (section_order > 0)
    const leader = _qtHSections.find(s => s.section_group === groupId);
    if (leader) keys.push(leader.key);
    // Members (section_order = 0)
    for (const m of _qtHGroupMembers) {
...
// ========== PHASE MANAGEMENT ==========
function _qtHShowAddPhaseModal() {
    const presetsHTML = AHV_STAGE_PRESETS.map((p, i) => `
    ...
    const nextOrder = _qtHRulePhases.length > 0 ? Math.max(..._qtHRulePhases.map(p => p.sort_order)) + 1 : 1;
        ...
        <div class="qt-modal">
            ...
            <div style="margin-top:8px;">
                ...
                <div id="qtPhaseSectionsList" style="max-height:200px;overflow-y:auto;margin-top:4px;">
                    ${_qtHSections.filter(s => !s.rule_phase).map(s => `
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtHAddPhase()">📂 Thêm Phần</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtHAddPhase() {
    ...
    await apiCall('/api/consult-rule-phases', 'PUT', { phases: _qtHRulePhases });
...
function _qtHEditPhase(phaseId) {
    const phase = _qtHRulePhases.find(p => p.id === phaseId);
    if (!phase) return;

    // Sections in this phase + available sections (no phase or different phase)
    const allSectioned = _qtHSections;
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtHSavePhase('${phaseId}')">💾 Lưu</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtHSavePhase(phaseId) {
    const phase = _qtHRulePhases.find(p => p.id === phaseId);
    if (!phase) return;
    ...
    items.forEach(item => {
        ...
        const sec = _qtHSections.find(s => s.key === key);
...
async function _qtHDeletePhase(phaseId) {
    const phase = _qtHRulePhases.find(p => p.id === phaseId);
    if (!phase) return;
    const count = _qtHSections.filter(s => s.rule_phase === phaseId).length;
    ...
    const updates = _qtHSections.filter(s => s.rule_phase === phaseId).map(s => ({ key: s.key, rule_phase: null }));
    if (updates.length > 0) await apiCall('/api/consult-types/batch/rule-phase', 'PATCH', { updates });

    _qtHRulePhases = _qtHRulePhases.filter(p => p.id !== phaseId);
    await apiCall('/api/consult-rule-phases', 'PUT', { phases: _qtHRulePhases });
...
function _qtHShowAddSectionModal() {
    if (_qtHUnsectioned.length === 0) return showToast('✅ Tất cả nút đã có loại!', 'success');
    const nextOrder = _qtHSections.length > 0 ? Math.max(..._qtHSections.map(s => s.section_order)) + 1 : 1;

    let listHTML = _qtHUnsectioned.map(t => `
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtHAddSections()">📌 Thêm loại</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtHAddSections() {
    ...
    for (const key of selected) {
        ...
        await apiCall(`/api/consult-flow-rules/${key}`, 'PUT', {
            rules: [{ to_type_key: key, is_default: true, delay_days: 0, sort_order: 1 }], crm_menu: 'affiliate_hv'
        });
        // Set section order
        await apiCall(`/api/consult-types/${key}/section-order`, 'PATCH', { section_order: startOrder, crm_menu: 'affiliate_hv' });
        startOrder++;
    }

    document.querySelector('.qt-modal-overlay')?.remove();
    showToast(`✅ Đã thêm ${selected.length} loại!`, 'success');
    await apiCall('/api/consult-sections/reindex', 'POST', { crm_menu: 'affiliate_hv' });
    await _qtHLoadData();
    _qtHSwitchTab('rules');
}

// ========== EDIT SECTION ORDER (click badge) ==========
function _qtHEditSectionOrder(key, currentOrder) {
    const tp = _qtHAllTypes.find(x => x.key === key);
    const label = tp ? `${tp.icon} ${tp.label}` : key;
    const sec = _qtHSections.find(s => s.key === key);
    const currentPhase = sec ? sec.rule_phase : '';

    const phaseOptions = _qtHRulePhases.map(p =>
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtHSaveSectionEdit('${key}',${currentOrder})">💾 Lưu</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtHSaveSectionEdit(key, oldOrder) {
    ...
    if (newOrder !== oldOrder) {
        promises.push(apiCall(`/api/consult-types/${key}/section-order`, 'PATCH', { section_order: newOrder, crm_menu: 'affiliate_hv' }));
    }
    const sec = _qtHSections.find(s => s.key === key);
    if (!sec || sec.rule_phase !== (newPhase || null)) {
        promises.push(apiCall(`/api/consult-types/${key}/rule-phase`, 'PATCH', { rule_phase: newPhase || null, crm_menu: 'affiliate_hv' }));
    }
...
async function _qtHDeleteSection(key) {
    const sec = _qtHSections.find(s => s.key === key);
    const isGroup = sec && sec.section_group;
    const groupKeys = isGroup ? _qtHGetGroupKeys(sec.section_group) : [key];
    const tp = _qtHAllTypes.find(x => x.key === key);
    const displayLabel = (isGroup && sec.section_group_label) ? sec.section_group_label : (tp ? `${tp.icon} ${tp.label}` : key);
    const ruleCount = (_qtHAllRules[key] || []).length;
    ...
    try {
        console.log('[_qtHDeleteSection] Deleting key:', key, 'groupKeys:', groupKeys);
        for (const k of groupKeys) {
            console.log('[_qtHDeleteSection] Processing key:', k);
            const r1 = await apiCall(`/api/consult-flow-rules/${k}`, 'DELETE');
            console.log('[_qtHDeleteSection] DELETE flow-rules:', k, r1);
            const r2 = await apiCall(`/api/consult-types/${k}/section-order`, 'PATCH', { section_order: 0 });
            console.log('[_qtHDeleteSection] PATCH section-order:', k, r2);
            const r3 = await apiCall(`/api/consult-types/${k}/rule-phase`, 'PATCH', { rule_phase: null });
            console.log('[_qtHDeleteSection] PATCH rule-phase:', k, r3);
            // Clear group fields
            const r4 = await apiCall(`/api/consult-types/${k}`, 'PATCH', { section_group: null, section_group_label: null });
            console.log('[_qtHDeleteSection] PATCH group fields:', k, r4);
        }
        ...
        console.error('[_qtHDeleteSection] ERROR:', e);
        showToast('❌ Lỗi xóa loại: ' + (e.message || ''), 'error');
    }
}

function _qtHToggleSection(header) {
...
function _qtHRenderFlowchart() {
    // Build journey nodes dynamically from _qtHSections (sorted by section_order)
    ...
    function renderNodeRow(sections) {
        ...
        for (let i = 0; i < sections.length; i++) {
            ...
            const t = _qtHAllTypes.find(x => x.key === sec.key);
            const icon = t ? t.icon : '📋';
            ...
            const isGroup = !!sec.section_group;
            const groupKeys = isGroup ? _qtHGetGroupKeys(sec.section_group) : [];
            ...
            nodesHtml += `
                <div class="qt-fc-node" onclick="_qtHScrollToRule('${sec.key}')" title="${displayName}${isGroup ? ' (nhóm ' + multiCount + ' nút: ' + groupKeys.map(k => { const gt = _qtHAllTypes.find(x => x.key === k); return gt ? gt.label : k; }).join(', ') + ')' : ''}">
...
// ========== EDIT RULES MODAL ==========
function _qtHShowEditRulesModal(fromStatus) {
    const rules = _qtHAllRules[fromStatus] || [];
    const statusLabel = AHV_FLOW_STATUS_LABELS[fromStatus] || _qtHGetTypeLabel(fromStatus) || fromStatus;

    // ★ Show ALL active types as eligible targets
    const eligibleTypes = _qtHAllTypes.filter(t => {
    ...
    const unsectionedKeys = new Set(_qtHUnsectioned.map(u => u.key));
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtHSaveRules('${fromStatus}')">💾 Lưu quy tắc</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtHSaveRules(fromStatus) {
    ...
    const sec = _qtHSections.find(s => s.key === fromStatus);
    const groupKeys = (sec && sec.section_group) ? _qtHGetGroupKeys(sec.section_group) : [fromStatus];
    for (const key of groupKeys) {
        await apiCall(`/api/consult-flow-rules/${key}`, 'PUT', { rules, crm_menu: 'affiliate_hv' });
    }
...
function _qtHShowAddRuleGroupModal() {
    // Show unsectioned types (exclude system keys that already have dedicated workflows)
    const SYSTEM_KEYS = ['cap_cuu_sep', 'huy'];
    const sectionKeys = new Set(_qtHSections.map(s => s.key));
    const available = _qtHAllTypes.filter(t => !sectionKeys.has(t.key) && t.is_active && !t.section_group && !SYSTEM_KEYS.includes(t.key));
        ...
        <div class="qt-modal">
            ...
            <div style="margin-top:12px;">
                ...
                <select id="qtNewRulePhase" style="margin-top:4px;">
                    <option value="">(Chưa phân phần)</option>
                    ${_qtHRulePhases.map(p => `<option value="${p.id}">${p.icon} PHẦN ${p.sort_order}: ${p.title}</option>`).join('')}
                </select>
            </div>
            <div style="margin-top:8px;">
                <label style="font-size:13px;font-weight:600;color:#334155;">Bắt đầu từ Loại số:</label>
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtHAddRuleGroup()">➕ Thêm</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtHAddRuleGroup() {
    ...
    if (isGroup) {
        ...
        for (const key of selected) {
            await apiCall(`/api/consult-flow-rules/${key}`, 'PUT', {
                rules: [{ to_type_key: key, is_default: key === leaderKey, delay_days: 0, sort_order: selected.indexOf(key) + 1 }], crm_menu: 'affiliate_hv'
            });
            // Set group for all
        ...
        await apiCall(`/api/consult-types/${leaderKey}/section-order`, 'PATCH', { section_order: startOrder, crm_menu: 'affiliate_hv' });
        if (selectedPhase) await apiCall(`/api/consult-types/${leaderKey}/rule-phase`, 'PATCH', { rule_phase: selectedPhase, crm_menu: 'affiliate_hv' });
        ...
        await apiCall(`/api/consult-types/${key}/section-order`, 'PATCH', { section_order: startOrder, crm_menu: 'affiliate_hv' });
        if (selectedPhase) await apiCall(`/api/consult-types/${key}/rule-phase`, 'PATCH', { rule_phase: selectedPhase, crm_menu: 'affiliate_hv' });
    }
    ...
    setTimeout(() => _qtHShowEditRulesModal(selected[0]), 300);
}

// ========== HELPERS ==========
function _qtHGetTypeLabel(key) {
    const t = _qtHAllTypes.find(x => x.key === key);
    return t ? `${t.icon} ${t.label}` : key;
}

async function _qtHSaveSortOrder(orders) {
    try {
        await apiCall('/api/consult-types/batch/sort-order', 'PATCH', { orders });
