    ...
    if (_hhIsMobile()) {
        ...
        document.getElementById('hhTableBody').innerHTML = pageItems.map((item, i) => {
            ...
            return `<tr style="background:${item.is_self ? '#fffbeb' : (item.is_direct ? '#fefce8' : '#f5f3ff')};">
                <td>${globalIdx + 1}</td>
                <td style="text-align:center;padding:4px 2px;font-size:14px;${window._hhCrmFilter !== 'ctv_hoa_hong' ? 'display:none;' : ''}">${(() => { if ((window._hhAffLockedIds||[]).includes(item.id)) return '<span title="TK Affiliate bị khóa" style="cursor:help;">🔒</span>'; if ((window._hhAffApprovedIds||[]).includes(item.id)) return '<span title="Có TK Affiliate" style="cursor:help;">🔑</span>'; if ((window._hhAffPendingIds||[]).includes(item.id)) return '<span title="Đang chờ duyệt TK" style="cursor:help;">⏳</span>'; if (item.crm_type === 'ctv_hoa_hong') return '<span title="Đã chuyển Affiliate (chưa có TK)" style="cursor:help;color:#7c3aed;font-weight:700;">💎</span>'; return ''; })()}</td>
