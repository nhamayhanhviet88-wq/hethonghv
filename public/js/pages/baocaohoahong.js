                ...
                <div style="overflow-x:auto;-webkit-overflow-scrolling:touch;">
                    <table style="min-width:800px;">
                        <thead>
                            <tr>
                                <th>#</th>
                                <th style="width:30px;text-align:center;padding:4px 2px;${crmFilter !== 'ctv_hoa_hong' ? 'display:none;' : ''}" title="Tài Khoản Affiliate">🔑</th>
                                <th>Tên KH</th>
                                <th>Người GT</th>
                                <th>SĐT</th>
    ...
    if (_hhIsMobile()) {
        ...
        document.getElementById('hhTableBody').innerHTML = pageItems.map((item, i) => {
            ...
            return `<tr style="background:${item.is_self ? '#fffbeb' : (item.is_direct ? '#fefce8' : '#f5f3ff')};">
                <td>${globalIdx + 1}</td>
                <td style="text-align:center;padding:4px 2px;font-size:14px;${crmFilter !== 'ctv_hoa_hong' ? 'display:none;' : ''}">${(() => { if ((window._hhAffLockedIds||[]).includes(item.id)) return '<span title="TK Affiliate bị khóa" style="cursor:help;">🔒</span>'; if ((window._hhAffApprovedIds||[]).includes(item.id)) return '<span title="Có TK Affiliate" style="cursor:help;">🔑</span>'; if ((window._hhAffPendingIds||[]).includes(item.id)) return '<span title="Đang chờ duyệt TK" style="cursor:help;">⏳</span>'; if (item.crm_type === 'ctv_hoa_hong') return '<span title="Đã chuyển Affiliate (chưa có TK)" style="cursor:help;color:#7c3aed;font-weight:700;">💎</span>'; return ''; })()}</td>
                <td><span onclick="hhShowCustomerPopup(${item.id})" style="cursor:pointer;display:inline-flex;align-items:center;background:linear-gradient(135deg,#1e3a5f,#2d5a8e);color:#fad24c;padding:4px 12px;border-radius:16px;font-size:11px;font-weight:700;white-space:nowrap;border:1px solid rgba(212,168,67,0.3);transition:all 0.2s;" onmouseover="this.style.boxShadow='0 2px 8px rgba(212,168,67,0.3)';this.style.borderColor='#fad24c'" onmouseout="this.style.boxShadow='none';this.style.borderColor='rgba(212,168,67,0.3)'">${item.customer_name}</span>${(item.is_silently_frozen && typeof currentUser !== 'undefined' && currentUser.role === 'giam_doc') ? ' <span title="KH Chuyển Affiliate, nền Affiliate Ông không được cập nhật" style="font-size:9px;padding:2px 6px;border-radius:5px;background:#fef2f2;color:#dc2626;font-weight:700;cursor:help;">🔒 KH Chuyển Affiliate</span>' : ''}</td>
