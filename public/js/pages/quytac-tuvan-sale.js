// ========== TRANG QUẢN LÝ QUY TẮC NÚT TƯ VẤN (SALE) — PREMIUM UI v2 ==========

// Status labels for display
const SALE_FLOW_STATUS_LABELS = {
...
let _qtSaleAllRules = {};
let _qtSaleStages = [];
let _qtSaleSections = [];      // dynamic sections with section_order
let _qtSaleUnsectioned = [];   // types without a section
let _qtSaleGroupMembers = [];  // buttons belonging to a group (section_order=0)
let _qtSaleRulePhases = [];    // dynamic phase groups (PHẦN 1, 2, 3...)
let _qtSaleIsGD = false;
let _qtSaleActiveTab = localStorage.getItem('qt_sale_active_tab') || 'buttons';
let _qtSaleSortDebounce = null;
...
async function renderQuyTacTuVanSalePage(container) {
    ...
    container.innerHTML = `
        <div class="qt-page">
            ...
            <div class="qt-tabs">
                <div class="qt-tab${_qtSaleActiveTab === 'buttons' ? ' active' : ''}" onclick="_qtSaleSwitchTab('buttons')" id="qtSaleTabButtons">📋 Danh Sách Nút (0)</div>
                <div class="qt-tab${_qtSaleActiveTab === 'rules' ? ' active' : ''}" onclick="_qtSaleSwitchTab('rules')" id="qtSaleTabRules">🔄 Quy Tắc Liên Kết</div>
            </div>
            <div class="qt-panel" id="qtSalePanel">${_qtSaleRenderSkeleton()}</div>
        </div>
    `;
...
function _qtSaleRenderSkeleton() {
    let cards = '';
...
async function _qtSaleLoadData() {
    ...
    _qtSaleStages.sort((a, b) => (a.sort_order || 0) - (b.sort_order || 0));

    const btnTab = document.getElementById('qtSaleTabButtons');
    if (btnTab) btnTab.innerHTML = `📋 Danh Sách Nút (${_qtSaleAllTypes.length})`;
    _qtSaleRenderActiveTab();
}

// ========== TAB SWITCHING ==========
function _qtSaleSwitchTab(tab) {
    _qtSaleActiveTab = tab;
    localStorage.setItem('qt_sale_active_tab', tab);
    document.querySelectorAll('.qt-tab').forEach(t => t.classList.remove('active'));
    const tabEl = document.getElementById(tab === 'buttons' ? 'qtSaleTabButtons' : 'qtSaleTabRules');
    if (tabEl) tabEl.classList.add('active');
    _qtSaleRenderActiveTab();
}

function _qtSaleRenderActiveTab() {
    if (_qtSaleActiveTab === 'buttons') _qtSaleRenderButtons();
    else _qtSaleRenderRules();
}

// ========== TAB 1: BUTTONS (Dynamic Stages) ==========
function _qtSaleRenderButtons() {
    const panel = document.getElementById('qtSalePanel');
    if (!panel) return;
    let html = '';
    ...
    if (_qtSaleIsGD) {
        html += `<div class="qt-action-bar" style="gap:8px;">
            <button class="qt-flow-edit-btn" style="background:linear-gradient(135deg,#10b981,#059669);" onclick="_qtSaleShowAddStageModal()">📁 Thêm giai đoạn</button>
            <button class="qt-flow-edit-btn" onclick="_qtSaleShowAddTypeModal()">➕ Thêm nút mới</button>
        </div>`;
    }

    // Render each stage from dynamic config
    for (const stage of _qtSaleStages) {
        const stageTypes = _qtSaleAllTypes.filter(t => t.stage === stage.id);
        html += `
            <div class="qt-stage" data-stage-id="${stage.id}">
                <div class="qt-stage-header" style="background:${stage.gradient};">
                    ...
                    ${_qtSaleIsGD ? `
                        <button onclick="event.stopPropagation();_qtSaleShowEditStageModal('${stage.id}')" style="background:none;border:none;cursor:pointer;font-size:14px;margin-left:4px;" title="Sửa giai đoạn">✏️</button>
                        <button onclick="event.stopPropagation();_qtSaleDeleteStage('${stage.id}')" style="background:none;border:none;cursor:pointer;font-size:14px;" title="Xóa giai đoạn">🗑️</button>
                    ` : ''}
                </div>
                <div class="qt-btn-grid" data-stage="${stage.id}">
                    ${stageTypes.map(t => _qtSaleRenderButtonCard(t)).join('')}
                </div>
            </div>
        `;
    }
    ...
    const unassigned = _qtSaleAllTypes.filter(t => !t.stage || !_qtSaleStages.find(s => s.id === t.stage));
        ...
        html += `
            <div class="qt-stage" data-stage-id="_other">
                ...
                <div class="qt-btn-grid" data-stage="_other">
                    ${unassigned.map(t => _qtSaleRenderButtonCard(t)).join('')}
                </div>
            </div>
        `;
    }
    ...
    if (_qtSaleIsGD) {
        panel.addEventListener('click', function(e) {
            ...
            if (editBtn) {
                ...
                if (card && card.dataset.key) _qtSaleShowEditTypeModal(card.dataset.key);
                return;
            }
            ...
            if (delBtn) {
                ...
                if (card && card.dataset.key) _qtSaleDeleteType(card.dataset.key);
                return;
            }
            ...
            if (toggleWrap) {
                ...
                if (card && card.dataset.key) _qtSaleToggleActive(card.dataset.key, card);
                return;
            }
        }, true);
    }
    ...
    if (_qtSaleIsGD && typeof Sortable !== 'undefined') {
        document.querySelectorAll('.qt-btn-grid').forEach(grid => {
            new Sortable(grid, {
                ...
                group: 'buttons',
                onEnd: _qtSaleOnButtonSortEnd
            });
        });
    }
}

function _qtSaleRenderButtonCard(t) {
    return `
        <div class="qt-btn-card ${t.is_active ? '' : 'inactive'}" data-key="${t.key}" data-active="${t.is_active ? '1' : '0'}" style="--card-accent:${t.color}">
            ${_qtSaleIsGD ? `<span class="qt-drag-hint">⠿</span>` : ''}
            ${_qtSaleIsGD ? `<button class="qt-edit-btn" type="button">✏️</button>` : ''}
            ${_qtSaleIsGD ? `<button class="qt-del-btn" type="button">🗑️</button>` : ''}
...
function _qtSaleOnButtonSortEnd() {
    clearTimeout(_qtSaleSortDebounce);
    _qtSaleSortDebounce = setTimeout(() => {
        const orders = [];
        let globalOrder = 0;
        document.querySelectorAll('.qt-btn-grid').forEach(grid => {
        ...
        _qtSaleSaveSortOrder(orders);
    }, 500);
}

// ========== STAGE MANAGEMENT ==========
function _qtSaleShowAddStageModal() {
    const presetsHTML = SALE_STAGE_PRESETS.map((p, i) => `
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                ...
                <button class="qt-btn qt-btn-primary" onclick="_qtSaleAddStage()">📁 Thêm giai đoạn</button>
            </div>
        </div>
    `;
...
async function _qtSaleAddStage() {
    ...
    const preset = SALE_STAGE_PRESETS[presetIdx];
    ...
    const newStage = {
        ...
        sort_order: _qtSaleStages.length + 1
    };

    _qtSaleStages.push(newStage);
    await apiCall('/api/consult-stages', 'PUT', { stages: _qtSaleStages });
    document.querySelector('.qt-modal-overlay')?.remove();
    showToast('✅ Đã thêm giai đoạn!', 'success');
    _qtSaleRenderButtons();
}

function _qtSaleShowEditStageModal(stageId) {
    const stage = _qtSaleStages.find(s => s.id === stageId);
    if (!stage) return;
    ...
    const currentPresetIdx = SALE_STAGE_PRESETS.findIndex(p => p.gradient === stage.gradient);
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtSaleSaveStage('${stageId}')">💾 Lưu</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtSaleSaveStage(stageId) {
    const stage = _qtSaleStages.find(s => s.id === stageId);
    if (!stage) return;
...
async function _qtSaleDeleteStage(stageId) {
    const stage = _qtSaleStages.find(s => s.id === stageId);
    if (!stage) return;
    const count = _qtSaleAllTypes.filter(t => t.stage === stageId).length;
    if (!confirm(`Xóa giai đoạn "${stage.title}"?\n${count > 0 ? `⚠️ ${count} nút sẽ chuyển về "Chưa phân loại"` : ''}`)) return;

    _qtSaleStages = _qtSaleStages.filter(s => s.id !== stageId);
    await apiCall('/api/consult-stages', 'PUT', { stages: _qtSaleStages });
...
// ========== TOGGLE ACTIVE ==========
async function _qtSaleToggleActive(key, cardEl) {
    const t = _qtSaleAllTypes.find(x => x.key === key);
    if (!t) return;
...
async function _qtSaleToggleSection_Active(key, toggleWrapEl) {
    const t = _qtSaleAllTypes.find(x => x.key === key);
    if (!t) return;
    const newActive = !t.is_active;
...
// ========== DELETE TYPE ==========
async function _qtSaleDeleteType(key) {
    const t = _qtSaleAllTypes.find(x => x.key === key);
    if (!t) return;
...
// ========== EDIT TYPE MODAL ==========
function _qtSaleShowEditTypeModal(key) {
    const t = _qtSaleAllTypes.find(x => x.key === key);
    if (!t) return;

    const stageOptions = _qtSaleStages.map(s => `<option value="${s.id}" ${t.stage === s.id ? 'selected' : ''}>${s.icon} ${s.title}</option>`).join('');
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtSaleSaveType('${key}')">💾 Lưu</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
    ...
    const updatePreview = () => {
        ...
        const txtColorEl = document.getElementById('qtEditColorText');
        if (txtColorEl) txtColorEl.value = color;
        const previewEl = document.getElementById('qtEditPreview');
        if (previewEl) {
            previewEl.innerHTML = `
...
async function _qtSaleSaveType(key) {
    const data = {
...
function _qtSaleShowAddTypeModal() {
    const stageOptions = _qtSaleStages.map(s => `<option value="${s.id}">${s.icon} ${s.title}</option>`).join('');
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtSaleAddType()">➕ Thêm</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
function _qtSaleRenderRules() {
    const panel = document.getElementById('qtSalePanel');
    if (!panel) return;
    let html = _qtSaleRenderFlowchart();
    ...
    if (_qtSaleIsGD) {
        const unsecCount = _qtSaleUnsectioned.length;
        html += `<div class="qt-action-bar" style="gap:8px;">
            <button class="qt-flow-edit-btn" style="background:linear-gradient(135deg,#10b981,#059669);" onclick="_qtSaleShowAddPhaseModal()">📂 Thêm Phần</button>
            <button class="qt-flow-edit-btn" onclick="_qtSaleShowAddRuleGroupModal()">➕ Thêm nhóm quy tắc mới ${unsecCount > 0 ? '<span style="background:#ef4444;color:white;padding:1px 7px;border-radius:10px;font-size:10px;margin-left:4px;">' + unsecCount + '</span>' : ''}</button>
        </div>`;
    }

    // ★ DYNAMIC phases from API
    const renderedPhaseIds = new Set();
    for (const phase of _qtSaleRulePhases) {
        const phaseSections = _qtSaleSections.filter(s => s.rule_phase === phase.id);
        ...
        html += `
            <div class="qt-section-divider" style="background:${phase.gradient};border-left-color:${phase.color};">
                ...
                ${_qtSaleIsGD ? `<div style="margin-left:auto;display:flex;gap:6px;">
                    <button onclick="event.stopPropagation();_qtSaleEditPhase('${phase.id}')" style="background:none;border:none;cursor:pointer;font-size:14px;" title="Sửa phần">✏️</button>
                    <button onclick="event.stopPropagation();_qtSaleDeletePhase('${phase.id}')" style="background:none;border:none;cursor:pointer;font-size:14px;" title="Xóa phần">🗑️</button>
                </div>` : ''}
            </div>
        `;
        ...
        for (const sec of phaseSections) {
            ...
            html += _qtSaleRenderSectionAccordion(sec);
        }
    }

    // Sections without a phase
    const unphased = _qtSaleSections.filter(s => !s.rule_phase || !renderedPhaseIds.has(s.rule_phase));
    ...
    if (firstHeader) _qtSaleToggleSection(firstHeader);
}

// Render a single section accordion item
function _qtSaleRenderSectionAccordion(sec) {
    const isGroup = !!sec.section_group;
    const groupKeys = isGroup ? _qtSaleGetGroupKeys(sec.section_group) : [sec.key];
    const rules = _qtSaleAllRules[sec.key];
    ...
    if (isGroup && sec.section_group_label) {
        ...
        const memberLabels = groupKeys.map(k => {
            const t = _qtSaleAllTypes.find(x => x.key === k);
    ...
    let html = `
        <div class="qt-flow-section" id="${sectionId}">
            <div class="qt-flow-header" onclick="_qtSaleToggleSection(this)">
                <div class="qt-flow-title">
                    <span class="qt-loai-badge" style="background:#3b82f6;cursor:${_qtSaleIsGD ? 'pointer' : 'default'};" ${_qtSaleIsGD ? `onclick="event.stopPropagation();_qtSaleEditSectionOrder('${sec.key}',${sec.section_order})" title="Click để đổi STT"` : ''}>Loại ${sec.section_order}</span>
                    ...
                    ${isGroup ? `<span style="font-size:10px;background:#8b5cf6;color:white;padding:1px 6px;border-radius:8px;">🔗 ${groupKeys.length} nút</span>
                        <span class="qt-group-members" style="display:inline-flex;gap:3px;flex-wrap:wrap;margin-left:4px;">${groupKeys.map(k => { const mt = _qtSaleAllTypes.find(x => x.key === k); return mt ? `<span style="font-size:9px;padding:1px 5px;border-radius:6px;background:${mt.color}20;color:${mt.color};font-weight:700;border:1px solid ${mt.color}30;white-space:nowrap;">${mt.icon} ${mt.label}</span>` : ''; }).join('')}</span>` : ''}
                    <span class="qt-flow-count">${rules ? rules.length : 0} nút</span>
                    ${_qtSaleIsGD ? `<span style="font-size:10px;padding:1px 6px;border-radius:8px;background:${sec.max_appointment_days > 0 ? '#f59e0b20' : '#64748b20'};color:${sec.max_appointment_days > 0 ? '#f59e0b' : '#94a3b8'};cursor:pointer;border:1px solid ${sec.max_appointment_days > 0 ? '#f59e0b40' : '#64748b30'};margin-left:4px;" onclick="event.stopPropagation();_qtSaleEditMaxDays('${sec.key}',${sec.max_appointment_days || 0})" title="Giới hạn ngày hẹn tối đa">📅 ${sec.max_appointment_days > 0 ? sec.max_appointment_days + ' ngày' : 'Không giới hạn'}</span>` : (sec.max_appointment_days > 0 ? `<span style="font-size:10px;padding:1px 6px;border-radius:8px;background:#f59e0b20;color:#f59e0b;">📅 ${sec.max_appointment_days} ngày</span>` : '')}
                </div>
                <div style="display:flex;align-items:center;gap:8px;">
                    ${_qtSaleIsGD ? (() => {
                        const secType = _qtSaleAllTypes.find(x => x.key === sec.key);
                        const secActive = secType ? secType.is_active : true;
                        return `<div class="qt-toggle-wrap" style="margin:0;gap:4px;" onclick="event.stopPropagation();_qtSaleToggleSection_Active('${sec.key}',this)" title="${secActive ? 'Click để tắt loại này' : 'Click để bật lại loại này'}">
                        ...
                        </div><button class="qt-flow-edit-btn" onclick="event.stopPropagation();_qtSaleShowEditRulesModal('${sec.key}')">✏️ Sửa</button><button class="qt-flow-edit-btn" style="background:#ef4444;padding:4px 8px;min-width:0;" onclick="event.stopPropagation();_qtSaleDeleteSection('${sec.key}')" title="Xóa loại">🗑️</button><button class="qt-flow-edit-btn" style="background:linear-gradient(135deg,#8b5cf6,#7c3aed);padding:4px 8px;min-width:0;" onclick="event.stopPropagation();_qtSaleShowMergeModal('${sec.key}')" title="Gom thêm nút vào loại này">➕</button>${isGroup ? `<button class="qt-flow-edit-btn" style="background:linear-gradient(135deg,#f59e0b,#d97706);padding:4px 8px;min-width:0;" onclick="event.stopPropagation();_qtSaleShowUngroupModal('${sec.key}')" title="Hủy gom nhóm">🔓</button>` : ''}<button class="qt-flow-edit-btn" style="background:linear-gradient(135deg,#0ea5e9,#0284c7);padding:4px 8px;min-width:0;position:relative;" onclick="event.stopPropagation();_qtSaleShowChangePhaseDropdown('${sec.key}',this)" title="Đổi thuộc Phần">▼</button>`;
                    })() : ''}
    ...
    if (!isWaitingStatus) {
        ...
        for (const r of rulesList) {
            const rtp = _qtSaleAllTypes.find(x => x.key === r.to_type_key);
...
function _qtSaleGetGroupKeys(groupId) {
    if (!groupId) return [];
    const keys = [];
    const leader = _qtSaleSections.find(s => s.section_group === groupId);
    if (leader) keys.push(leader.key);
    for (const m of _qtSaleGroupMembers) {
...
function _qtSaleShowChangePhaseDropdown(key, btnEl) {
...
// ========== PHASE MANAGEMENT ==========
function _qtSaleShowAddPhaseModal() {
    const presetsHTML = SALE_STAGE_PRESETS.map((p, i) => `
        ...
        <div class="qt-modal">
            ...
            <div style="margin-top:8px;">
                ...
                <div id="qtPhaseSectionsList" style="max-height:200px;overflow-y:auto;margin-top:4px;">
                    ${_qtSaleSections.filter(s => !s.rule_phase).map(s => `
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtSaleAddPhase()">📂 Thêm Phần</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtSaleAddPhase() {
    ...
    const nextOrder = _qtSaleRulePhases.length > 0 ? Math.max(..._qtSaleRulePhases.map(p => p.sort_order)) + 1 : 1;

    _qtSaleRulePhases.push({
        id, title, icon: icon || '📋',
...
function _qtSaleEditPhase(phaseId) {
    const phase = _qtSaleRulePhases.find(p => p.id === phaseId);
    if (!phase) return;

    const allSectioned = _qtSaleSections;
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtSaleSavePhase('${phaseId}')">💾 Lưu</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtSaleSavePhase(phaseId) {
    const phase = _qtSaleRulePhases.find(p => p.id === phaseId);
    if (!phase) return;
...
async function _qtSaleDeletePhase(phaseId) {
    const phase = _qtSaleRulePhases.find(p => p.id === phaseId);
    if (!phase) return;
    const count = _qtSaleSections.filter(s => s.rule_phase === phaseId).length;
...
// ========== ADD SECTION ==========
function _qtSaleShowAddSectionModal() {
    if (_qtSaleUnsectioned.length === 0) return showToast('✅ Tất cả nút đã có loại!', 'success');
    const nextOrder = _qtSaleSections.length > 0 ? Math.max(..._qtSaleSections.map(s => s.section_order)) + 1 : 1;

    let listHTML = _qtSaleUnsectioned.map(t => `
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtSaleAddSections()">📌 Thêm loại</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtSaleAddSections() {
...
// ========== EDIT SECTION ORDER ==========
function _qtSaleEditSectionOrder(key, currentOrder) {
    const tp = _qtSaleAllTypes.find(x => x.key === key);
    const label = tp ? `${tp.icon} ${tp.label}` : key;
    const sec = _qtSaleSections.find(s => s.key === key);
    const currentPhase = sec ? sec.rule_phase : '';

    const phaseOptions = _qtSaleRulePhases.map(p =>
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtSaleSaveSectionEdit('${key}',${currentOrder})">💾 Lưu</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtSaleSaveSectionEdit(key, oldOrder) {
...
async function _qtSaleDeleteSection(key) {
    const sec = _qtSaleSections.find(s => s.key === key);
    const isGroup = sec && sec.section_group;
    const groupKeys = isGroup ? _qtSaleGetGroupKeys(sec.section_group) : [key];
    const tp = _qtSaleAllTypes.find(x => x.key === key);
    const displayLabel = (isGroup && sec.section_group_label) ? sec.section_group_label : (tp ? `${tp.icon} ${tp.label}` : key);
    const ruleCount = (_qtSaleAllRules[key] || []).length;
...
function _qtSaleToggleSection(header) {
...
// ========== FLOWCHART ==========
function _qtSaleRenderFlowchart() {
    ...
    for (const sec of _qtSaleSections) {
    ...
    function renderNodeRow(sections) {
        ...
        for (let i = 0; i < sections.length; i++) {
            ...
            const t = _qtSaleAllTypes.find(x => x.key === sec.key);
            const icon = t ? t.icon : '📋';
            const color = t ? t.color : '#6b7280';
            const label = t ? t.label : sec.label || sec.key;
            const isGroup = !!sec.section_group;
            const groupKeys = isGroup ? _qtSaleGetGroupKeys(sec.section_group) : [];
            ...
            nodesHtml += `
                <div class="qt-fc-node" onclick="_qtSaleScrollToRule('${sec.key}')" title="${displayName}${isGroup ? ' (nhóm ' + multiCount + ' nút: ' + groupKeys.map(k => { const gt = _qtSaleAllTypes.find(x => x.key === k); return gt ? gt.label : k; }).join(', ') + ')' : ''}">
...
function _qtSaleScrollToRule(key) {
...
// ========== EDIT RULES MODAL ==========
function _qtSaleShowEditRulesModal(fromStatus) {
    const rules = _qtSaleAllRules[fromStatus] || [];
    const statusLabel = SALE_FLOW_STATUS_LABELS[fromStatus] || _qtSaleGetTypeLabel(fromStatus) || fromStatus;

    const eligibleTypes = _qtSaleAllTypes.filter(t => {
    ...
    const unsectionedKeys = new Set(_qtSaleUnsectioned.map(u => u.key));
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtSaleSaveRules('${fromStatus}')">💾 Lưu quy tắc</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
    ...
    if (typeof Sortable !== 'undefined') {
        new Sortable(ruleList, {
            ...
            ghostClass: 'sortable-ghost',
            chosenClass: 'sortable-chosen',
            onEnd: () => _qtSaleUpdateRuleSTT()
        });
    }
}

function _qtSaleResortRuleList() {
    ...
    _qtSaleUpdateRuleSTT();
}

function _qtSaleUpdateRuleSTT() {
    const list = document.getElementById('qtRuleList');
    if (!list) return;
    let stt = 0;
...
async function _qtSaleSaveRules(fromStatus) {
    const items = document.querySelectorAll('.qt-rule-item');
    const rules = [];
    let order = 0;
    items.forEach(item => {
        const cb = item.querySelector('.qt-ri-check');
...
function _qtSaleShowAddRuleGroupModal() {
        ...
        <div class="qt-modal">
            ...
            <div style="margin-top:12px;">
                ...
                <select id="qtNewRulePhase" style="margin-top:4px;">
                    <option value="">(Chưa phân phần)</option>
                    ${_qtSaleRulePhases.map(p => `<option value="${p.id}">${p.icon} PHẦN ${p.sort_order}: ${p.title}</option>`).join('')}
                </select>
            </div>
            <div style="margin-top:8px;">
                <label style="font-size:13px;font-weight:600;color:#334155;">Bắt đầu từ Loại số:</label>
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtSaleAddRuleGroup()">➕ Thêm</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
    ...
    overlay.querySelectorAll('.qt-ri-check').forEach(cb => {
        cb.addEventListener('change', () => {
            ...
            const row = document.getElementById('qtGroupNameRow');
            if (row) row.style.display = checked >= 2 ? 'block' : 'none';
        });
    });
}

async function _qtSaleAddRuleGroup() {
    ...
    setTimeout(() => _qtSaleShowEditRulesModal(selected[0]), 300);
}

// ========== MERGE INTO EXISTING GROUP ==========
function _qtSaleShowMergeModal(sectionKey) {
    const sec = _qtSaleSections.find(s => s.key === sectionKey);
    if (!sec) return;
    const isGroup = !!sec.section_group;
    const groupKeys = isGroup ? _qtSaleGetGroupKeys(sec.section_group) : [sec.key];
    const tp = _qtSaleAllTypes.find(x => x.key === sec.key);
    ...
    const available = _qtSaleAllTypes.filter(t =>
        t.is_active &&
        !sectionKeys.has(t.key) &&
        !groupMemberKeys.has(t.key) &&
        !groupKeys.includes(t.key) &&
        !SYSTEM_KEYS.includes(t.key)
    );

    if (available.length === 0) {
        return showToast('✅ Không còn nút nào để gom thêm!', 'success');
    }
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            <h3>➕ Gom thêm nút vào: ${currentLabel}</h3>
            <p style="font-size:12px;color:#64748b;margin-bottom:8px;">Hiện tại: <b>${groupKeys.length} nút</b> — Chọn nút để gom thêm vào nhóm này.</p>
            <div style="display:flex;flex-wrap:wrap;gap:4px;margin-bottom:12px;">
                ${groupKeys.map(k => { const mt = _qtSaleAllTypes.find(x => x.key === k); return mt ? `<span style="font-size:10px;padding:2px 8px;border-radius:6px;background:${mt.color}20;color:${mt.color};font-weight:700;border:1px solid ${mt.color}30;">${mt.icon} ${mt.label}</span>` : ''; }).join('')}
            </div>
            <div class="qt-rule-list" id="qtMergeList" style="max-height:280px;overflow-y:auto;">${listHTML}</div>
            ${!isGroup ? `
            <div style="margin-top:12px;padding:10px;background:#f0f9ff;border:2px solid #3b82f6;border-radius:10px;">
                <label style="font-size:13px;font-weight:600;color:#1e40af;">🔗 Tên loại nhóm (bắt buộc):</label>
                <input type="text" id="qtMergeGroupName" placeholder="VD: Tư Vấn Khách" value="${currentLabel}" style="width:100%;padding:8px 12px;font-size:14px;font-weight:600;border:1px solid #93c5fd;border-radius:8px;margin-top:4px;">
            </div>` : ''}
            <div id="qtMergePreview" style="margin-top:8px;font-size:12px;color:#64748b;"></div>
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtSaleMergeIntoGroup('${sectionKey}')">➕ Gom vào nhóm</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
    ...
    const updatePreview = () => {
        const checked = overlay.querySelectorAll('#qtMergeList .qt-ri-check:checked').length;
        const el = document.getElementById('qtMergePreview');
        if (el) el.innerHTML = checked > 0 ? `⚡ Sau khi gom: <b>${groupKeys.length + checked} nút</b>` : '';
    };
    overlay.querySelectorAll('.qt-ri-check').forEach(cb => cb.addEventListener('change', updatePreview));
}

async function _qtSaleMergeIntoGroup(sectionKey) {
    const sec = _qtSaleSections.find(s => s.key === sectionKey);
    if (!sec) return;

    const items = document.querySelectorAll('#qtMergeList .qt-rule-item');
    const selected = [];
    items.forEach(item => {
        if (item.querySelector('.qt-ri-check').checked) selected.push(item.dataset.key);
    });
    ...
    const isGroup = !!sec.section_group;
    let groupId = sec.section_group;
    const leaderKey = sec.key;

    if (!isGroup) {
        const groupName = (document.getElementById('qtMergeGroupName')?.value || '').trim();
        if (!groupName) return showToast('❌ Nhập tên loại nhóm!', 'error');
    ...
    const leaderRules = _qtSaleAllRules[leaderKey] || [];
...
// ========== UNGROUP ==========
function _qtSaleShowUngroupModal(sectionKey) {
    const sec = _qtSaleSections.find(s => s.key === sectionKey);
    ...
    let listHTML = groupKeys.map(k => {
        const mt = _qtSaleAllTypes.find(x => x.key === k); const isLeader = k === sec.key;
    ...
    overlay.innerHTML = `<div class="qt-modal"><h3>🔓 Hủy gom nhóm: ${currentLabel}</h3>
        ...
        <div class="qt-actions" style="gap:8px;"><button class="qt-btn" style="background:linear-gradient(135deg,#ef4444,#dc2626);color:white;box-shadow:0 2px 8px rgba(239,68,68,.3);" onclick="_qtSaleDisbandGroup('${sectionKey}')">💥 Giải tán toàn bộ</button><div style="flex:1;"></div><button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button><button class="qt-btn" style="background:linear-gradient(135deg,#f59e0b,#d97706);color:white;box-shadow:0 2px 8px rgba(245,158,11,.3);" onclick="_qtSaleUngroupSelected('${sectionKey}')">🔓 Bỏ ra khỏi nhóm</button></div></div>`;
    document.body.appendChild(overlay);
...
async function _qtSaleUngroupSelected(sectionKey) {
    const sec = _qtSaleSections.find(s => s.key === sectionKey); if (!sec || !sec.section_group) return;
    const groupKeys = _qtSaleGetGroupKeys(sec.section_group);
...
async function _qtSaleDisbandGroup(sectionKey) {
    const sec = _qtSaleSections.find(s => s.key === sectionKey); if (!sec || !sec.section_group) return;
    const groupKeys = _qtSaleGetGroupKeys(sec.section_group); const label = sec.section_group_label || sec.key;
    if (!confirm(`💥 Giải tán nhóm "${label}"?\n\n${groupKeys.length} nút sẽ tách ra thành các loại riêng biệt.`)) return;
    for (const key of groupKeys) { await apiCall(`/api/consult-types/${key}`, 'PATCH', { section_group: null, section_group_label: null, crm_menu: 'sale' }); if (key !== sec.key) { await apiCall(`/api/consult-types/${key}/section-order`, 'PATCH', { section_order: 0, crm_menu: 'sale' }); } }
    document.querySelector('.qt-modal-overlay')?.remove(); showToast(`✅ Đã giải tán nhóm "${label}"!`, 'success'); await _qtSaleLoadData(); _qtSaleSwitchTab('rules');
}

// ========== HELPERS ==========
function _qtSaleGetTypeLabel(key) {
    const t = _qtSaleAllTypes.find(x => x.key === key);
    return t ? `${t.icon} ${t.label}` : key;
}

async function _qtSaleSaveSortOrder(orders) {
    try {
        await apiCall('/api/consult-types/batch/sort-order', 'PATCH', { orders, crm_menu: 'sale' });
