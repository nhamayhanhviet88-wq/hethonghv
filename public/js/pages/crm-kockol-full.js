...
// ★ V4: Deposit Picker
var _kockolDepositList = [];
async function _kockolFetchDeposits() {
    try { const data = await apiCall('/api/dht/unclaimed-deposits'); _kockolDepositList = data.deposits || []; } catch(e) { _kockolDepositList = []; }
}
function _kockolClearSelectedDeposit() {
...
function _kockolFilterDepositList() {
    var q = (document.getElementById('consultDepositSearch')?.value || '').toLowerCase();
    if (!q) {
        _kockolClearSelectedDeposit();
    }
    var dd = document.getElementById('consultDepositDropdown'); if (!dd) return;
    var filtered = _kockolDepositList.filter(function(d) { return !q || (d.payment_code+' '+(d.transfer_note||'')+' '+d.amount+' '+(d.customer_name||'')).toLowerCase().indexOf(q)>=0; });
...
async function renderCRMKocKolPage(container) {
    ...
    container.innerHTML = `<div style="display:flex;height:calc(100vh - 120px);gap:0;">${sidebarHTML}<div id="crmMainContent" style="flex:1;overflow:auto;padding:16px 20px;">` + `
        ...
        <div style="display:flex; gap:12px; margin-bottom:12px; flex-wrap:wrap; align-items:center;">
            ...
            <select id="crmFilterCustomerType" class="form-control" style="width:auto;min-width:160px;font-weight:600;" onchange="_kockolRenderFilteredTable()">
            ...
            <select id="crmFilterAffStatus" class="form-control" style="width:auto;min-width:180px;" onchange="_kockolRenderFilteredTable()">
...
var _kockolPendingCtvIds = []; // customer IDs with pending CTV requests
var _kockolAffPendingIds = []; // customer IDs with pending affiliate account requests
var _kockolAffApprovedIds = []; // customer IDs that already have affiliate accounts
...
function _kockolRenderFilteredTable() {
    ...
    if (dr.from && dr.to) {
        filtered = filtered.filter(c => {
            let dateField;
            if (_kockolActiveCat === 'cho_xu_ly' || _kockolActiveCat === 'xu_ly_tre') dateField = c.appointment_date;
            else if (_kockolActiveCat === 'huy_khach') dateField = c.cancel_approved_at || c.created_at;
    ...
    if (affFilterVal) {
        if (affFilterVal === 'approved') {
            filtered = filtered.filter(c => _kockolAffApprovedIds.includes(c.id) && !_kockolAffLockedIds.includes(c.id));
        } else if (affFilterVal === 'locked') {
            filtered = filtered.filter(c => _kockolAffLockedIds.includes(c.id));
        } else if (affFilterVal === 'pending') {
            filtered = filtered.filter(c => _kockolAffPendingIds.includes(c.id));
        } else if (affFilterVal === 'none') {
            filtered = filtered.filter(c => !_kockolAffApprovedIds.includes(c.id) && !_kockolAffPendingIds.includes(c.id));
        }
    }
    const _affFilterEl = document.getElementById('crmFilterAffStatus');
    if (_affFilterEl) {
        const _allForAff = _kockolAllCustomers;
        const _appCnt = _allForAff.filter(c => _kockolAffApprovedIds.includes(c.id) && !_kockolAffLockedIds.includes(c.id)).length;
        const _lockCnt = _allForAff.filter(c => _kockolAffLockedIds.includes(c.id)).length;
        const _penCnt = _allForAff.filter(c => _kockolAffPendingIds.includes(c.id)).length;
...
function _kockolRenderCustomerRow(c, stats, stt) {
    ...
    return `<tr class="${_pinClass}" data-customer-id="${c.id}">
        ...
        <td style="font-size:12px;vertical-align:middle;">
            ...
            <div style="font-size:11px;font-weight:700;color:#e65100;cursor:pointer;" onclick="_kockolOpenOrderCodesPopup(${c.id})">${s.latestOrderCode || '—'}</div>
        </td>
        ...
        <td style="font-size:12px;vertical-align:middle;">
            ${(() => {
                ...
                <div style="max-width:180px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:12px;font-weight:700;" title="${_nameTitle}">
                    <span onclick="_kockolOpenCustomerDetail(${c.id})" style="cursor:pointer;color:${_cc.text};background:${_cc.bg};border:1px solid ${_cc.border};padding:3px 10px;border-radius:20px;">
        ...
        <td style="font-size:12px;vertical-align:middle;">
            ...
            ${c.referrer_id ? `
                <div style="font-size:11px;color:#475569;margin-bottom:2px;">
                    Người GT: <span style="cursor:pointer;text-decoration:underline;color:var(--info);font-weight:600;" onclick="event.stopPropagation();_kockolOpenAffiliateDetail(${c.referrer_id})">${c.referrer_name || c.referrer_customer_name}</span>
                </div>
            ` : (currentUser.role === 'giam_doc' ? `<div style="font-size:11px;color:#94a3b8;margin-bottom:2px;cursor:pointer;" onclick="_kockolOpenReferrerSearch(${c.id})">🔍 Tìm Người GT</div>` : '')}
...
async function loadCrmKocKolData() {
    ...
    try {
        ...
        _kockolPendingCtvIds = (pendingData.customers || []).map(c => c.id);
    } catch(e) { _kockolPendingCtvIds = []; }

    // Fetch affiliate account status (pending + approved)
    try {
        const affStatus = await apiCall('/api/affiliate-account/batch-status');
        _kockolAffPendingIds = affStatus.pendingCustomerIds || [];
        _kockolAffApprovedIds = affStatus.approvedCustomerIds || [];
        _kockolAffLockedIds = affStatus.lockedCustomerIds || [];
        _kockolAffApprovedMap = affStatus.approvedMap || {};
    } catch(e) { _kockolAffPendingIds = []; _kockolAffApprovedIds = []; _kockolAffLockedIds = []; _kockolAffApprovedMap = {}; }
...
async function _kockolOpenConsultModal(customerId) {
    ...
    if (frTypes) {
        ...
        else { const order = ['giuc_coc','dat_coc','chot_don','nhan_tin','cap_cuu_sep']; allowedTypes = order.map(k => [k, KOCKOL_CONSULT_TYPES[k]]).filter(([,v]) => v); }
    ...
    const bodyHTML = `
        ...
        <div class="form-group" id="consultDepositGroup" style="display:none;">
            ...
            <input type="text" id="consultDepositSearch" class="form-control" placeholder="🔍 Gõ mã tiền, số tiền, nội dung CK..."
                autocomplete="off" oninput="_kockolFilterDepositList()" onfocus="_kockolFilterDepositList()"
            ...
            <div id="consultDepositSelected" style="display:none;background:#f0fdf4;border:1px solid #059669;border-radius:8px;padding:10px 14px;margin-top:6px;position:relative;">
                ...
                <button type="button" onclick="_kockolClearSelectedDeposit()" style="position:absolute;top:8px;right:8px;background:none;border:none;color:#ef4444;font-size:16px;font-weight:800;cursor:pointer;padding:2px 6px;" title="Xóa chọn">✕</button>
            </div>
...
function _kockolBuildOrderCardHTML(codes, items, customer) {
            ...
            <div style="padding:12px;border:1px solid ${oc.status === 'completed' ? '#10b981' : oc.status === 'cancelled' ? '#ef4444' : hasDHT ? '#3b82f6' : '#e5e7eb'};border-radius:10px;margin-bottom:8px;background:${oc.status === 'completed' ? '#f0fdf4' : oc.status === 'cancelled' ? '#fef2f2' : hasDHT ? '#eff6ff' : '#fafafa'};">
                <div style="display:flex;flex-wrap:wrap;gap:12px;align-items:center;">
                    <div style="min-width:90px;">
                        ...
                        <div style="font-weight:700;color:${hasDHT ? '#e65100' : '#475569'};font-size:15px;${hasDHT ? 'cursor:pointer;text-decoration:underline;' : ''}" ${hasDHT ? `onclick="window._dhtRestoreModalFn = () => _kockolOpenOrderCodesPopup(${customer.id}); _dhtShowDetail('${oc.dht_order_id}')"` : ''}>${oc.order_code}</div>
                    </div>
...
function _kockolFetchSampleOrderCode() {
...
async function _kockolSubmitConsultLog(customerId) {
    ...
    if (log_type === 'gui_mau') {
        ...
        if (!sampleOrderCode || sampleOrderCode.startsWith('Đang tải') || sampleOrderCode.startsWith('Lỗi')) {
            showToast('Không thể tạo mã đơn áo mẫu!', 'error'); _kockolEnableSubmitBtn(); return;
        }
        if (!content) { showToast('Vui lòng nhập nội dung tư vấn!', 'error'); _kockolEnableSubmitBtn(); return; }
        if (!window._consultImageBlob) { showToast('Vui lòng dán hình ảnh (Ctrl+V)!', 'error'); _kockolEnableSubmitBtn(); return; }
        ...
        try {
            ...
            if (data.success) {
                showToast('✅ Gửi mẫu vải/áo thành công!'); closeModal(); window._consultImageBlob = null; loadCrmKocKolData();
    ...
    if (log_type === 'huy_don_tra_coc') {
        ...
        if (!reason) { showToast('Vui lòng nhập lý do hủy đơn trả cọc!', 'error'); _kockolEnableSubmitBtn(); return; }
        if (!window._consultImageBlob) { showToast('Vui lòng chụp/dán hình ảnh minh chứng!', 'error'); _kockolEnableSubmitBtn(); return; }
        try {
            const formData = new FormData();
    ...
    if (log_type === 'dat_coc') {
        ...
        const depositAmount = Number(document.getElementById('consultDepositAmount')?.value) || 0;
        if (!paymentRecordId || depositAmount <= 0) {
            showToast('Vui lòng chọn mã tiền đặt cọc từ danh sách!', 'error'); _kockolEnableSubmitBtn(); return;
        }
        if (!appointment_date && !window._currentConsultCustomerPinned) { showToast('Vui lòng chọn ngày hẹn!', 'error'); _kockolEnableSubmitBtn(); return; }
        ...
        try {
            ...
            const res = await fetch(`/api/customers/${customerId}/consult`, { method: 'POST', body: formData });
            const data = await res.json();
            if (data.success) { showToast('✅ Đặt cọc thành công! Mã tiền đã được khóa.'); closeModal(); window._consultImageBlob = null; loadCrmKocKolData(); }
            else { showToast(data.error || 'Lỗi!', 'error'); _kockolEnableSubmitBtn(); }
        } catch (err) { showToast('Lỗi kết nối!', 'error'); _kockolEnableSubmitBtn(); }
        return;
    }
    ...
    if (log_type === 'chot_don') {
        ...
        try {
            ...
            const data = await res.json();
            if (data.success) {
                showToast('✅ Chốt đơn thành công! Mã đơn đã sẵn sàng tạo ở Đơn Hàng Tổng.'); closeModal(); window._consultImageBlob = null; loadCrmKocKolData();
            } else { showToast(data.error || 'Lỗi!', 'error'); _kockolEnableSubmitBtn(); }
        } catch (err) { showToast('Lỗi kết nối!', 'error'); _kockolEnableSubmitBtn(); }
        return;
    }
...
const KOCKOL_ROLE_LABELS = { giam_doc:'Giám Đốc', quan_ly_cap_cao:'Quản Lý Cấp Cao', quan_ly:'Quản Lý', truong_phong:'Trưởng Phòng', nhan_vien:'Nhân Viên', thu_viec:'Thử Việc', part_time:'Part Time', hoa_hong:'Hoa Hồng', ctv:'CTV', nuoi_duong:'Nuôi Dưỡng', sinh_vien:'Sinh Viên', tkaffiliate:'TK Affiliate' };
                ...
                <div style="padding:6px 24px 20px;">
                    <div style="background:#f8fafc;border-radius:12px;border:1px solid #e2e8f0;overflow:hidden;">
                        <div style="display:grid;grid-template-columns:1fr 1fr;gap:0;">
                            ...
                            <div style="padding:14px 16px;border-bottom:1px solid #e2e8f0;border-right:1px solid #e2e8f0;"><div style="font-size:11px;color:#94a3b8;font-weight:600;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:4px;">💰 Chiết khấu</div><div style="font-size:14px;font-weight:700;color:#059669;">${u.tier_percentage ? u.tier_percentage + '%' : '—'} ${u.tier_name ? '(' + u.tier_name + ')' : ''}</div></div>
                            <div style="padding:14px 16px;border-bottom:1px solid #e2e8f0;"><div style="font-size:11px;color:#94a3b8;font-weight:600;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:4px;">👨‍👦 Affiliate Cha</div><div style="font-size:14px;font-weight:600;color:#1e293b;">${u.source_customer_name || '—'}</div></div>
                            <div style="padding:14px 16px;border-right:1px solid #e2e8f0;"><div style="font-size:11px;color:#94a3b8;font-weight:600;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:4px;">🎂 Ngày sinh</div><div style="font-size:14px;font-weight:600;color:#1e293b;">${u.birth_date ? new Date(u.birth_date).toLocaleDateString('vi-VN') : '—'}</div></div>
                            <div style="padding:14px 16px;"><div style="font-size:11px;color:#94a3b8;font-weight:600;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:4px;">💵 Số dư</div><div style="font-size:14px;font-weight:700;color:#059669;">${u.balance ? formatCurrency(u.balance) : '0đ'}</div></div>
                        </div>
                    </div>
                </div>
            </div>
...
async function _kockolOpenEditAffiliateFromCrm(userId) {
    try {
        ...
        const provinceOptions = KOCKOL_VN_PROVINCES.map(p => `<option value="${p}" ${u.province === p ? 'selected' : ''}>${p}</option>`).join('');
        ...
        openModal(`✏️ Sửa TK Affiliate: ${u.full_name}`, bodyHTML, `<button class="btn" onclick="_kockolSubmitEditAffFromCrm(${u.id})" style="background:var(--gold);color:#122546;padding:8px 24px;border-radius:8px;font-weight:600;">💾 Lưu thay đổi</button>`);
