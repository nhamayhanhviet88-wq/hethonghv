...
let _qtCRulePhases = [];    // dynamic phase groups (PHẦN 1, 2, 3...)
let _qtCIsGD = false;
let _qtCActiveTab = localStorage.getItem('qt_active_tab') || 'buttons';
let _qtCSortDebounce = null;

// ========== MAIN ENTRY ==========
async function renderQuyTacTuVanCtvPage(container) {
        ...
        <div class="qt-page">
            ...
            <div class="qt-header">
                ...
                <span class="qt-header-badge">
                    ${_qtCIsGD ? '🔓 Chế độ chỉnh sửa' : '👁️ Chế độ xem'}
                </span>
            </div>
            <div class="qt-tabs">
                <div class="qt-tab${_qtCActiveTab === 'buttons' ? ' active' : ''}" onclick="_qtCSwitchTab('buttons')" id="qtTabButtons">📋 Danh Sách Nút (0)</div>
                <div class="qt-tab${_qtCActiveTab === 'rules' ? ' active' : ''}" onclick="_qtCSwitchTab('rules')" id="qtTabRules">🔄 Quy Tắc Liên Kết</div>
            </div>
...
async function _qtCLoadData() {
    const [typesData, rulesData, stagesData, sectionsData, phasesData] = await Promise.all([
        apiCall('/api/consult-types?crm_menu=ctv'),
        apiCall('/api/consult-flow-rules?crm_menu=ctv'),
        apiCall('/api/consult-stages'),
        apiCall('/api/consult-sections?crm_menu=ctv'),
    ...
    _qtCUnsectioned = (sectionsData.unsectioned || []).filter(t => !['huy', 'cap_cuu_sep'].includes(t.key));
    ...
    document.getElementById('qtTabButtons').innerHTML = `📋 Danh Sách Nút (${_qtCAllTypes.length})`;
    _qtCRenderActiveTab();
}
...
function _qtCRenderButtons() {
    ...
    if (_qtCIsGD) {
        html += `<div class="qt-action-bar" style="gap:8px;">
            <button class="qt-flow-edit-btn" style="background:linear-gradient(135deg,#10b981,#059669);" onclick="_qtCShowAddStageModal()">📁 Thêm giai đoạn</button>
            <button class="qt-flow-edit-btn" onclick="_qtCShowAddTypeModal()">➕ Thêm nút mới</button>
        </div>`;
    }
        ...
        html += `
            <div class="qt-stage" data-stage-id="${stage.id}">
                <div class="qt-stage-header" style="background:${stage.gradient};">
                    ...
                    ${_qtCIsGD ? `
                        <button onclick="event.stopPropagation();_qtCShowEditStageModal('${stage.id}')" style="background:none;border:none;cursor:pointer;font-size:14px;margin-left:4px;" title="Sửa giai đoạn">✏️</button>
                        <button onclick="event.stopPropagation();_qtCDeleteStage('${stage.id}')" style="background:none;border:none;cursor:pointer;font-size:14px;" title="Xóa giai đoạn">🗑️</button>
                    ` : ''}
                </div>
    ...
    if (_qtCIsGD) {
        panel.addEventListener('click', function(e) {
            ...
            if (toggleWrap) {
                ...
                if (card && card.dataset.key) _qtCToggleActive(card.dataset.key, card);
                return;
            }
        }, true);
    }
...
function _qtCRenderButtonCard(t) {
    return `
        <div class="qt-btn-card ${t.is_active ? '' : 'inactive'}" data-key="${t.key}" data-active="${t.is_active ? '1' : '0'}" style="--card-accent:${t.color}">
            ${_qtCIsGD ? `<span class="qt-drag-hint">⠿</span>` : ''}
            ${_qtCIsGD ? `<button class="qt-edit-btn" type="button">✏️</button>` : ''}
            ${_qtCIsGD ? `<button class="qt-del-btn" type="button">🗑️</button>` : ''}
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                ...
                <button class="qt-btn qt-btn-primary" onclick="_qtCAddStage()">📁 Thêm giai đoạn</button>
            </div>
        </div>
    `;
...
function _qtCShowEditStageModal(stageId) {
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtCSaveStage('${stageId}')">💾 Lưu</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
// ========== TOGGLE ACTIVE ==========
async function _qtCToggleActive(key, cardEl) {
    ...
    try {
        await apiCall(`/api/consult-types/${key}`, 'PUT', {
            ...
            is_active: newActive, stage: t.stage || null, crm_menu: 'ctv'
        });
...
async function _qtCToggleSection_Active(key, toggleWrapEl) {
    const t = _qtCAllTypes.find(x => x.key === key);
    if (!t) return;
    const newActive = !t.is_active;
...
function _qtCShowEditTypeModal(key) {
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtCSaveType('${key}')">💾 Lưu</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
function _qtCShowAddTypeModal() {
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtCAddType()">➕ Thêm</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
function _qtCRenderRules() {
    ...
    if (_qtCIsGD) {
        ...
        html += `<div class="qt-action-bar" style="gap:8px;">
            <button class="qt-flow-edit-btn" style="background:linear-gradient(135deg,#10b981,#059669);" onclick="_qtCShowAddPhaseModal()">📂 Thêm Phần</button>
            <button class="qt-flow-edit-btn" onclick="_qtCShowAddRuleGroupModal()">➕ Thêm nhóm quy tắc mới ${unsecCount > 0 ? '<span style="background:#ef4444;color:white;padding:1px 7px;border-radius:10px;font-size:10px;margin-left:4px;">' + unsecCount + '</span>' : ''}</button>
        </div>`;
    }
        ...
        html += `
            <div class="qt-section-divider" style="background:${phase.gradient};border-left-color:${phase.color};">
                ...
                ${_qtCIsGD ? `<div style="margin-left:auto;display:flex;gap:6px;">
                    <button onclick="event.stopPropagation();_qtCEditPhase('${phase.id}')" style="background:none;border:none;cursor:pointer;font-size:14px;" title="Sửa phần">✏️</button>
                    <button onclick="event.stopPropagation();_qtCDeletePhase('${phase.id}')" style="background:none;border:none;cursor:pointer;font-size:14px;" title="Xóa phần">🗑️</button>
                </div>` : ''}
            </div>
        `;
        ...
        <div class="qt-flow-section" id="${sectionId}">
            <div class="qt-flow-header" onclick="_qtCToggleSection(this)">
                <div class="qt-flow-title">
                    <span class="qt-loai-badge" style="background:#3b82f6;cursor:${_qtCIsGD ? 'pointer' : 'default'};" ${_qtCIsGD ? `onclick="event.stopPropagation();_qtCEditSectionOrder('${sec.key}',${sec.section_order})" title="Click để đổi STT"` : ''}>Loại ${sec.section_order}</span>
                ...
                <div style="display:flex;align-items:center;gap:8px;">
                    ${_qtCIsGD ? (() => {
                        const secType = _qtCAllTypes.find(x => x.key === sec.key);
                        const secActive = secType ? secType.is_active : true;
                        return `<div class="qt-toggle-wrap" style="margin:0;gap:4px;" onclick="event.stopPropagation();_qtCToggleSection_Active('${sec.key}',this)" title="${secActive ? 'Click để tắt loại này' : 'Click để bật lại loại này'}">
...
function _qtCShowChangePhaseDropdown(key, btnEl) {
...
async function _qtCChangePhase(key, phaseId) {
    document.querySelectorAll('.qt-phase-dropdown').forEach(d => d.remove());
    try {
        await apiCall(`/api/consult-types/${key}/rule-phase`, 'PATCH', { rule_phase: phaseId || null, crm_menu: 'ctv' });
...
function _qtCShowAddPhaseModal() {
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtCAddPhase()">📂 Thêm Phần</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
function _qtCEditPhase(phaseId) {
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtCSavePhase('${phaseId}')">💾 Lưu</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
function _qtCShowAddSectionModal() {
    if (_qtCUnsectioned.length === 0) return showToast('✅ Tất cả nút đã có loại!', 'success');
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtCAddSections()">📌 Thêm loại</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
function _qtCEditSectionOrder(key, currentOrder) {
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtCSaveSectionEdit('${key}',${currentOrder})">💾 Lưu</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
function _qtCRenderFlowchart() {
    ...
    function renderNodeRow(sections) {
        ...
        for (let i = 0; i < sections.length; i++) {
            ...
            nodesHtml += `
                <div class="qt-fc-node" onclick="_qtCScrollToRule('${sec.key}')" title="${displayName}${isGroup ? ' (nhóm ' + multiCount + ' nút: ' + groupKeys.map(k => { const gt = _qtCAllTypes.find(x => x.key === k); return gt ? gt.label : k; }).join(', ') + ')' : ''}">
...
function _qtCShowEditRulesModal(fromStatus) {
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtCSaveRules('${fromStatus}')">💾 Lưu quy tắc</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
        ...
        <div class="qt-modal">
            ...
            <div style="margin-top:12px;">
                ...
                <select id="qtNewRulePhase" style="margin-top:4px;">
                    <option value="">(Chưa phân phần)</option>
                    ${_qtCRulePhases.map(p => `<option value="${p.id}">${p.icon} PHẦN ${p.sort_order}: ${p.title}</option>`).join('')}
                </select>
            </div>
            <div style="margin-top:8px;">
                <label style="font-size:13px;font-weight:600;color:#334155;">Bắt đầu từ Loại số:</label>
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtCAddRuleGroup()">➕ Thêm</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
function _qtCShowMergeModal(sectionKey) {
    ...
    // Available: unsectioned + not in any group + active + not system keys
    const SYSTEM_KEYS = ['huy', 'cap_cuu_sep'];
    const sectionKeys = new Set(_qtCSections.map(s => s.key));
    const groupMemberKeys = new Set(_qtCGroupMembers.map(m => m.key));
    const available = _qtCAllTypes.filter(t =>
        t.is_active &&
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div style="display:flex;flex-wrap:wrap;gap:4px;margin-bottom:12px;">
                ${groupKeys.map(k => { const mt = _qtCAllTypes.find(x => x.key === k); return mt ? `<span style="font-size:10px;padding:2px 8px;border-radius:6px;background:${mt.color}20;color:${mt.color};font-weight:700;border:1px solid ${mt.color}30;">${mt.icon} ${mt.label}</span>` : ''; }).join('')}
            </div>
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtCMergeIntoGroup('${sectionKey}')">➕ Gom vào nhóm</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);

    // Live preview of count
    const updatePreview = () => {
...
async function _qtCMergeIntoGroup(sectionKey) {
    const sec = _qtCSections.find(s => s.key === sectionKey);
    if (!sec) return;
    ...
    // Case 1: Section is currently single → convert to group
    if (!isGroup) {
    ...
    // Copy flow rules from leader
    const leaderRules = _qtCAllRules[leaderKey] || [];

    // Add each selected button to the group
    for (const key of selected) {
