...
function _ctvRenderSurchargeCheckboxes() {
    ...
    group.innerHTML = ordered.filter(item => {
        ...
        const isChecked = !!_ctvState.surcharges[item.key];
    ...
    const rowsHtml = renderedItems.map((item, idx) => {
        ...
        return `
            <tr style="background:white;">
                ...
                <td style="border:1px solid #cbd5e1; padding:10px 14px; vertical-align: top;">
                    ${detailsHtml}
                </td>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div style="font-size:12px; max-width:300px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">
                                            ${mats.map(m => `${m.is_best_seller ? '🔥 Bán Chạy - ' : ''}${m.name}: ${Number(m.price).toLocaleString('vi-VN')}đ`).join(', ')}
                                        </div>
                                    </td>
                                    <td>
                                ...
                                ${mats.filter(m => !m.inactive).map(m => {
                                    ...
                                    return `
                                        <div style="display: flex; justify-content: space-between; align-items: center; padding: 8px 12px; background: #f8fafc; border-radius: 10px; border: 1px solid #f1f5f9;">
                                            <span style="font-weight: 700; color: #334155; font-size:12.5px;">${m.is_best_seller ? '🔥 Bán Chạy - ' : ''}${m.name}</span>
