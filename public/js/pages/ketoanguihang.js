...
async function renderKetoanguihangPage(container) {
    ...
    container.innerHTML = `<style>
        ...
        .sh-late-blink {
            animation: shLatePulseBlink 1.2s infinite ease-in-out;
        ...
        .sh-chongay-blink {
            animation: shChongayBlink 1.4s infinite ease-in-out;
        }
    </style>
...
function _shRenderSidebar() {
    ...
    const filters = [
        ...
        { key:'rescheduled', icon:'🟡', label:'Hẹn Lại Lịch Ra Hàng', color:'#d97706', bg:'#fffbeb' },
        { key:'rescheduled_customer', icon:'🟠', label:'Hẹn Lại Gửi Khách', color:'#ea580c', bg:'#fff7ed' },
...
function _shGetOrderMenu(o) {
    ...
    if (isSample) {
        ...
        if (isApproved) {
            ...
            if (reschedDate && reschedDate > today) {
                return { key: 'rescheduled', label: 'Hẹn Lại Lịch Ra Hàng', color: '#d97706', bg: '#fffbeb' };
...
function _shFormatRescheduleStatus(o) {
    ...
    if (effDate) {
        ...
        if (effDate < today) {
            return { label: 'Xử Lý Trễ', color: '#ffffff', bg: '#dc2626', class: 'sh-late-blink' };
        }
    }
    const pendingItems = o.items ? o.items.filter(item => item.shipping_status === 'pending') : [];
    const isEligibleToSend = pendingItems.length > 0 && pendingItems.every(item => item.all_done);
