...
function _cfRenderTable() {
    ...
    _cf.records.forEach(function(r) {
        ...
        if (r.description && r.description.includes('Gửi ship lần')) {
            descCell = '<span class="cf-reship-blink">' + descText + '</span>';
        }
        h += '<td style="max-width:220px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="'+(r.description||'')+'">'+descCell+'</td>';
...
function _cfShowDetail(id) {
    ...
    if (r.description && r.description.includes('Gửi ship lần')) {
        descVal = '<span class="cf-reship-blink">' + descVal + '</span>';
    }
    body += row('📝 Nội dung', descVal);
