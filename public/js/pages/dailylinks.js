        ...
        if (isOpen) {
            _sidebarSortMembers(d.members).forEach(m => {
                ...
                <div onclick="_dlSelUser(${m.id})" class="_dlMemberRow" style="
                    ...
                    <span style="font-size:13px;font-weight:${isSel ? '700' : '500'};color:${isSel ? 'white' : '#334155'};white-space:nowrap;overflow:hidden;text-overflow:ellipsis;display:flex;align-items:center;gap:4px;">${m.full_name}${_sidebarRoleBadge(m.role)}${_dlOverrideUserIds.has(m.id) ? '<span title="Đã tùy chỉnh công việc" style="display:inline-flex;align-items:center;justify-content:center;background:linear-gradient(135deg,#f59e0b,#d97706);color:white;font-size:8px;padding:2px 5px;border-radius:4px;font-weight:800;line-height:1;flex-shrink:0;box-shadow:0 1px 3px rgba(217,119,6,0.3);">✏️ TC</span>' : ''}</span>
                </div>`;
            });
        }
        h += '</div>';
...
const _DL_VIDEO_PLATFORMS = [
    ...
    { key:'instagram',  label:'Instagram',           pattern:'instagram.com', icon:'📷', color:'#E4405F', placeholder:'https://www.instagram.com/reel/... hoặc /username/reel/...',
      validate: v => { const l=v.toLowerCase(); return l.includes('instagram.com') && l.includes('/reel/'); },
      errHint:'Link phải là Instagram Reel (chứa instagram.com và /reel/)' },
    { key:'youtube',    label:'Youtube',             pattern:'youtube.com', icon:'▶️', color:'#FF0000', placeholder:'https://www.youtube.com/watch?v=... hoặc /shorts/...',
      validate: v => { const l=v.toLowerCase(); return l.includes('youtube.com/watch') || l.includes('youtube.com/shorts/'); },
      errHint:'Link phải là video YouTube (youtube.com/watch hoặc youtube.com/shorts/)' },
