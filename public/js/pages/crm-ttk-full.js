// ========== CRM Tự Tìm Kiếm CẦU — 14-column layout with consultation system ==========
...
function _ttkRenderCustomerRow(c, stats, stt) {
    ...
    return `<tr class="${_pinClass}">
        <td style="text-align:center;padding:4px 2px;">
            ${!c.readonly ? `<span class="crm-pin-btn ${c.is_pinned ? 'active' : ''}" onclick="event.stopPropagation();_ttkTogglePin(${c.id})" title="${c.is_pinned ? 'Bỏ pin' : 'Pin khách'}">${c.is_pinned ? '📌' : '<span style="opacity:0.3">📌</span>'}</span>` : ''}
        </td>
        ...
        <td>
            ${!c.readonly ? '<button class="btn btn-sm" onclick="event.stopPropagation();_ttkOpenCustomerInfo(' + c.id + ')" style="font-size:9px;padding:1px 5px;margin-right:4px;background:var(--gray-700);color:var(--gold);" title="Cập nhật thông tin">✏️</button>' : ''}
            ${(() => {
                const _colors = [
...
async function _ttkOpenConsultModal(customerId) {
    ...
    const footerHTML = `
        ...
        <button class="btn btn-primary" id="consultSubmitBtn" onclick="_ttkSubmitConsultLog(${customerId})" style="width:auto;">📝 GHI NHẬN</button>
    `;
...
async function _ttkSubmitConsultLog(customerId) {
    ...
    if (log_type === 'huy') {
        ...
        if (!reason) { showToast('Vui lòng nhập lý do hủy!', 'error'); _ttkEnableSubmitBtn(); return; }
        try {
            ...
            else { showToast(data.error || 'Lỗi!', 'error'); _ttkEnableSubmitBtn(); }
        } catch (err) { showToast('Lỗi kết nối!', 'error'); _ttkEnableSubmitBtn(); }
        return;
    }
    ...
    if (log_type === 'cap_cuu_sep') {
        ...
        if (!content) { showToast('Vui lòng nhập nội dung tình huống!', 'error'); _ttkEnableSubmitBtn(); return; }
        if (!handler_id) { showToast('Vui lòng chọn Sếp xử lý!', 'error'); _ttkEnableSubmitBtn(); return; }
        try {
    ...
    if (log_type === 'dat_coc') {
        ...
        if (depositAmount <= 0) {
            showToast('Vui lòng nhập số tiền đặt cọc!', 'error'); _ttkEnableSubmitBtn(); return;
        }
        ...
        try {
            ...
            if (data.success) {
                showToast('✅ Đặt cọc thành công!'); closeModal(); window._consultImageBlob = null; loadCrmTtkData();
            } else { showToast(data.error || 'Lỗi!', 'error'); _ttkEnableSubmitBtn(); }
        } catch (err) { showToast('Lỗi kết nối!', 'error'); _ttkEnableSubmitBtn(); }
        return;
    }
    ...
    if (log_type === 'chot_don') {
        ...
        if (!address) { showToast('Vui lòng nhập địa chỉ!', 'error'); _ttkEnableSubmitBtn(); return; }
        if (!city) { showToast('Vui lòng chọn thành phố!', 'error'); _ttkEnableSubmitBtn(); return; }
        if (!sbhDate) { showToast('Vui lòng chọn ngày hẹn sau bán hàng!', 'error'); _ttkEnableSubmitBtn(); return; }

        // Phone validate
        if (phone && !/^\d{10}$/.test(phone)) {
            showToast('SĐT phải đúng 10 chữ số', 'error'); _ttkEnableSubmitBtn(); return;
        }

        // Collect order items
        const rows = document.querySelectorAll('#consultOrderTable tbody tr');
        if (rows.length === 0) { showToast('Vui lòng thêm ít nhất 1 sản phẩm!', 'error'); _ttkEnableSubmitBtn(); return; }
        const items = [];
        ...
        if (items.length === 0) { showToast('Vui lòng nhập sản phẩm hợp lệ!', 'error'); _ttkEnableSubmitBtn(); return; }
        ...
        try {
            ...
            const data = await res.json();
            if (data.success) {
                showToast('✅ Chốt đơn thành công! Chuyển sang Sau Bán Hàng.'); closeModal(); window._consultImageBlob = null; loadCrmTtkData();
            } else { showToast(data.error || 'Lỗi!', 'error'); _ttkEnableSubmitBtn(); }
        } catch (err) { showToast('Lỗi kết nối!', 'error'); _ttkEnableSubmitBtn(); }
        return;
    }
    ...
    if (!content) { showToast('Vui lòng nhập nội dung tư vấn!', 'error'); _ttkEnableSubmitBtn(); return; }
...
async function _ttkOpenConsultHistory(customerId) {
    ...
    let bodyHTML = `
        <div style="display:flex;gap:8px;margin-bottom:16px;">
            ...
            <button class="btn btn-sm tab-btn" onclick="_ttkSwitchConsultTab('history', this)" style="font-size:12px;">📜 Lịch Sử (${logs.length})</button>
            <button class="btn btn-sm tab-btn" onclick="_ttkSwitchConsultTab('order', this)" style="font-size:12px;">📦 Đơn Hàng</button>
        </div>
        ...
        <div id="tabInfo">
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;font-size:13px;">
                ...
                ${(c.referrer_name || c.referrer_customer_name) ? `<div><strong>Người GT:</strong> <span style="cursor:pointer;text-decoration:underline;color:var(--info);" onclick="_ttkOpenAffiliateDetail(${c.referrer_id})">${c.referrer_name || c.referrer_customer_name}</span></div>` : ''}
    ...
    const footerHTML = `
        ...
        <button class="btn btn-primary" onclick="_ttkOpenConsultModal(${c.id})" style="width:auto;">📝 Tư Vấn</button>
        <button class="btn btn-primary" onclick="_ttkSaveOrderItems(${c.id})" style="width:auto;">💾 Lưu Đơn</button>
        <button class="btn" onclick="_ttkRequestCancel(${c.id})" style="width:auto;background:var(--danger);color:white;">❌ Hủy KH</button>
    `;
...
async function _ttkOpenReferrerSearch(customerId) {
    const bodyHTML = `
        <div class="form-group">
            ...
            <input type="text" id="referrerSearchInput" class="form-control" placeholder="Nhập tên hoặc SĐT để lọc..." oninput="_ttkFilterReferrerList(${customerId})">
        </div>
...
async function _ttkOpenCustomerInfo(customerId) {
    ...
    const bodyHTML = `
        ...
        <div class="form-group">
            ...
            <button class="btn btn-sm" onclick="_ttkAddHolidayRow()" style="font-size:12px;margin-top:6px;">➕ Thêm ngày lễ</button>
        </div>
    `;

    const footerHTML = `
        <button class="btn btn-secondary" onclick="closeModal()">Hủy</button>
        <button class="btn btn-primary" onclick="_ttkSaveCustomerInfo(${customerId})" style="width:auto;">💾 LƯU</button>
    `;
...
const TTK_CRM_LABELS_AFF = { nhu_cau: 'Chăm Sóc KH Nhu Cầu', ctv: 'Chăm Sóc CTV', tu_tim_kiem: 'CRM Tự Tìm Kiếm', goi_hop_tac: 'CRM Gọi Điện Hợp Tác', goi_ban_hang: 'CRM Gọi Điện Bán Hàng', koc_tiktok: 'CRM KOL/KOC Tiktok' };
const TTK_ROLE_LABELS_AFF = { giam_doc:'Giám Đốc', quan_ly_cap_cao:'Quản Lý Cấp Cao', quan_ly:'Quản Lý', truong_phong:'Trưởng Phòng', nhan_vien:'Nhân Viên', part_time:'Part Time', hoa_hong:'Hoa Hồng', ctv:'CTV', nuoi_duong:'Nuôi Dưỡng', sinh_vien:'Sinh Viên', tkaffiliate:'TK Affiliate' };
...
async function _ttkOpenEditAffiliateFromCrm(userId) {
    try {
        ...
        const footerHTML = `<button class="btn" onclick="_ttkSubmitEditAffFromCrm(${u.id})" style="background:var(--gold);color:#122546;padding:8px 24px;border-radius:8px;font-weight:600;">💾 Lưu thay đổi</button>`;
