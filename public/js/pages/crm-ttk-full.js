// ========== CRM T? Tìm Ki?m C?U — 14-column layout with consultation system ==========

// Format deposit input with thousand separators (500000 ? 500.000)
function _ttkFormatDepositInput(el) {
...
function _ttkGetBirthdayDisplay(birthdayStr) {
...
function _ttkIsBirthdayToday(birthdayStr) {
...
// Merge dynamic types from consult_type_configs API into TTK_CONSULT_TYPES
async function _ttkSyncConsultTypes() {
    try {
        const data = await apiCall('/api/consult-types?crm_menu=tu_tim_kiem');
        if (data.types && Array.isArray(data.types)) {
            for (const t of data.types) {
                if (!t.key || !t.is_active) continue;
                // Add or update (API types override defaults)
                TTK_CONSULT_TYPES[t.key] = {
                    label: t.label || t.key,
                    icon: t.icon || '??',
...
async function renderCRMTtkPage(container) {
    let topStaffOptions = '';
    ...
    container.innerHTML = `
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;">
            <div></div>
            <a href="/quytacnuttuvancrmttk" onclick="event.preventDefault();navigate('quytacnuttuvancrmttk')"
        ...
        <div class="crm-stat-cards" id="crmStatCards">
            <div class="crm-stat-card" data-cat="phai_xu_ly" style="background:linear-gradient(135deg,#ef4444,#dc2626);color:white;" onclick="_ttkFilterByCat('phai_xu_ly')">
                <div class="stat-icon">??</div>
                <div class="stat-count" id="crmStatPhaiXuLy">0</div>
                <div class="stat-label">Ph?i x? lý hôm nay</div>
            </div>
            <div class="crm-stat-card" data-cat="da_xu_ly" style="background:linear-gradient(135deg,#10b981,#059669);color:white;" onclick="_ttkFilterByCat('da_xu_ly')">
                <div class="stat-icon">?</div>
                <div class="stat-count" id="crmStatDaXuLy">0</div>
                <div class="stat-label">Ðã x? lý hôm nay</div>
            </div>
            <div class="crm-stat-card" data-cat="xu_ly_tre" style="background:linear-gradient(135deg,#8b5cf6,#7c3aed);color:white;" onclick="_ttkFilterByCat('xu_ly_tre')">
                <div class="stat-icon">??</div>
                <div class="stat-count" id="crmStatXuLyTre">0</div>
                <div class="stat-label">Khách x? lý tr?</div>
            </div>
            <div class="crm-stat-card" data-cat="cho_xu_ly" style="background:linear-gradient(135deg,#f59e0b,#d97706);color:white;" onclick="_ttkFilterByCat('cho_xu_ly')">
                <div class="stat-icon">?</div>
                <div class="stat-count" id="crmStatChoXuLy">0</div>
                <div class="stat-label">Ch? x? lý</div>
            </div>
            <div class="crm-stat-card" data-cat="huy_khach" style="background:linear-gradient(135deg,#6b7280,#4b5563);color:white;" onclick="_ttkFilterByCat('huy_khach')">
                <div class="stat-icon">??</div>
        ...
        <div class="crm-date-filter" id="crmDateFilter">
            ...
            <label>Ngày</label>
            <select id="crmDateDay" onchange="_ttkUpdateDateFilterCounts();_ttkRenderFilteredTable()">
                <option value="">T?t C?</option>
                ${(() => { let o = ''; for (let d = 1; d <= 31; d++) o += '<option value="' + d + '">Ngày ' + d + '</option>'; return o; })()}
            </select>
            <label>Tháng</label>
            <select id="crmDateMonth" onchange="_ttkUpdateDateFilterCounts();_ttkRenderFilteredTable()">
                <option value="" selected>T?t C?</option>
                ${(() => { let o = ''; for (let m = 1; m <= 12; m++) o += '<option value="' + m + '">Tháng ' + m + '</option>'; return o; })()}
            </select>
            <label>Nam</label>
            <select id="crmDateYear" onchange="_ttkUpdateDateFilterCounts();_ttkRenderFilteredTable()">
        ...
        <div style="display:flex; gap:12px; margin-bottom:12px; flex-wrap:wrap; align-items:center;">
            <select id="crmFilterConsultType" class="form-control" style="width:auto;min-width:200px;" onchange="_ttkRenderFilteredTable()">
    ...
    document.getElementById('crmFilterConsultType').addEventListener('change', () => _ttkRenderFilteredTable());
    const topStaffEl = document.getElementById('crmTopStaffFilter');
    if (topStaffEl) topStaffEl.addEventListener('change', () => loadCrmTtkData());
    let st;
    document.getElementById('crmSearch').addEventListener('input', () => { clearTimeout(st); st = setTimeout(loadCrmTtkData, 400); });

    await loadCrmTtkData();

    // Auto-select 'Ph?i x? lý hôm nay' on page load
    _ttkActiveCat = null;
    _ttkFilterByCat('phai_xu_ly');
}

var _ttkActiveCat = null; // null = all, or 'phai_xu_ly'|'moi_chuyen'|'da_xu_ly'|'cho_xu_ly'|'huy_khach'
var _ttkAllCustomers = []; // full list for re-filtering
var _ttkAllStats = {}; // consult stats
var _ttkCurrentPage = 1;
var _ttkPageSize = 50;

function _ttkFilterByCat(cat) {
    if (_ttkActiveCat === cat) { _ttkActiveCat = null; } else { _ttkActiveCat = cat; }
    _ttkCurrentPage = 1; // reset page on category change
    ...
    if (_ttkActiveCat) {
        const el = document.querySelector('.crm-stat-card[data-cat="' + _ttkActiveCat + '"]');
    ...
    _ttkUpdateConsultTypeDropdown();
    _ttkRenderFilteredTable();
}

function _ttkUpdateDateFilterCounts() {
    const cat = _ttkActiveCat;
    if (cat !== 'cho_xu_ly' && cat !== 'huy_khach' && cat !== 'xu_ly_tre') return;
    const catCustomers = _ttkAllCustomers.filter(c => _ttkGetCategory(c, _ttkAllStats) === cat);
...
function _ttkGetCategory(c, stats) {
    ...
    const isBirthdayToday = _ttkIsBirthdayToday(c.birthday);
...
function _ttkUpdateConsultTypeDropdown(filteredList) {
    ...
    if (!custs) {
        custs = _ttkAllCustomers;
        if (_ttkActiveCat) {
            custs = _ttkAllCustomers.filter(c => _ttkGetCategory(c, _ttkAllStats) === _ttkActiveCat);
        }
    }
...
function _ttkRenderFilteredTable() {
    const customers = _ttkAllCustomers;
    const stats = _ttkAllStats;
    const tbody = document.getElementById('crmCtvTbody');
    
    let filtered = customers;
    if (_ttkActiveCat) {
        if (_ttkActiveCat === 'phai_xu_ly') {
            // Include both phai_xu_ly and moi_chuyen
            filtered = customers.filter(c => {
                const cat = _ttkGetCategory(c, stats);
                return cat === 'phai_xu_ly' || cat === 'moi_chuyen';
            });
        } else {
            filtered = customers.filter(c => _ttkGetCategory(c, stats) === _ttkActiveCat);
        }
    }

    // Apply date filter for cho_xu_ly and huy_khach (BEFORE consult type filter)
    const isDateCat = (_ttkActiveCat === 'cho_xu_ly' || _ttkActiveCat === 'huy_khach' || _ttkActiveCat === 'xu_ly_tre');
    if (isDateCat) {
        ...
        if (hasMonth || hasDay) {
            filtered = filtered.filter(c => {
                let dateField;
                if (_ttkActiveCat === 'cho_xu_ly' || _ttkActiveCat === 'xu_ly_tre') {
    ...
    _ttkUpdateConsultTypeDropdown(filtered);
    ...
    // Pagination
    const totalPages = Math.ceil(filtered.length / _ttkPageSize);
    if (_ttkCurrentPage > totalPages) _ttkCurrentPage = totalPages;
    const startIdx = (_ttkCurrentPage - 1) * _ttkPageSize;
    const paged = filtered.slice(startIdx, startIdx + _ttkPageSize);

    // Split table into two sections for phai_xu_ly
    if (_ttkActiveCat === 'phai_xu_ly') {
        const moiChuyenRows = paged.filter(c => _ttkGetCategory(c, stats) === 'moi_chuyen');
        const phaiXuLyRows = paged.filter(c => _ttkGetCategory(c, stats) === 'phai_xu_ly');
        let html = '';
        let stt = startIdx + 1;
        if (moiChuyenRows.length > 0) {
            ...
            html += moiChuyenRows.map(c => _ttkRenderCustomerRow(c, stats, stt++)).join('');
        }
        if (phaiXuLyRows.length > 0) {
            ...
            html += phaiXuLyRows.map(c => _ttkRenderCustomerRow(c, stats, stt++)).join('');
        }
        tbody.innerHTML = html;
    } else {
        tbody.innerHTML = paged.map((c, idx) => _ttkRenderCustomerRow(c, stats, startIdx + idx + 1)).join('');
    }

    // Render pagination
    const pgEl = document.getElementById('crmPagination');
    if (totalPages <= 1) { pgEl.innerHTML = ''; return; }
    let pgHtml = '<button ' + (_ttkCurrentPage <= 1 ? 'disabled' : '') + ' onclick="_ttkGoToPage(' + (_ttkCurrentPage - 1) + ')">?</button>';
    for (let p = 1; p <= totalPages; p++) {
        pgHtml += '<button class="' + (p === _ttkCurrentPage ? 'active' : '') + '" onclick="_ttkGoToPage(' + p + ')">' + p + '</button>';
    }
    pgHtml += '<button ' + (_ttkCurrentPage >= totalPages ? 'disabled' : '') + ' onclick="_ttkGoToPage(' + (_ttkCurrentPage + 1) + ')">?</button>';
    pgHtml += '<span class="pg-info">' + (startIdx+1) + '–' + Math.min(startIdx + _ttkPageSize, filtered.length) + ' / ' + filtered.length + '</span>';
    pgEl.innerHTML = pgHtml;
}

function _ttkGoToPage(page) {
    _ttkCurrentPage = page;
    _ttkRenderFilteredTable();
    // Scroll to table top
...
function _ttkRenderCustomerRow(c, stats, stt) {
    ...
    let lastType = s.lastLog ? TTK_CONSULT_TYPES[s.lastLog.log_type] : null;
    // Override: special cancel statuses always show their own label
    if (OVERRIDE_STATUSES.includes(c.order_status) && TTK_CONSULT_TYPES[c.order_status]) {
        lastType = TTK_CONSULT_TYPES[c.order_status];
    }
    ...
    return `<tr class="${_pinClass}">
        <td style="text-align:center;padding:4px 2px;">
            ${!c.readonly ? `<span class="crm-pin-btn ${c.is_pinned ? 'active' : ''}" onclick="event.stopPropagation();_ttkTogglePin(${c.id})" title="${c.is_pinned ? 'B? pin' : 'Pin khách'}">${c.is_pinned ? '??' : '<span style="opacity:0.3">??</span>'}</span>` : ''}
        </td>
        ...
        <td style="font-size:11px;font-weight:700;color:#e65100;cursor:pointer;" onclick="_ttkOpenOrderCodesPopup(${c.id})">${s.latestOrderCode || '—'}</td>
        <td>
            ${c.readonly ? (
            ...
            ` : (c.cancel_approved === -2) ? `
                <button class="btn btn-sm consult-btn" onclick="_ttkOpenConsultModal(${c.id})" 
        ...
        <td style="font-size:12px;color:#e65100;font-weight:600;cursor:pointer;" onclick="_ttkOpenCustomerDetail(${c.id}).then(()=>setTimeout(()=>_ttkSwitchCDTab('history'),100))" title="${lastContent}">
        ...
        <td>
            ${!c.readonly ? '<button class="btn btn-sm" onclick="event.stopPropagation();_ttkOpenCustomerInfo(' + c.id + ')" style="font-size:9px;padding:1px 5px;margin-right:4px;background:var(--gray-700);color:var(--gold);" title="C?p nh?t thông tin">??</button>' : ''}
            ${(() => {
                const _colors = [
        ...
        <td style="font-size:12px;${currentUser.role === 'giam_doc' ? 'cursor:pointer;' : ''}" onclick="${currentUser.role === 'giam_doc' && !c.referrer_id ? '_ttkOpenReferrerSearch(' + c.id + ')' : ''}">
            ${c.referrer_id ? `<span style="cursor:pointer;text-decoration:underline;color:var(--info);font-weight:600;" onclick="event.stopPropagation();_ttkOpenAffiliateDetail(${c.referrer_id})">${c.referrer_name || c.referrer_customer_name}</span>` : (currentUser.role === 'giam_doc' ? '<span style="color:var(--gray-500)" title="Click d? tìm">?? Tìm</span>' : '<span style="color:var(--gray-500)">—</span>')}
        </td>
...
async function loadCrmTtkData() {
    // Sync dynamic consult types from API (adds any new types created in settings)
    await _ttkSyncConsultTypes();

    let url = '/api/customers?crm_type=tu_tim_kiem';
    ...
    _ttkAllCustomers = customers;
    _ttkAllStats = stats;

    // Count categories
    const counts = { phai_xu_ly: 0, moi_chuyen: 0, da_xu_ly: 0, xu_ly_tre: 0, cho_xu_ly: 0, huy_khach: 0 };
    customers.forEach(c => { const cat = _ttkGetCategory(c, stats); counts[cat]++; });
    ...
    document.querySelectorAll('.crm-stat-card').forEach(c => c.classList.remove('active'));
    if (_ttkActiveCat) {
        const activeEl = document.querySelector('.crm-stat-card[data-cat="' + _ttkActiveCat + '"]');
...
function applyCrmTtkFilter() { loadCrmTtkData(); }

// ========== PIN KHÁCH HÀNG ==========
async function _ttkTogglePin(customerId) {
    try {
        const res = await apiCall(`/api/customers/${customerId}/pin`, 'PATCH');
        if (res.success) {
            // Update local data
            const c = _ttkAllCustomers.find(x => x.id === customerId);
            if (c) {
...
// ========== CONSULTATION MODAL ==========
async function _ttkOpenConsultModal(customerId) {
    ...
    try {
        const frData = await apiCall('/api/consult-flow-rules?crm_menu=tu_tim_kiem');
    ...
    const allTypes = Object.entries(TTK_CONSULT_TYPES);
    ...
    function _getFlowRuleTypes(status) {
        ...
        return rules
            .map(r => [r.to_type_key, TTK_CONSULT_TYPES[r.to_type_key]])
    ...
    if (frTypes) {
        ...
        else { const order = ['giuc_coc','dat_coc','nhan_tin','cap_cuu_sep']; allowedTypes = order.map(k => [k, TTK_CONSULT_TYPES[k]]).filter(([,v]) => v); }
    ...
    // ? Store section key + max days for max_appointment_days enforcement in _ttkOnConsultTypeChange
    ...
    const historyHTML = consultLogs.length > 0 ? `
        <div style="margin-bottom:12px;">
            <button type="button" onclick="_ttkToggleConsultHistory()" style="background:none;border:1px solid var(--gray-200);border-radius:6px;padding:5px 12px;font-size:11px;color:var(--gray-500);cursor:pointer;display:flex;align-items:center;gap:4px;width:100%;">
            ...
            <div id="consultHistoryPanel" style="display:none;max-height:300px;overflow-y:auto;padding:10px;background:var(--gray-50);border-radius:0 0 8px 8px;border:1px solid var(--gray-200);border-top:none;">
                ${_ttkBuildGroupedHistoryHTML(consultLogs, { compact: true })}
            </div>
        </div>
    ` : '';
    ...
    const bodyHTML = `
        ...
        <div class="form-group">
            ...
            <select id="consultType" class="form-control" onchange="_ttkOnConsultTypeChange()">
                ${typeOptions}
            </select>
        </div>
        <div class="form-group" id="consultDepositGroup" style="display:none;">
            ...
            <input type="text" id="consultDepositAmount" class="form-control" placeholder="Nh?p s? ti?n d?t c?c..." 
                ...
                oninput="_ttkFormatDepositInput(this)">
        </div>
        ...
        <div class="form-group" id="consultImageGroup">
            ...
            <div id="consultImageArea" class="image-paste-area" tabindex="0">
                ...
                <button id="consultImageRemove" class="btn btn-sm" style="display:none;position:absolute;top:8px;right:8px;background:var(--danger);color:white;font-size:11px;padding:2px 8px;" onclick="_ttkRemoveConsultImage()">?</button>
            </div>
        </div>
        <div class="form-group" id="consultNextTypeGroup" style="display:none">
            ...
            <select id="consultNextType" class="form-control" onchange="_ttkUpdateApptLabel()">
                ${Object.entries(TTK_CONSULT_TYPES).filter(([k]) => ['goi_dien','nhan_tin','gap_truc_tiep','gui_bao_gia','gui_mau','thiet_ke','bao_sua','dat_coc','chot_don','cap_cuu_sep','huy'].includes(k)).map(([k, v]) =>
                ...
                <table class="table" style="font-size:13px;" id="consultOrderTable">
                    ...
                    <tbody>
                        ${existingItems.length > 0 ? existingItems.map(it => `<tr>
                            ...
                            <td><input type="text" class="form-control oi-price" value="${formatCurrency(it.unit_price||0)}" style="font-size:13px;padding:6px 8px;" oninput="_ttkFormatDepositInput(this);_ttkCalcConsultOrderTotal()"></td>
                            <td class="oi-total" style="text-align:right;font-weight:600">${formatCurrency(it.total)}</td>
                            <td><button class="btn btn-sm" onclick="this.closest('tr').remove();_ttkCalcConsultOrderTotal();" style="color:var(--danger)">?</button></td>
                        </tr>`).join('') : ''}
                    </tbody>
                </table>
                <div style="display:flex;justify-content:space-between;align-items:center;margin-top:8px;">
                    <button class="btn btn-sm" onclick="_ttkAddConsultOrderRow()" style="font-size:12px;">? Thêm dòng</button>
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
                ...
                <div class="form-group">
                    ...
                    <select id="consultCity" class="form-control">
                        ...
                        ${TTK_VN_PROVINCES.map(p => `<option value="${p}" ${customerInfo.province === p ? 'selected' : ''}>${p}</option>`).join('')}
                    </select>
                </div>
            </div>
            <div class="form-group" style="display:none">
                <label>Tu V?n Ti?p Theo <span style="color:var(--danger)">*</span></label>
                <select id="consultChotDonNextType" class="form-control" onchange="_ttkUpdateChotDonApptLabel()">
    ...
    const footerHTML = `
        ...
        <button class="btn btn-primary" id="consultSubmitBtn" onclick="_ttkSubmitConsultLog(${customerId})" style="width:auto;">?? GHI NH?N</button>
    `;
    ...
    setTimeout(() => {
        ...
        if (area) {
            area.addEventListener('paste', _ttkHandleConsultImagePaste);
            area.addEventListener('click', () => area.focus());
        }
        document.querySelectorAll('#consultOrderTable .oi-qty, #consultOrderTable .oi-price').forEach(el => el.addEventListener('input', _ttkCalcConsultOrderTotal));
        _ttkOnConsultTypeChange(); // trigger to show/hide correct fields
    }, 100);
}

window._consultImageBlob = null;

function _ttkHandleConsultImagePaste(e) {
...
function _ttkRemoveConsultImage() {
    window._consultImageBlob = null;
...
function _ttkOnConsultTypeChange() {
...
function _ttkUpdateApptLabel() {
    ...
    const val = sel.value;
    const typeInfo = TTK_CONSULT_TYPES[val];
...
function _ttkUpdateChotDonApptLabel() {
...
function _ttkBuildGroupedHistoryHTML(logs, options = {}) {
    ...
    return Object.entries(groups).map(([key, items]) => {
        ...
        const logsHTML = items.map((log, idx) => {
            const t = TTK_CONSULT_TYPES[log.log_type] || { icon: '??', label: log.log_type, color: '#6b7280' };
            const d = new Date(log.created_at);
            const days = ['CN','T2','T3','T4','T5','T6','T7'];
            const dayName = days[d.getDay()];
...
function _ttkBuildOrderCardHTML(codes, customer) {
...
// ========== ORDER CODES POPUP ==========
async function _ttkOpenOrderCodesPopup(customerId) {
    // Open full customer detail popup with "Ðon Hàng" tab pre-selected
    await _ttkOpenCustomerDetail(customerId);
    setTimeout(() => _ttkSwitchCDTab('orders'), 100);
}

// Per-order completion
async function _ttkCompleteOrder(orderId, customerId) {
    ...
    try {
        ...
        if (res.success) {
            ...
            closeModal();
            _ttkOpenOrderCodesPopup(customerId);
        } else {
...
async function _ttkCancelOrder(orderId, customerId) {
...
function _ttkToggleConsultHistory() {
...
function _ttkAddConsultOrderRow() {
    const tbody = document.querySelector('#consultOrderTable tbody');
    tbody.insertAdjacentHTML('beforeend', `<tr>
        <td><input class="form-control oi-desc" value="" style="font-size:13px;padding:6px 8px;"></td>
        <td><input type="number" class="form-control oi-qty" value="0" min="0" style="font-size:13px;padding:6px 8px;width:70px;" oninput="_ttkCalcConsultOrderTotal()"></td>
        <td><input type="text" class="form-control oi-price" value="0" style="font-size:13px;padding:6px 8px;" oninput="_ttkFormatDepositInput(this);_ttkCalcConsultOrderTotal()"></td>
        <td class="oi-total" style="text-align:right;font-weight:600">0</td>
        <td><button class="btn btn-sm" onclick="this.closest('tr').remove();_ttkCalcConsultOrderTotal();" style="color:var(--danger)">?</button></td>
    </tr>`);
}

function _ttkCalcConsultOrderTotal() {
    let grand = 0;
...
async function _ttkSubmitConsultLog(customerId) {
    ...
    if (log_type === 'huy') {
        ...
        if (!reason) { showToast('Vui lòng nh?p lý do h?y!', 'error'); _ttkEnableSubmitBtn(); return; }
        try {
            const data = await apiCall(`/api/customers/${customerId}/cancel`, 'POST', { reason });
            if (data.success) { showToast('? ' + data.message); closeModal(); loadCrmTtkData(); }
            else { showToast(data.error || 'L?i!', 'error'); _ttkEnableSubmitBtn(); }
        } catch (err) { showToast('L?i k?t n?i!', 'error'); _ttkEnableSubmitBtn(); }
        return;
    }
    ...
    if (log_type === 'cap_cuu_sep') {
        ...
        if (!content) { showToast('Vui lòng nh?p n?i dung tình hu?ng!', 'error'); _ttkEnableSubmitBtn(); return; }
        if (!handler_id) { showToast('Vui lòng ch?n S?p x? lý!', 'error'); _ttkEnableSubmitBtn(); return; }
        try {
            ...
            if (data.success) {
                showToast('?? ' + data.message); closeModal(); window._consultImageBlob = null; loadCrmTtkData();
            } else { showToast(data.error || 'L?i!', 'error'); _ttkEnableSubmitBtn(); }
        } catch (err) { showToast('L?i k?t n?i!', 'error'); _ttkEnableSubmitBtn(); }
        return;
    }
    ...
    if (log_type === 'dat_coc') {
        ...
        if (depositAmount <= 0) {
            showToast('Vui lòng nh?p s? ti?n d?t c?c!', 'error'); _ttkEnableSubmitBtn(); return;
        }
        ...
        try {
            ...
            if (data.success) {
                showToast('? Ð?t c?c thành công!'); closeModal(); window._consultImageBlob = null; loadCrmTtkData();
            } else { showToast(data.error || 'L?i!', 'error'); _ttkEnableSubmitBtn(); }
        } catch (err) { showToast('L?i k?t n?i!', 'error'); _ttkEnableSubmitBtn(); }
        return;
    }
    ...
    if (log_type === 'chot_don') {
        ...
        if (!address) { showToast('Vui lòng nh?p d?a ch?!', 'error'); _ttkEnableSubmitBtn(); return; }
        if (!city) { showToast('Vui lòng ch?n thành ph?!', 'error'); _ttkEnableSubmitBtn(); return; }
        if (!sbhDate) { showToast('Vui lòng ch?n ngày h?n sau bán hàng!', 'error'); _ttkEnableSubmitBtn(); return; }

        // Phone validate
        if (phone && !/^\d{10}$/.test(phone)) {
            showToast('SÐT ph?i dúng 10 ch? s?', 'error'); _ttkEnableSubmitBtn(); return;
        }

        // Collect order items
        const rows = document.querySelectorAll('#consultOrderTable tbody tr');
        if (rows.length === 0) { showToast('Vui lòng thêm ít nh?t 1 s?n ph?m!', 'error'); _ttkEnableSubmitBtn(); return; }
        const items = [];
        ...
        if (items.length === 0) { showToast('Vui lòng nh?p s?n ph?m h?p l?!', 'error'); _ttkEnableSubmitBtn(); return; }
        ...
        try {
            ...
            const data = await res.json();
            if (data.success) {
                showToast('? Ch?t don thành công! Chuy?n sang Sau Bán Hàng.'); closeModal(); window._consultImageBlob = null; loadCrmTtkData();
            } else { showToast(data.error || 'L?i!', 'error'); _ttkEnableSubmitBtn(); }
        } catch (err) { showToast('L?i k?t n?i!', 'error'); _ttkEnableSubmitBtn(); }
        return;
    }
    ...
    if (!content) { showToast('Vui lòng nh?p n?i dung tu v?n!', 'error'); _ttkEnableSubmitBtn(); return; }
    ...
    if (imageRequiredTypes.includes(log_type) && !window._consultImageBlob) {
        showToast('Vui lòng dán hình ?nh (Ctrl+V)!', 'error'); _ttkEnableSubmitBtn(); return;
    }
    if (!appointment_date) { showToast('Vui lòng ch?n ngày h?n!', 'error'); _ttkEnableSubmitBtn(); return; }
...
async function _ttkOpenConsultHistory(customerId) {
    ...
    let bodyHTML = `
        <div style="display:flex;gap:8px;margin-bottom:16px;">
            <button class="btn btn-sm tab-btn active" onclick="_ttkSwitchConsultTab('info', this)" style="font-size:12px;">?? Thông Tin</button>
            <button class="btn btn-sm tab-btn" onclick="_ttkSwitchConsultTab('history', this)" style="font-size:12px;">?? L?ch S? (${logs.length})</button>
            <button class="btn btn-sm tab-btn" onclick="_ttkSwitchConsultTab('order', this)" style="font-size:12px;">?? Ðon Hàng</button>
        </div>
        ...
        <div id="tabInfo">
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;font-size:13px;">
                ...
                ${(c.referrer_name || c.referrer_customer_name) ? `<div><strong>Ngu?i GT:</strong> <span style="cursor:pointer;text-decoration:underline;color:var(--info);" onclick="_ttkOpenAffiliateDetail(${c.referrer_id})">${c.referrer_name || c.referrer_customer_name}</span></div>` : ''}
        ...
        <div id="tabHistory" style="display:none;">
            ...
            `<div style="max-height:350px;overflow-y:auto;">
                ${_ttkBuildGroupedHistoryHTML(logs)}
            </div>`}
        </div>

        <div id="tabOrder" style="display:none;">
            ${_ttkBuildOrderCardHTML(codes, items, c, totalDeposit)}
        </div>
    `;

    const footerHTML = `
        <button class="btn btn-secondary" onclick="closeModal()">Ðóng</button>
        <button class="btn btn-primary" onclick="_ttkOpenConsultModal(${c.id})" style="width:auto;">?? Tu V?n</button>
        <button class="btn btn-primary" onclick="_ttkSaveOrderItems(${c.id})" style="width:auto;">?? Luu Ðon</button>
        <button class="btn" onclick="_ttkRequestCancel(${c.id})" style="width:auto;background:var(--danger);color:white;">? H?y KH</button>
    `;
...
function _ttkSwitchConsultTab(tab, btn) {
...
// ========== UPDATE APPOINTMENT ==========
async function _ttkUpdateAppointment(customerId, date) {
...
// ========== REFERRER SEARCH ==========
let _ttkAllReferrerCustomers = [];
async function _ttkOpenReferrerSearch(customerId) {
    const bodyHTML = `
        <div class="form-group">
            <label>Tìm Ngu?i Gi?i Thi?u (tên ho?c SÐT)</label>
            <input type="text" id="referrerSearchInput" class="form-control" placeholder="Nh?p tên ho?c SÐT d? l?c..." oninput="_ttkFilterReferrerList(${customerId})">
        </div>
    ...
    try {
        ...
        _ttkAllReferrerCustomers = data.customers || [];
        _ttkRenderReferrerList(customerId, _ttkAllReferrerCustomers);
    } catch(e) {
...
function _ttkFilterReferrerList(customerId) {
    const q = (document.getElementById('referrerSearchInput')?.value || '').toLowerCase().trim();
    const filtered = q ? _ttkAllReferrerCustomers.filter(c =>
        (c.customer_name || '').toLowerCase().includes(q) ||
        (c.phone || '').includes(q)
    ) : _ttkAllReferrerCustomers;
    _ttkRenderReferrerList(customerId, filtered);
}

function _ttkRenderReferrerList(customerId, customers) {
    ...
    results.innerHTML = customers.map(c => {
        ...
        return `
            <div onclick="_ttkSelectReferrer(${customerId}, ${c.id})" 
...
async function _ttkSelectReferrer(customerId, referrerCustomerId) {
...
async function _ttkOpenCustomerInfo(customerId) {
    ...
    const provinceOptions = TTK_PROVINCES.map(p => 
    ...
    const bodyHTML = `
        ...
        <div class="form-group">
            ...
            <button class="btn btn-sm" onclick="_ttkAddHolidayRow()" style="font-size:12px;margin-top:6px;">? Thêm ngày l?</button>
        </div>
    `;

    const footerHTML = `
        <button class="btn btn-secondary" onclick="closeModal()">H?y</button>
        <button class="btn btn-primary" onclick="_ttkSaveCustomerInfo(${customerId})" style="width:auto;">?? LUU</button>
    `;

    openModal('?? C?p Nh?t Thông Tin KH', bodyHTML, footerHTML);
}

function _ttkAddHolidayRow() {
    let dOpts = '<option value="">Ngày</option>';
...
async function _ttkSaveCustomerInfo(customerId) {
...
// ========== CHI TI?T KHÁCH HÀNG ==========
async function _ttkOpenCustomerDetail(customerId) {
    ...
    const lastConsultTypePopup = lastLogPopup ? TTK_CONSULT_TYPES[lastLogPopup.log_type] : null;
        ...
        <div style="margin:-12px -12px 0;font-family:'Segoe UI',system-ui,sans-serif;">
            ...
            ${(c.referrer_name || c.referrer_customer_name) ? `
                <div style="margin-top:10px;padding:10px 14px;background:linear-gradient(135deg,#eff6ff,#dbeafe);border-radius:8px;border:1px solid #bfdbfe;font-size:13px;">
                    ...
                    <span style="cursor:pointer;text-decoration:underline;color:#3b82f6;font-weight:600;" onclick="_ttkOpenAffiliateDetail(${c.referrer_id})">${c.referrer_name || c.referrer_customer_name}</span>
    ...
    const orderTab = _ttkBuildOrderCardHTML(orderCodes, orders, c, cdTotalDeposit);

    const bodyHTML = `
        <div style="display:flex;gap:8px;margin-bottom:12px;flex-wrap:wrap;">
            <span class="cdtab-btn" onclick="_ttkSwitchCDTab('info')" id="cdtab-info-btn"
            ...
            <span class="cdtab-btn" onclick="_ttkSwitchCDTab('history')" id="cdtab-history-btn"
            ...
            <span class="cdtab-btn" onclick="_ttkSwitchCDTab('orders')" id="cdtab-orders-btn"
    ...
    const lastConsultType = lastLog ? TTK_CONSULT_TYPES[lastLog.log_type] : null;
    ...
    const footerHTML = `
        ...
        ${!c.cancel_requested && !c.cancel_approved ? `
            <button class="btn btn-primary" onclick="closeModal();_ttkOpenConsultModal(${customerId});" style="width:auto;${consultBtnColor ? 'background:' + consultBtnColor + ';color:' + consultBtnTextColor + ';' : ''}">${consultBtnLabel}</button>
        ` : ''}
    `;
...
const TTK_CRM_LABELS_AFF = { nhu_cau: 'Cham Sóc KH Nhu C?u', ctv: 'Cham Sóc CTV', tu_tim_kiem: 'CRM T? Tìm Ki?m', goi_hop_tac: 'CRM G?i Ði?n H?p Tác', goi_ban_hang: 'CRM G?i Ði?n Bán Hàng', koc_tiktok: 'CRM KOL/KOC Tiktok' };
const TTK_ROLE_LABELS_AFF = { giam_doc:'Giám Ð?c', quan_ly_cap_cao:'Qu?n Lý C?p Cao', quan_ly:'Qu?n Lý', truong_phong:'Tru?ng Phòng', nhan_vien:'Nhân Viên', part_time:'Part Time', hoa_hong:'Hoa H?ng', ctv:'CTV', nuoi_duong:'Nuôi Du?ng', sinh_vien:'Sinh Viên', tkaffiliate:'TK Affiliate' };

async function _ttkOpenAffiliateDetail(userId) {
    if (!userId) return;
    try {
                    ...
                    <div style="background:#f8fafc;border-radius:12px;border:1px solid #e2e8f0;overflow:hidden;">
                        <div style="display:grid;grid-template-columns:1fr 1fr;gap:0;">
                            ...
                            <div style="padding:14px 16px;border-bottom:1px solid #e2e8f0;">
                                ...
                                <div style="font-size:14px;font-weight:600;color:#1e293b;">${TTK_CRM_LABELS_AFF[u.source_crm_type] || u.source_crm_type || '—'}</div>
                            </div>
                            <div style="padding:14px 16px;border-bottom:1px solid #e2e8f0;border-right:1px solid #e2e8f0;">
                            ...
                            <div style="padding:14px 16px;border-bottom:1px solid #e2e8f0;">
                                ...
                                <div style="font-size:14px;font-weight:600;color:#1e293b;">${TTK_ROLE_LABELS_AFF[u.role] || u.role}</div>
                            </div>
                        </div>
                    </div>
                </div>
...
async function _ttkToggleAffiliateStatus(userId, newStatus) {
    try {
        ...
        if (data.success) {
            ...
            _ttkOpenAffiliateDetail(userId);
        } else {
...
async function _ttkOpenEditAffiliateFromCrm(userId) {
    try {
        ...
        const footerHTML = `<button class="btn" onclick="_ttkSubmitEditAffFromCrm(${u.id})" style="background:var(--gold);color:#122546;padding:8px 24px;border-radius:8px;font-weight:600;">?? Luu thay d?i</button>`;
...
async function _ttkSubmitEditAffFromCrm(userId) {
    const body = {
