...
function _crmGetDateRange() {
    ...
    switch (_crmDatePreset) {
        ...
        case 'this_month': { const m = new Date(_crmSelectedYear, today.getMonth(), 1); return { from: fmt(m), to: fmt(today) }; }
        case 'last_month': { const m1 = new Date(_crmSelectedYear, today.getMonth()-1, 1); const m2 = new Date(_crmSelectedYear, today.getMonth(), 0); return { from: fmt(m1), to: fmt(m2) }; }
        case 'custom': return { from: _crmDateFrom, to: _crmDateTo };
...
function _crmSelectSidebarUser(userId, userName) {
...
function _crmBuildDateFilterHTML() {
    ...
    return `<div id="crmDateChips" style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;margin-bottom:12px;padding:10px 14px;background:linear-gradient(135deg,#f8fafc,#f1f5f9);border:1.5px solid #e2e8f0;border-radius:12px;">
        ...
        <div id="crmCustomDateArea" style="display:${_crmDatePreset==='custom'?'flex':'none'};align-items:center;gap:6px;margin-left:4px;">
            <input type="date" id="crmCustomFrom" value="${dr.from}" style="padding:4px 8px;border:1.5px solid #e2e8f0;border-radius:8px;font-size:11px;font-weight:600;">
            <span style="font-size:11px;color:#9ca3af;">→</span>
            <input type="date" id="crmCustomTo" value="${dr.to}" style="padding:4px 8px;border:1.5px solid #e2e8f0;border-radius:8px;font-size:11px;font-weight:600;">
            <button onclick="_crmApplyCustomDate()" style="padding:4px 10px;border-radius:8px;font-size:11px;font-weight:700;cursor:pointer;border:1.5px solid #059669;background:linear-gradient(135deg,#059669,#10b981);color:white;">✓</button>
        </div>
    </div>`;
}
...
async function renderCRMNhuCauPage(container) {
    ...
    container.innerHTML = `
        ...
        ${sidebarHTML}
        <div id="crmMainContent" style="flex:1;overflow:auto;padding:16px 20px;">
        <style>
        ...
        <div style="display:flex; gap:12px; margin-bottom:12px; flex-wrap:wrap; align-items:center;">
            ...
            <select id="crmFilterCustomerType" class="form-control" style="width:auto;min-width:160px;font-weight:600;" onchange="_crmRenderFilteredTable()">
...
function _crmRenderFilteredTable() {
    ...
    // Apply date filter using date chips (universal, not just specific categories)
    const dr = _crmGetDateRange();
    if (dr.from && dr.to) {
        filtered = filtered.filter(c => {
            // Use appointment_date for cho_xu_ly/xu_ly_tre, cancel_approved_at for huy_khach, created_at for others
            let dateField;
            if (_crmActiveCat === 'cho_xu_ly' || _crmActiveCat === 'xu_ly_tre') dateField = c.appointment_date;
            else if (_crmActiveCat === 'huy_khach') dateField = c.cancel_approved_at || c.created_at;
    ...
    // Re-render date chips to update active state
    const dcArea = document.getElementById('crmDateChipsArea');
    if (dcArea) dcArea.innerHTML = _crmBuildDateFilterHTML();
    ...
    return `<tr class="${_pinClass}" data-customer-id="${c.id}">
        ...
        <td style="font-size:12px;">
            <div style="display:flex;align-items:center;gap:4px;margin-bottom:4px;">
                ...
                ${(() => {
                    ...
                    return `<span onclick="openCustomerDetail(${c.id})" title="${c.customer_name}" style="cursor:pointer;display:inline-block;max-width:180px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;vertical-align:middle;padding:3px 12px;border-radius:20px;font-size:12px;font-weight:700;background:${_cc.bg};color:${_cc.text};border:1px solid ${_cc.border};transition:all 0.2s;" onmouseover="this.style.boxShadow='0 2px 8px ${_cc.border}'" onmouseout="this.style.boxShadow='none'">${_bdayIcon}${c.customer_name}</span><span onclick="event.stopPropagation();_crmCopyText('${c.customer_name.replace(/'/g, "\\'")}',this,'Tên')" style="cursor:pointer;font-size:11px;color:#94a3b8;margin-left:4px;transition:color 0.2s;vertical-align:middle;" onmouseover="this.style.color='#3b82f6'" onmouseout="this.style.color='#94a3b8'" title="Copy tên">📋</span>`;
                })()}
            </div>
            <div style="font-size:11px;color:#64748b;margin-bottom:2px;display:flex;align-items:center;gap:4px;flex-wrap:wrap;">
        ...
        <td style="text-align:center;padding:4px 2px;">
            ${!c.readonly && canDo('crm_nhu_cau', 'edit') && c.cancel_approved !== 1 ? (() => {
                ...
                return `<span onclick="event.stopPropagation();openCrmTransferPopup(${c.id})" title="Đề Xuất Chuyển CRM" style="cursor:pointer;font-size:16px;opacity:0.5;transition:opacity .2s;" onmouseover="this.style.opacity='1'" onmouseout="this.style.opacity='0.5'">🔄</span>`;
            })() : ''}
        </td>
    </tr>`;
}
...
async function loadCrmNhuCauData() {
    ...
    try {
        ...
        _crmPendingCtvIds = (pendingData.customers || []).map(c => c.id);
    } catch(e) { _crmPendingCtvIds = []; }
    ...
    customers.forEach(c => { const cat = _crmGetCategory(c, stats); if (counts[cat] !== undefined) counts[cat]++; });
    ...
    if (targetId) {
        ...
        const targetCustomer = _crmAllCustomers.find(c => c.id === tid);
        if (targetCustomer) {
            // 1. Detect the customer's actual category
            const targetCat = _crmGetCategory(targetCustomer, _crmAllStats);
            const tabCat = targetCat === 'moi_chuyen' ? 'phai_xu_ly' : targetCat;
            
            // 2. Set active category directly (DO NOT use _crmFilterByCat — it toggles!)
            _crmActiveCat = tabCat;
            document.querySelectorAll('.crm-stat-card').forEach(c => c.classList.remove('active'));
            ...
            // 3. Clear all sub-filters (date + consult type) so customer is NOT filtered out
            ...
            // 4. Show/hide date filter UI based on category
            ...
            // 5. Build filtered+sorted list (mirrors _crmRenderFilteredTable logic, no date/consult sub-filters)
            let filtered = _crmAllCustomers;
            if (tabCat === 'phai_xu_ly') {
                filtered = _crmAllCustomers.filter(c => { const cat = _crmGetCategory(c, _crmAllStats); return cat === 'phai_xu_ly' || cat === 'moi_chuyen'; });
            } else {
                filtered = _crmAllCustomers.filter(c => _crmGetCategory(c, _crmAllStats) === tabCat);
            }
            filtered = [...filtered].sort((a, b) => {
...
async function openConsultModal(customerId) {
    ...
    if (frTypes) {
        ...
        else { const order = ['giuc_coc','dat_coc','chot_don','nhan_tin','cap_cuu_sep']; allowedTypes = order.map(k => [k, CONSULT_TYPES[k]]).filter(([,v]) => v); }
...
async function submitConsultLog(customerId) {
    ...
    if (log_type === 'gui_mau') {
        ...
        try {
            ...
            if (data.success) {
                showToast('✅ Gửi mẫu vải/áo thành công!'); closeModal(); window._consultImageBlob = null; loadCrmNhuCauData();
...
const CRM_LABELS_AFF = { nhu_cau: 'Chăm Sóc KH Nhu Cầu', ctv: 'Chăm Sóc CTV', ctv_hoa_hong: 'Chăm Sóc Affiliate', koc_tiktok: 'CRM KOL/KOC Tiktok' };
const ROLE_LABELS_AFF = { giam_doc:'Giám Đốc', quan_ly_cap_cao:'Quản Lý Cấp Cao', quan_ly:'Quản Lý', truong_phong:'Trưởng Phòng', nhan_vien:'Nhân Viên', thu_viec:'Thử Việc', part_time:'Part Time', hoa_hong:'Hoa Hồng', ctv:'CTV', nuoi_duong:'Nuôi Dưỡng', sinh_vien:'Sinh Viên', tkaffiliate:'TK Affiliate' };
