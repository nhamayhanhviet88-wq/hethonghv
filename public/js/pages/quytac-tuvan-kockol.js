...
async function _qtKLoadData() {
    ...
    _qtKUnsectioned = (sectionsData.unsectioned || []).filter(t => !['huy', 'cap_cuu_sep'].includes(t.key));
...
async function _qtKDeleteStage(stageId) {
    ...
    if (orders.length > 0) {
        await apiCall('/api/consult-types/batch/sort-order', 'PATCH', { orders, crm_menu: 'koc_tiktok' });
    }
...
async function _qtKToggleSection_Active(key, toggleWrapEl) {
    const t = _qtKAllTypes.find(x => x.key === key);
    if (!t) return;
    const newActive = !t.is_active;
...
async function _qtKSaveType(key) {
    ...
    await apiCall(`/api/consult-types/${key}`, 'PUT', {...data, crm_menu: 'koc_tiktok'});
    document.querySelector('.qt-modal-overlay')?.remove();
        ...
        <div class="qt-flow-section" id="${sectionId}">
            <div class="qt-flow-header" onclick="_qtKToggleSection(this)">
                ...
                <div style="display:flex;align-items:center;gap:8px;">
                    ${_qtKIsGD ? (() => {
                        const secType = _qtKAllTypes.find(x => x.key === sec.key);
                        const secActive = secType ? secType.is_active : true;
                        return `<div class="qt-toggle-wrap" style="margin:0;gap:4px;" onclick="event.stopPropagation();_qtKToggleSection_Active('${sec.key}',this)" title="${secActive ? 'Click để tắt loại này' : 'Click để bật lại loại này'}">
                        ...
                        </div><button class="qt-flow-edit-btn" onclick="event.stopPropagation();_qtKShowEditRulesModal('${sec.key}')">✏️ Sửa</button><button class="qt-flow-edit-btn" style="background:#ef4444;padding:4px 8px;min-width:0;" onclick="event.stopPropagation();_qtKDeleteSection('${sec.key}')" title="Xóa loại">🗑️</button><button class="qt-flow-edit-btn" style="background:linear-gradient(135deg,#8b5cf6,#7c3aed);padding:4px 8px;min-width:0;" onclick="event.stopPropagation();_qtKShowMergeModal('${sec.key}')" title="Gom thêm nút vào loại này">➕</button>${isGroup ? `<button class="qt-flow-edit-btn" style="background:linear-gradient(135deg,#f59e0b,#d97706);padding:4px 8px;min-width:0;" onclick="event.stopPropagation();_qtKShowUngroupModal('${sec.key}')" title="Hủy gom nhóm">🔓</button>` : ''}<button class="qt-flow-edit-btn" style="background:linear-gradient(135deg,#0ea5e9,#0284c7);padding:4px 8px;min-width:0;position:relative;" onclick="event.stopPropagation();_qtKShowChangePhaseDropdown('${sec.key}',this)" title="Đổi thuộc Phần">▼</button>`;
                    })() : ''}
...
async function _qtKEditMaxDays(key, currentVal) {
    ...
    try {
        await apiCall(`/api/consult-types/${key}/max-appointment-days`, 'PATCH', { max_appointment_days: days, crm_menu: 'koc_tiktok' });
...
function _qtKShowChangePhaseDropdown(key, btnEl) {
...
async function _qtKChangePhase(key, phaseId) {
    document.querySelectorAll('.qt-phase-dropdown').forEach(d => d.remove());
    try {
        await apiCall(`/api/consult-types/${key}/rule-phase`, 'PATCH', { rule_phase: phaseId || null, crm_menu: 'koc_tiktok' });
...
async function _qtKAddPhase() {
    ...
    if (updates.length > 0) await apiCall('/api/consult-types/batch/rule-phase', 'PATCH', { updates, crm_menu: 'koc_tiktok' });
...
function _qtKShowEditRulesModal(fromStatus) {
    ...
    const statusLabel = KOCKOL_FLOW_STATUS_LABELS[fromStatus] || _qtKGetTypeLabel(fromStatus) || fromStatus;
...
function _qtKShowMergeModal(sectionKey) {
    const sec = _qtKSections.find(s => s.key === sectionKey);
    if (!sec) return;
    const isGroup = !!sec.section_group;
    const groupKeys = isGroup ? _qtKGetGroupKeys(sec.section_group) : [sec.key];
    const tp = _qtKAllTypes.find(x => x.key === sec.key);
    ...
    const SYSTEM_KEYS = ['huy', 'cap_cuu_sep'];
    const sectionKeys = new Set(_qtKSections.map(s => s.key));
    const groupMemberKeys = new Set(_qtKGroupMembers.map(m => m.key));
    const available = _qtKAllTypes.filter(t =>
        t.is_active &&
    ...
    overlay.innerHTML = `
        <div class="qt-modal">
            ...
            <div style="display:flex;flex-wrap:wrap;gap:4px;margin-bottom:12px;">
                ${groupKeys.map(k => { const mt = _qtKAllTypes.find(x => x.key === k); return mt ? `<span style="font-size:10px;padding:2px 8px;border-radius:6px;background:${mt.color}20;color:${mt.color};font-weight:700;border:1px solid ${mt.color}30;">${mt.icon} ${mt.label}</span>` : ''; }).join('')}
            </div>
            ...
            <div class="qt-actions">
                <button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button>
                <button class="qt-btn qt-btn-primary" onclick="_qtKMergeIntoGroup('${sectionKey}')">➕ Gom vào nhóm</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function _qtKMergeIntoGroup(sectionKey) {
    const sec = _qtKSections.find(s => s.key === sectionKey);
    if (!sec) return;
    ...
    const leaderRules = _qtKAllRules[leaderKey] || [];
    ...
    for (const key of selected) {
        ...
        await apiCall(`/api/consult-types/${key}/section-order`, 'PATCH', { section_order: 0, crm_menu: 'koc_tiktok' });
...
// ========== UNGROUP ==========
function _qtKShowUngroupModal(sectionKey) {
    const sec = _qtKSections.find(s => s.key === sectionKey);
    if (!sec || !sec.section_group) return;
    const groupKeys = _qtKGetGroupKeys(sec.section_group);
    ...
    let listHTML = groupKeys.map(k => {
        const mt = _qtKAllTypes.find(x => x.key === k);
    ...
    overlay.innerHTML = `<div class="qt-modal"><h3>🔓 Hủy gom nhóm: ${currentLabel}</h3>
        ...
        <div class="qt-actions" style="gap:8px;"><button class="qt-btn" style="background:linear-gradient(135deg,#ef4444,#dc2626);color:white;box-shadow:0 2px 8px rgba(239,68,68,.3);" onclick="_qtKDisbandGroup('${sectionKey}')">💥 Giải tán toàn bộ</button><div style="flex:1;"></div><button class="qt-btn qt-btn-secondary" onclick="this.closest('.qt-modal-overlay').remove()">Hủy</button><button class="qt-btn" style="background:linear-gradient(135deg,#f59e0b,#d97706);color:white;box-shadow:0 2px 8px rgba(245,158,11,.3);" onclick="_qtKUngroupSelected('${sectionKey}')">🔓 Bỏ ra khỏi nhóm</button></div></div>`;
    document.body.appendChild(overlay);
...
async function _qtKUngroupSelected(sectionKey) {
    const sec = _qtKSections.find(s => s.key === sectionKey); if (!sec || !sec.section_group) return;
    const groupKeys = _qtKGetGroupKeys(sec.section_group);
    ...
    document.querySelector('.qt-modal-overlay')?.remove(); showToast(`✅ Đã bỏ ${toRemove.length} nút ra khỏi nhóm!`, 'success'); await _qtKLoadData(); _qtKSwitchTab('rules');
}
async function _qtKDisbandGroup(sectionKey) {
    const sec = _qtKSections.find(s => s.key === sectionKey); if (!sec || !sec.section_group) return;
    const groupKeys = _qtKGetGroupKeys(sec.section_group); const label = sec.section_group_label || sec.key;
    ...
    document.querySelector('.qt-modal-overlay')?.remove(); showToast(`✅ Đã giải tán nhóm "${label}"!`, 'success'); await _qtKLoadData(); _qtKSwitchTab('rules');
}
