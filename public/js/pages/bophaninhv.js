...
async function _bpiRenderFieldsModal() {
    ...
    html += '<div style="width:330px;border-right:1px solid #e2e8f0;padding-right:16px;display:flex;flex-direction:column">';
    ...
    if (_bpFields.length) {
        _bpFields.forEach(function(f) {
            ...
            html += '<span id="_bpiFieldNameVal_' + f.id + '" style="font-weight:600;color:#1e293b;max-width:210px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">🎨 ' + f.name + '</span>';
...
async function _bpiFieldEdit(id) {
    ...
    if (nameSpan && actionsDiv) {
        nameSpan.innerHTML = '<input id="_bpiFieldEditInput_' + id + '" value="' + f.name.replace(/"/g, '&quot;') + '" style="width:180px;padding:4px 6px;border:1px solid #cbd5e1;border-radius:6px;font-size:11px;font-weight:700" onclick="event.stopPropagation()">';
