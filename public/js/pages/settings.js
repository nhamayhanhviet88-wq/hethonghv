            ...
            html += `
                <li class="setting-item" id="setting-item-${item.id}">
                    ...
                    <div class="item-actions" style="display:flex;gap:4px;align-items:center;">
                        ${isSources ? `
                            <button class="btn btn-xs" onclick="toggleSourceDp(${item.id})" style="background:${item.show_in_dp ? '#dbeafe' : '#f3f4f6'};color:${item.show_in_dp ? '#1e40af' : '#9ca3af'};border:1px solid ${item.show_in_dp ? '#93c5fd' : '#e5e7eb'};padding:3px 7px;font-size:11px;cursor:pointer;font-weight:700;" title="${item.show_in_dp ? 'Đang bật ở Lĩnh Vực Đồng Phục — Bấm để tắt' : 'Đang tắt ở Lĩnh Vực Đồng Phục — Bấm để bật'}">👕 Đồng Phục ${item.show_in_dp ? '✅' : '❌'}</button>
                            <button class="btn btn-xs" onclick="toggleSourcePetTem(${item.id})" style="background:${item.show_in_pettem ? '#ffedd5' : '#f3f4f6'};color:${item.show_in_pettem ? '#c2410c' : '#9ca3af'};border:1px solid ${item.show_in_pettem ? '#fdba74' : '#e5e7eb'};padding:3px 7px;font-size:11px;cursor:pointer;font-weight:700;" title="${item.show_in_pettem ? 'Đang bật ở Lĩnh Vực PET/TEM — Bấm để tắt' : 'Đang tắt ở Lĩnh Vực PET/TEM — Bấm để bật'}">🏷️ PET/TEM ${item.show_in_pettem ? '✅' : '❌'}</button>
