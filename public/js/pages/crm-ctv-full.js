...
var _ctvDepositList = [];
async function _ctvFetchDeposits() {
    try {
        const data = await apiCall('/api/dht/unclaimed-deposits');
        _ctvDepositList = data.deposits || [];
    } catch(e) { _ctvDepositList = []; }
}
function _ctvClearSelectedDeposit() {
...
function _ctvPickDeposit(prId) {
    var d = _ctvDepositList.find(function(x){ return x.id === prId; });
    if (!d) return;
...
async function renderCRMCtvPage(container) {
    window._crmReloadCurrentPage = () => loadCrmCtvData();
    window._crmRenderCurrentTable = () => _ctvRenderFilteredTable();
    ...
    container.innerHTML = `<div style="display:flex;height:calc(100vh - 120px);gap:0;">${sidebarHTML}<div id="crmMainContent" style="flex:1;overflow:auto;padding:16px 20px;">
        ...
        <div class="crm-stat-cards" id="crmStatCards">
            ...
            <div class="crm-stat-card" data-cat="gui_hang_hoan_thanh" style="background:linear-gradient(135deg,#0d9488,#0f766e);color:white;" onclick="_ctvFilterByCat('gui_hang_hoan_thanh')">
                <div class="stat-icon">📦✅</div>
        ...
        <div class="card">
            <div class="card-body" style="overflow-x:auto; padding:8px;">
                <table class="table crm-ctv-table" id="crmCtvTable">
                    ...
                    <tbody id="crmCtvTbody"><tr><td colspan="12" style="text-align:center;padding:40px;">⏳ Đang tải...</td></tr></tbody>
                </table>
...
function _ctvRenderFilteredTable() {
    ...
    if (dr.from && dr.to) {
        filtered = filtered.filter(c => {
            let dateField;
            if (_ctvActiveCat === 'cho_xu_ly' || _ctvActiveCat === 'xu_ly_tre') dateField = c.appointment_date;
            else if (_ctvActiveCat === 'huy_khach') dateField = c.cancel_approved_at || c.created_at;
    ...
    if (affFilterVal) {
        if (affFilterVal === 'approved') {
            filtered = filtered.filter(c => _ctvAffApprovedIds.includes(c.id) && !_ctvAffLockedIds.includes(c.id));
        } else if (affFilterVal === 'locked') {
            filtered = filtered.filter(c => _ctvAffLockedIds.includes(c.id));
        } else if (affFilterVal === 'pending') {
            filtered = filtered.filter(c => _ctvAffPendingIds.includes(c.id));
        } else if (affFilterVal === 'none') {
            filtered = filtered.filter(c => !_ctvAffApprovedIds.includes(c.id) && !_ctvAffPendingIds.includes(c.id));
        }
    }
    const _affFilterEl = document.getElementById('crmFilterAffStatus');
    if (_affFilterEl) {
        const _allForAff = _ctvAllCustomers;
        const _appCnt = _allForAff.filter(c => _ctvAffApprovedIds.includes(c.id) && !_ctvAffLockedIds.includes(c.id)).length;
        const _lockCnt = _allForAff.filter(c => _ctvAffLockedIds.includes(c.id)).length;
        const _penCnt = _allForAff.filter(c => _ctvAffPendingIds.includes(c.id)).length;
...
function _ctvRenderCustomerRow(c, stats, stt) {
    ...
    return `<tr class="${_pinClass}" data-customer-id="${c.id}">
        ...
        <td style="font-size:12px;vertical-align:middle;">
            ...
            <div style="font-size:11px;font-weight:700;color:#e65100;cursor:pointer;" onclick="_ctvOpenOrderCodesPopup(${c.id})">${s.latestOrderCode || '—'}</div>
        </td>
        ...
        <td style="font-size:12px;vertical-align:middle;">
            ${(() => {
                ...
                <div style="max-width:180px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:12px;font-weight:700;" title="${_nameTitle}">
                    <span onclick="_ctvOpenCustomerDetail(${c.id})" style="cursor:pointer;color:${_cc.text};background:${_cc.bg};border:1px solid ${_cc.border};padding:3px 10px;border-radius:20px;">
        ...
        <!-- Column 9: Liên Hệ (SĐT + Link + Địa chỉ) -->
        <td style="font-size:12px;vertical-align:middle;">
        ...
        <td style="font-size:12px;vertical-align:middle;">
            ...
            ${c.referrer_id ? `
                <div style="font-size:11px;color:#475569;margin-bottom:2px;">
                    Người GT: <span style="cursor:pointer;text-decoration:underline;color:var(--info);font-weight:600;" onclick="event.stopPropagation();_ctvOpenAffiliateDetail(${c.referrer_id})">${c.referrer_name || c.referrer_customer_name}</span>
                </div>
            ` : (currentUser.role === 'giam_doc' ? `<div style="font-size:11px;color:#94a3b8;margin-bottom:2px;cursor:pointer;" onclick="_ctvOpenReferrerSearch(${c.id})">🔍 Tìm Người GT</div>` : '')}
...
async function loadCrmCtvData() {
    ...
    try {
        ...
        _ctvAffLockedIds = affStatus.lockedCustomerIds || [];
        _ctvAffApprovedMap = affStatus.approvedMap || {};
    } catch(e) { _ctvAffPendingIds = []; _ctvAffApprovedIds = []; _ctvAffLockedIds = []; _ctvAffApprovedMap = {}; }
    ...
    const bodyHTML = `
        ...
        <div class="form-group" id="consultDepositGroup" style="display:none;">
            ...
            <input type="text" id="consultDepositSearch" class="form-control" placeholder="🔍 Gõ mã tiền, số tiền, nội dung CK..." 
                autocomplete="off" oninput="_ctvFilterDepositList()" onfocus="_ctvFilterDepositList()"
            ...
            <div id="consultDepositSelected" style="display:none;background:#f0fdf4;border:1px solid #059669;border-radius:8px;padding:10px 14px;margin-top:6px;position:relative;">
                ...
                <button type="button" onclick="_ctvClearSelectedDeposit()" style="position:absolute;top:8px;right:8px;background:none;border:none;color:#ef4444;font-size:16px;font-weight:800;cursor:pointer;padding:2px 6px;" title="Xóa chọn">✕</button>
            </div>
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
                ...
                <div class="form-group">
                    ...
                    <select id="consultCity" class="form-control">
                        ...
                        ${CTV_VN_PROVINCES.map(p => `<option value="${p}" ${customerInfo.province === p ? 'selected' : ''}>${p}</option>`).join('')}
                    </select>
                </div>
            </div>
...
function _ctvBuildOrderCardHTML(codes, items, customer) {
            ...
            <div style="padding:12px;border:1px solid ${oc.status === 'completed' ? '#10b981' : oc.status === 'cancelled' ? '#ef4444' : hasDHT ? '#3b82f6' : '#e5e7eb'};border-radius:10px;margin-bottom:8px;background:${oc.status === 'completed' ? '#f0fdf4' : oc.status === 'cancelled' ? '#fef2f2' : hasDHT ? '#eff6ff' : '#fafafa'};">
                <div style="display:flex;flex-wrap:wrap;gap:12px;align-items:center;">
                    <div style="min-width:90px;">
                        ...
                        <div style="font-weight:700;color:${hasDHT ? '#e65100' : '#475569'};font-size:15px;${hasDHT ? 'cursor:pointer;text-decoration:underline;' : ''}" ${hasDHT ? `onclick="window._dhtRestoreModalFn = () => _ctvOpenOrderCodesPopup(${customer.id}); _dhtShowDetail('${oc.dht_order_id}')"` : ''}>${oc.order_code}</div>
                    </div>
...
function _ctvFetchSampleOrderCode() {
...
async function _ctvSubmitConsultLog(customerId) {
    ...
    if (log_type === 'gui_mau') {
        ...
        try {
            ...
            if (data.success) {
                showToast('✅ Gửi mẫu vải/áo thành công!'); closeModal(); window._consultImageBlob = null; loadCrmCtvData();
    ...
    if (log_type === 'huy_don_tra_coc') {
        ...
        if (!reason) { showToast('Vui lòng nhập lý do hủy đơn trả cọc!', 'error'); _ctvEnableSubmitBtn(); return; }
        if (!window._consultImageBlob) { showToast('Vui lòng chụp/dán hình ảnh minh chứng!', 'error'); _ctvEnableSubmitBtn(); return; }
        try {
            const formData = new FormData();
    ...
    if (log_type === 'dat_coc') {
        ...
        const depositAmount = Number(document.getElementById('consultDepositAmount')?.value) || 0;
        if (!paymentRecordId || depositAmount <= 0) {
            showToast('Vui lòng chọn mã tiền đặt cọc từ danh sách!', 'error'); _ctvEnableSubmitBtn(); return;
        }
        if (!appointment_date && !window._currentConsultCustomerPinned) { showToast('Vui lòng chọn ngày hẹn!', 'error'); _ctvEnableSubmitBtn(); return; }
    ...
    if (log_type === 'chot_don') {
        ...
        try {
            ...
            const data = await res.json();
            if (data.success) {
                showToast('✅ Chốt đơn thành công! Mã đơn đã sẵn sàng tạo ở Đơn Hàng Tổng.'); closeModal(); window._consultImageBlob = null; loadCrmCtvData();
            } else { showToast(data.error || 'Lỗi!', 'error'); _ctvEnableSubmitBtn(); }
        } catch (err) { showToast('Lỗi kết nối!', 'error'); _ctvEnableSubmitBtn(); }
        return;
    }
...
const CTV_CRM_LABELS_AFF = { nhu_cau: 'Chăm Sóc KH Nhu Cầu', ctv: 'Chăm Sóc CTV', ctv_hoa_hong: 'Chăm Sóc Affiliate', koc_tiktok: 'CRM KOL/KOC Tiktok' };
const CTV_ROLE_LABELS_AFF = { giam_doc:'Giám Đốc', quan_ly_cap_cao:'Quản Lý Cấp Cao', quan_ly:'Quản Lý', truong_phong:'Trưởng Phòng', nhan_vien:'Nhân Viên', thu_viec:'Thử Việc', part_time:'Part Time', hoa_hong:'Hoa Hồng', ctv:'CTV', nuoi_duong:'Nuôi Dưỡng', sinh_vien:'Sinh Viên', tkaffiliate:'TK Affiliate' };
...
async function _ctvOpenEditAffiliateFromCrm(userId) {
    try {
        ...
        const provinceOptions = CTV_VN_PROVINCES.map(p => `<option value="${p}" ${u.province === p ? 'selected' : ''}>${p}</option>`).join('');
