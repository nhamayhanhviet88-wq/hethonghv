...
if (typeof window._dhtShowCreate !== 'function') {
    const wrapper = async function(preselectedOrderCode) {
        if (typeof _loadScript === 'function') {
            await _loadScript('/js/pages/dht_create.js?v=20260721_san_ready_goods_v2');
        }
...
async function _dhtShowDetail(id) {
    try {
        ...
        if (String(o.id).startsWith('sample_')) {
            ...
            if (shipCK > 0) {
                finRows.push(['🚚 Cước Vận Chuyển (HV Trả CK)', fmt(shipCK) + 'đ', '#f97316', true]);
            }
            finRows.push(
            ...
            if (shipCK > 0) {
                finRows.push(['🚚 Cước Vận Chuyển (HV Trả CK)', fmt(shipCK) + 'đ', '#7c3aed', true]);
            }
            finRows.push(
                ['Còn lại', fmt(finRemaining) + 'đ', remColor, true],
            );
        ...
        var saleNotesHTML = `<div style="font-weight:900;font-size:15px;color:#9a3412;margin-bottom:12px">📋 Sale Dặn Kế Toán Trước Gửi Hàng</div>`;
        saleNotesHTML += `<table style="width:100%;border-collapse:collapse">`;
        saleNotesHTML += row('🚚 Vận Chuyển YC Của Sale', o.carrier_name || '—');
        if (o.carrier_extra) {
            var ce = typeof o.carrier_extra === 'string' ? JSON.parse(o.carrier_extra) : o.carrier_extra;
            if (ce.type === 'nha_xe') {
                saleNotesHTML += row('🚌 Tên Nhà Xe', ce.bus_name || '—');
                saleNotesHTML += row('📞 SĐT Nhà Xe', ce.bus_phone ? '<a href="tel:'+ce.bus_phone+'" style="color:var(--info)">'+ce.bus_phone+'</a>' : '—');
                saleNotesHTML += row('📍 Địa Điểm Xe Đỗ', ce.bus_location || '—');
                saleNotesHTML += row('🗺️ Xe Đi Về Đâu', ce.bus_destination || '—');
                saleNotesHTML += row('🕐 Giờ Xe Chạy', ce.bus_departure_time || '—');
            } else if (ce.type === 'nguoi_nhan_ho') {
                saleNotesHTML += row('🤝 Người Nhận Hộ', ce.proxy_name || '—');
                saleNotesHTML += row('📍 Địa Chỉ Nhận Hộ', ce.proxy_address || '—');
        ...
        saleNotesHTML += row('📷 Ảnh TC', proofHtml);
        ...
        saleNotesHTML += row('📊 Tiến Độ Ra Hàng', progressSaleHTML);
        saleNotesHTML += row('📅 Ngày gửi dự kiến', formatExpectedShipDateWithDay(o.expected_ship_date));
        ...
        saleNotesHTML += row('⏰ Yêu Cầu Chuẩn Giờ Hàng Ra', deliveryTimeHtml);
        saleNotesHTML += `</table>`;
        ...
        if (totalItemsCount > 0) {
            ...
            if (o.shipping_status === 'shipped' || o.shipped_at) {
                ...
                var _payerLabel = o.shipping_fee_payer === 'hv' ? ((o.tracking_code && o.tracking_code.trim()) ? 'HV trả cước vận chuyển' : (o.shipping_fee_method === 'ck' ? 'HV trả CK' : (o.shipping_fee_method === 'tm' ? 'HV trả TM' : 'HV trả cước vận chuyển'))) : o.shipping_fee_payer === 'khach' ? 'Khách trả' : '—';
...
function _dhtShowItemDetail(idx) {
    ...
    if (it.design_pdf_url) {
        const o = window._dhtCurrentOrder || {};
        html += row('File thiết kế PDF', `<a href="${it.design_pdf_url}" download="${o.order_code || 'DONHANG'} - Phieu ${idx + 1}.pdf" target="_blank" style="color:#0284c7;font-weight:800;text-decoration:underline">Mở File PDF</a>`);
    }
    ...
    if (quantities.length > 0) {
        const hasSizes = quantities.some(q => q.size);
        const hasNotes = quantities.some(q => q.note);
        ...
        html += `<table style="width:100%;border-collapse:collapse;font-size:12px"><thead><tr style="background:var(--navy);color:#fff">`;
        if (hasSizes) {
            html += `<th style="padding:6px 10px;text-align:left">Size</th>`;
        }
        html += `<th style="padding:6px 10px;text-align:center">SL</th><th style="padding:6px 10px;text-align:right">Đơn Giá</th><th style="padding:6px 10px;text-align:center">VAT</th><th style="padding:6px 10px;text-align:right">Thành Tiền</th>`;
        if (hasNotes) {
            html += `<th style="padding:6px 10px;text-align:left">Ghi chú</th>`;
        }
        html += `</tr></thead><tbody>`;
        for (const q of quantities) {
            ...
            if (hasSizes) {
                html += `<td style="padding:6px 10px;text-align:left;font-weight:600;color:#475569">${q.size || '—'}</td>`;
            }
            html += `<td style="padding:6px 10px;text-align:center;font-weight:700">${q.qty}</td><td style="padding:6px 10px;text-align:right">${fmt(q.price)}đ</td><td style="padding:6px 10px;text-align:center;font-weight:700;color:#6366f1">${vatPercent > 0 ? vatPercent+'%' : '0%'}</td><td style="padding:6px 10px;text-align:right;font-weight:800;color:#dc2626">${fmt(qBase + (qBase * vatPercent / 100))}đ</td>`;
            if (hasNotes) {
                html += `<td style="padding:6px 10px;text-align:left;color:#64748b">${q.note || ''}</td>`;
            }
            html += `</tr>`;
        }
...
// ========== HELPER FUNCTIONS FOR PRINT FIRST COMPLIANCE ==========
window._dhtPrintSingleSheetImage = function(imgUrl, orderCode, idx) {
    const printWindow = window.open('', '_blank', 'width=900,height=950');
    if (!printWindow) {
        alert('⚠️ Trình duyệt của bạn đã chặn cửa sổ bật lên. Vui lòng cấp quyền mở cửa sổ bật lên để thực hiện in phiếu.');
        return;
    }
    printWindow.document.write(`
        ...
        <html>
        <head>
            <title>In Phiếu Sản Xuất - ${orderCode} - Phiếu ${idx}</title>
            <style>
...
window._dhtPrintAndMark = function(orderId, itemId, idx, imgUrl, orderCode) {
    window._dhtPrintSingleSheetImage(imgUrl, orderCode, idx + 1);
    
    const printKey = `dht_printed_${orderId}_${itemId}`;
    localStorage.setItem(printKey, 'true');
    
    const statusEl = document.getElementById(`print_status_${idx}`);
    if (statusEl) {
        statusEl.innerHTML = `<span style="color:#10b981;display:inline-flex;align-items:center;gap:4px">✅ Đã in phiếu</span>`;
    }
    
    window._dhtCheckAllPrintedAndUnlock(orderId);
};

window._dhtCheckAllPrintedAndUnlock = function(orderId) {
    const itemIds = window._dhtCurrentOrderItems || [];
    if (itemIds.length === 0) return;
    
    const allPrinted = itemIds.every(itemId => {
        const printKey = `dht_printed_${orderId}_${itemId}`;
        return localStorage.getItem(printKey) === 'true';
    });
    
    const warningEl = document.getElementById('dht_print_warning_box');
    const confirmBtnWrap = document.getElementById('dht_confirm_btn_wrap');
    
    if (allPrinted) {
        if (warningEl) warningEl.style.display = 'none';
        if (confirmBtnWrap) confirmBtnWrap.style.display = 'block';
    } else {
        if (warningEl) warningEl.style.display = 'flex';
        if (confirmBtnWrap) confirmBtnWrap.style.display = 'none';
    }
};
