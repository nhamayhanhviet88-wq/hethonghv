...
async function debug() {
    try {
        console.log('--- SIDEBAR TREE ROW BY ROW FOR JUNE 2026 ---');
        // Let's run the tree query's cte logic, but output the distinct orders that form June 2026
        const treeRows = await db.all(`
        ...
        let sumTree = 0;
        treeRows.forEach(r => {
            console.log(`- ${r.order_code} (created_by: ${r.created_by}): ${Number(r.remaining_amount).toLocaleString('vi-VN')}đ`);
            sumTree += Number(r.remaining_amount);
        });
        console.log(`Total Sidebar Tree June 2026: ${sumTree.toLocaleString('vi-VN')}đ (Count: ${treeRows.length})`);
