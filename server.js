...
async function start() {
    ...
    // One-time: reset sewing records salary if not approved yet
    try { await db.exec("UPDATE sewing_records SET salary = 0 WHERE salary_approved = false"); } catch(e) {}
    ...
    // Migration: kv_locations - add restricted_material_id column
    try {
    ...
    try { await db.exec(`ALTER TABLE dht_orders ADD COLUMN IF NOT EXISTS design_email_status TEXT`); } catch(e) {}
    try { await db.exec(`ALTER TABLE dht_orders ADD COLUMN IF NOT EXISTS design_email_error TEXT`); } catch(e) {}
    try { await db.exec(`ALTER TABLE dht_orders ADD COLUMN IF NOT EXISTS design_email_recipient TEXT`); } catch(e) {}
    ...
    try {
        await db.exec(`
            UPDATE dht_orders
            SET design_email_sent_count = COALESCE(edit_count, 0) + 1,
                design_email_sent_at = COALESCE(last_updated_at, created_at)
            WHERE design_email_status = 'sent'
              AND (design_email_sent_at IS NULL OR design_email_sent_count = 0)
        `);
    } catch(e) {}
    ...
    try { await db.exec(`ALTER TABLE dht_orders ADD COLUMN IF NOT EXISTS rescheduled_ship_minute INTEGER`); } catch(e) {}
    ...
    try { await db.exec(`INSERT INTO global_penalty_config (key, label, amount) VALUES ('bcv_overdue_task', 'Bảng Công Việc — Chậm Deadline Công Việc', 100000) ON CONFLICT (key) DO NOTHING`); } catch(e) {}
    ...
    try { await db.exec(`ALTER TABLE dht_order_items ADD COLUMN IF NOT EXISTS custom_layout JSONB DEFAULT '{}'`); } catch(e) {}
    ...
    fastify.register(require('@fastify/compress'), { global: true, encodings: ['gzip', 'deflate'] });
    ...
    fastify.register(require('./routes/luongthoep'));
    ...
    // Mobile Tra Soát Đơn Hàng — standalone touch-optimized page
    fastify.get('/m/trasoatdonhang', async (request, reply) => {
        return reply.sendFile('mobile-trasoatdonhang.html');
    });
    ...
    // Mobile Quản Lý Nhập Kho Vải — alternative path
    fastify.get('/m/quanlynhapkho', async (request, reply) => {
        return reply.sendFile('mobile-quanlynhapkho.html');
    });

    // Mobile Xuất Vải Để Cắt — standalone touch-optimized page
    fastify.get('/m/xuatvaicat', async (request, reply) => {
        return reply.sendFile('mobile-xuatvaicat.html');
    });
    fastify.get('/m/xuavatcat', async (request, reply) => {
        return reply.sendFile('mobile-xuatvaicat.html');
    });
